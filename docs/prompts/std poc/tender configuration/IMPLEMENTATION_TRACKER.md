...
| 9 | [`9. …smoke_test_specification.md`](9.%20procurement_officer_tender_configuration_poc_smoke_test_specification.md) | Smoke path + evidence pack | Partial (MVP) | **MVP:** `npm run test:ui:smoke:procurement-officer` (`officer-tender-poc-mvp.spec.ts`) + step **8** Python module. Full **OFF-ST-001–018** second pass deferred. | Requires `seed_core_minimal` user `procurement.officer@moh.test` (see `.env.ui`). **STD-OFFICER-003** Playwright URL `/desk/…` vs `/app/…`. |
...
| 13 | Run officer smoke (doc 9) and attach evidence | Partial | MVP Playwright `npm run test:ui:smoke:procurement-officer`; full OFF-ST table deferred. | **Officer step 9** row. |

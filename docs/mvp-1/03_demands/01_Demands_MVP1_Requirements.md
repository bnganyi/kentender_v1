...
- KenTender MVP Canonical Demo Data Contract, version 2.0
...
| DIA-FR-002 | The workspace shall default to the current user's entity, organisational scope, roles and assigned work. |
...
| DIA-FR-006 | The Requester and `owner_org_unit` shall default from the authenticated user's active scope assignment. |
| DIA-FR-007 | A user with more than one permitted organisational assignment shall explicitly choose the owning unit. |
...
| DIA-FR-117 | One Demand may be consumed by one or more Plan Items or procurement packages. |
| DIA-FR-118 | Consumption shall identify Demand, Demand Item, Plan Item or package, quantity where applicable, amount and actor/time. |
| DIA-FR-119 | Planning shall not consume more than the approved and unconsumed quantity or amount. |
| DIA-FR-120 | Planning usage shall be derived as Not taken up, Partially planned or Fully planned. |
...
| DIA-FR-123 | Cancellation or reduction in Planning shall return any unconsumed scope to the Demand without creating or duplicating a funding reservation. |
...
- Plan Item or package identifier
...
2. **Create/Edit Demand** — compact guided intake for the business need and Need Items.
...
Before implementation, the canonical demo-data document shall be revised to include the Demand story and version incremented.
...
- Planning usage initially Not taken up, then updateable by the Planning seed extension.
...
| DIA-AC-008 | Planning can partially and then fully consume the Approved Demand without changing its approval status. |

...
**Document ID:** KENTENDER-MVP-CMOM-1.0  
**Version:** 1.0  
**Date:** 11 August 2026  
**Status:** Draft for product-owner validation — controlling only after explicit approval  
...
| 3 | Procurement Planner | Select the Approved Demand and complete the Plan Item | Draft Plan Item in the applicable Plan Version |
...
1. One selected Approved Demand creates one Draft Plan Item by default.
2. Selecting the Demand and editing the resulting Plan Item are parts of one user journey, not duplicate source-selection steps.
3. Aggregation is shown only when the planner explicitly adds another compatible Approved Demand to the same Draft Plan Item.
4. “Keep separate” must result in separate Plan Items; it cannot be a label stored on one combined item.
5. Indicative lots divide the expected Tender structure within a Plan Item. They do not create Plan Items.
6. An Approved Plan is not edited in place.
7. Adding a Demand to an Approved Plan quietly creates or reuses one Draft successor.
8. The current Approved Plan Version and its existing Tender handoffs remain operational during revision.
9. Only the Draft successor is editable.
10. Approval of the successor replaces the current Plan Version according to an auditable state transition.
...
This operating model becomes controlling only after explicit product-owner approval. Until then, it is the proposed correction baseline and a stop condition for further module expansion.

Acceptance of this document does not automatically approve any existing module requirements, Stitch pack, Cursor pack or implementation. Each must still be reconciled and reissued.

...
**Status:** Active — C01–C06 Done; **C07** remaining (canonical seed / SCN-ADD arithmetic)  
**Date:** 13 August 2026  
...
| PLN-UI-07 / 07A Finance | **UI-07 Done** (literal Stitch drawer + SVC-007). **UI-07A Partial** (same-task shortfall markup + confirm API reject; no SCN-PLN-FUND-SHORT-001) |
...
| PLN-GATE-C03 | **Full re-implement** PLN-UI-01…05 (+ formation services) | Prompt 03 | C02 | Done | UI-01…05 Stitch v1.9 ports; layout guard + Playwright + `make ui-planning-builder-gate` green; AC-016 remains Partial (server reject + Combine disabled) |
...
| PLN-GATE-C05 | **Full re-implement** PLN-UI-07 / 07A / 08 + Finance/professional services | Prompt 05 | C04 | Done | UI-07 + UI-07A + UI-08 literal Stitch; SVC-007…011; `test_submit_plan_for_review` 4/4; `test_record_plan_decision` 4/4; `test_approve_plan_version_gate05` 5/5; layout guard UI-08; Playwright `planning-plan-review.spec.ts` 3/3 + `assertStitchDeskChrome`; `planning-finance-confirm.spec.ts` |
| PLN-GATE-C06 | **Full re-implement** PLN-UI-09 / 10 + successor/publish/handoff | Prompt 06 | C05 | Done | **UI-09 + UI-10 Done** (literal Stitch + SVC-012…015 + `get_plan_update` / `save_plan_update`). Seed SCN-ADD arithmetic remains C07 |
| PLN-GATE-C07 | Canonical seed + regression close-out | Prompt 07 | C06 | Not started | All UI-01…10/07A Done before close-out |

**Makefile targets:** `ui-planning-contribution-gate` removed (C02); absence covered by `test_planning_contribution_absent` inside `ui-planning-approval-gate`. **`ui-planning-finance-gate` added (C05 UI-07)**. Keep/extend workspace/builder gates; add `ui-planning-revision-gate`, final `ui-planning-mvp1-gate`.
...
| PLN-SVC-010 | Record professional decision | Return / Approve trail | C05 | Done | `test_record_plan_decision` 4/4 (recommend / return comment / role deny); Stitch **Return to planner** |
...
| PLN-UIC-002 | Inline form errors (return/confirm notes, formation reason, removal reason) | ktFormErrors | UI-02/04/05A/07/08 | Partial | UI-02 register + UI-05A reason + UI-07/07A return + UI-08 return comment use `ktFormErrors` (Playwright); UI-04 remaining |
| PLN-UIC-003 | Layout / Stitch contract guards | `test_planning_ui_stitch_layout_guard` | Each UI | Partial | UI-01…10 + UI-05A + UI-07A markers (`test_plan_update_fixture_markers` + prior); chrome registry includes `procurement-plan-update` |
...
| PLN-AC-016 | Cross-OU Combine rejected | SVC-004 | C03 | Partial | Server rejects mixed OU; UI disables Combine |
...
| PLN-AC-026 | Successor approve applies removal; new handoff blocks | SVC-011 + SVC-017 | C05 | Partial | `test_concurrent_handoff_blocks_successor_approval`; apply on `approve_plan_version` |
| PLN-AC-027 | Tender/downstream: no action + reject; combined whole only | SVC-017 | C03 | Partial | `test_active_with_handoff_rejected` + `test_combined_item_removed_as_whole`; builder omits overflow |
...
| PLN-NFR-005 | No Message dialog for field validation | UIC-002 | Partial | UI-02/05A/07/07A Playwright: empty reason → inline error, no Message dialog |
...
5. **C07** — Seed v2.6 + SCN-ADD arithmetic + regression; UI-01…10/07A canvases are Done.

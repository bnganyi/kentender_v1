...
| PLN-SVC-006 | Validate Plan | Issue-led Ready + **Stale** after Ready when inputs change | C04 | Done | `test_validate_plan` 2/2 (`test_ready_then_input_change_is_stale`); `effective_validation_status`; fingerprint on Ready; PLN-GAP-FR-003. |
...
| PLN-UI-06 | Plan Item editor | `PLN-UI-06.html` | SVC-005 | Done | Literal Stitch v1.9 port; **Indicative lots expected** default + details visible (PLN-GAP-UI-001); layout guard + `planning-plan-item-editor.spec.ts`; Request Finance creates Awaiting task (SVC-007) |
...
| ID | Work item | Notes | Depends on | Status | Evidence |
|---|---|---|---|---|---|
| PLN-SEED-001 | Rebuild Planning seed to Demo Contract v2.7 | 455m item; Finance **after** Plan Item; `RSV-MOH-0001` only via live `confirm_plan_item_funding`; no contribution/treatment | C02, C05 | Done | `test_planning_mvp_seed_contract` 7/7 + `validate.py` (`planning.rsv_0001_after_plan_item`, `planning.rsv_0001_finance_provenance`, `planning.no_scn_item_at_base`); Demands-only `demands.principal.no_rsv_link` / `budget.canonical.no_rsv_0001`; double-run still one RSV. Corrects Budget-pre-created RSV Done claim (PLN-GAP-SEED-001). |
| PLN-SEED-002 | SCN-PLN-ADD-001 | V1 operational; V2 535m; RSV-MOH-0002 after Finance | C06 | Done | `test_scn_pln_add_001` 7/7 — Anne correct+submit + James `record_business_decision(Support)` → Planning Ready **no Demand RSV**; `stop_before_finance` Draft 535m + V1/Tender live; Finance then `RSV-MOH-0002`; approve V2. PLN-GAP-SEED-006. |
...
| PLN-SEED-004 | Personas USA | Requester, HoD, Planner, BO, HoP, Viewer — explicit PE/OU | C01 | Done | `test_seed_004_persona_usa` — plus `kisumu.viewer` Planning Viewer + PE-CGKIS (PLN-GAP-SEED-007); Peter also Viewer so UI-08 opens without Reviewer/Approver |
...
| PLN-AC-003 | UI-06 no Demand reselect | UI-06 + layout + server deny | C04 | Done | Layout guard forbids Add another Demand + `assertNotIn("aggregate_plan_allocations", live)`; Playwright `planning-plan-item-editor.spec.ts`; `test_aggregate_plan_allocations` 2/2 hard-deny. PLN-GAP-FR-001. |
...
| PLN-AC-005 | Cannot edit HoD facts in Planning | SVC-005 negatives | C04 | Done | `test_hod_owned_facts_rejected`; UI-06 copy + `/desk/demand-detail/` or `/desk/demand-form/` (`demand_desk_route`; `planning-plan-item-editor.spec.ts`). PLN-GAP-FR-005. |
...
| PLN-AC-018 | Strategy SVC pass-through unchanged | Handoff | C06 | Done | `test_copies_demand_strategy_and_pvc_snapshots` + `test_strategy_and_pvc_writes_ignored_from_editor`; seed V1 IV backfill from Demand labels; handoff `snapshot_json` copies IV `strategy_snapshot` / `pvc_snapshot` (`test_creates_immutable_snapshot_and_blocks_propose_removal`). PLN-GAP-FR-002. |
| PLN-AC-019 | Tender take-up Active only + snapshot; **Initiator only** | SVC-014 | C06 | Done | `test_create_planning_handoff_snapshot` 4/4 (Initiator happy; Planner/Viewer deny); UI-09 reads take-up (`Tender active` + code); omit Propose when handoff; Playwright handoff case. Corrects prior Planner-as-actor Done claim (PLN-GAP-PERM-001). |
...
| Date | Change |
|---|---|
| 2026-08-14 | **PLN-AC-005 evidence** — Demands amend copy + Desk routes (`/desk/demand-detail/` / `/desk/demand-form/`). See gap tracker Wave 3 FR-005. |
| 2026-08-13 | **Wave 1 correction (FR-001 / PERM-002/003 / SEED-003/004 / FR-002)** — hard-deny `aggregate_plan_allocations`; `get_available_actions` + Viewer View-only Finance queue; James/Grace/Peter seeded; `include_scn_fund_short` / `include_scn_remove`; handoff Finance/Strategy lineage. AC-003 / AC-018 / SVC-014 Done with new evidence. See gap tracker Wave 1. |
| 2026-08-13 | **Wave 0 correction (PERM-001 / SEED-001 / SEED-002)** — `CAP_PLAN_HANDOFF` Initiator-only take-up (`test_create_planning_handoff_snapshot` 4/4); Option C `RSV-MOH-0001` via live Finance (`test_planning_mvp_seed_contract` 7/7); 019 HoD scope return (`test_dem_seed_002_returned_shortfall`). SVC-014 / AC-019 / SEED-001 Done with new evidence. See gap tracker Wave 0. |

...
**Status:** Active — Gate 00–05 Done; next Gate 06 (revision / implementation)  
**Date:** 11 August 2026  
**Authority:** `PLANNING-MVP1-REQ-1.5` → approved Stitch `PLN-UI-01`…`10` → Stitch Prompts v1.6 → `PLANNING-MVP1-CURSOR-1.4` → Canonical Demo Data Contract v2.5 → Org Scope Model  
...
**Overall:** Gate 00 Done. RET-001…005 Done. Gate 01 Done. Gate 02 Done. Gate 03 Done ([GATE_03_WORKSPACE_AND_REGISTER.md](GATE_03_WORKSPACE_AND_REGISTER.md)). Gate 04 Done ([GATE_04_DEMAND_AND_PLAN_ITEM_EDITOR.md](GATE_04_DEMAND_AND_PLAN_ITEM_EDITOR.md)). Gate 05 Done ([GATE_05_VALIDATION_AND_APPROVAL.md](GATE_05_VALIDATION_AND_APPROVAL.md)) — contribution + consolidated review/approval. **Next:** Gate 06 (`PLN-UI-09`…`10`).
...
| Doc | Role |
|---|---|
| [Procurement_Planning_MVP1_Requirements_v1.5.md](Procurement_Planning_MVP1_Requirements_v1.5.md) | Locked behaviour / FR / AC / NFR |
| [Procurement_Planning_MVP1_Stitch_Prompts_v1.6.md](Procurement_Planning_MVP1_Stitch_Prompts_v1.6.md) | Approved design prompts |
| [Procurement_Planning_MVP1_Cursor_Implementation_Pack_v1.4.md](Procurement_Planning_MVP1_Cursor_Implementation_Pack_v1.4.md) | Ordered Gates 00–08 |
...
| [Contract v2.5](../00_common/KenTender_MVP_Canonical_Demo_Data_Contract_v2.5.md) | Fixture identities + `SCN-PLN-ADD-001` |
| [Org scope model](../00_common/00_KenTender_Procuring_Entity_and_Organisation_Scope_Model.md) | PE + Organisation Unit |

**Precedence on conflict:** Requirements v1.5 > Cursor pack v1.4 > Canonical Demo Data Contract v2.5 > approved Stitch outputs and Stitch prompts v1.6 > repo conventions that do not conflict.
...
| PLN-SVC-008 | Departmental contribution submit | `submit_departmental_contribution` | PLN-SVC-007, PLN-SCH-007 | Done | `test_submit_departmental_contribution` 5/5; `planning-contribution-drawer.spec.ts`; `make ui-planning-contribution-gate` |
| PLN-SVC-009 | Submit for review | `submit_plan_for_review` | PLN-SVC-008 | Done | `test_submit_plan_for_review` 4/4; builder CTA → review |
| PLN-SVC-010 | Record plan decision | `record_plan_decision` | PLN-SVC-009 | Done | `test_record_plan_decision` 4/4 (recommend / return comment / role deny) |
| PLN-SVC-011 | Approve plan version (atomic) | `approve_plan_version` | PLN-SVC-010 | Done | `test_approve_plan_version_gate05` 4/4; In review + Recommend + Ready; Effective-once |
...
| ID | Work item | Notes | Depends on | Status | Evidence |
|---|---|---|---|---|---|
| PLN-INT-001 | Demands: eligibility + effective Planning Consumption on approval | No Demand mutation of scope/ownership | PLN-SVC-004, PLN-SVC-011 | Done | Eligibility + add without Demand mutation; consumption on approve via Gate 05 `approve_plan_version` |
...
| PLN-UI-06 | Plan Item editor | `PLN-UI-06.html` | PLN-SVC-005, PLN-SVC-007 | Done | REQ v1.5 Preference and reservation (literal Stitch); retired questionnaire; Strategy in Source Demand; builder coverage; `test_update_plan_item` 8/8; layout guard; `planning-plan-item-editor.spec.ts` |
| PLN-UI-07 | Departmental contribution drawer | `PLN-UI-07.html` | PLN-SVC-008 | Done | Literal Stitch drawer; layout guard; Playwright open/inline declaration error/submit → Submitted; `make ui-planning-contribution-gate` |
| PLN-UI-08 | Consolidated review and approval | `PLN-UI-08.html` | PLN-SVC-009…011 | Done | Literal Stitch `procurement-plan-review`; layout guard; `planning-plan-review.spec.ts` 2/2; chrome registry; `make ui-planning-approval-gate` |
...
| PLN-UIC-001 | Stitch Desk chrome registry + `kt-stitch-canvas` + `assertStitchDeskChrome` | Shared chrome gate | PLN-UI-01…10 | Partial | Gate 03–05: workspace/register/builder/editor/review registered; UI-04/UI-07 overlay chrome in Playwright; UI-09…10 later |
| PLN-UIC-002 | Inline form errors (`kt_form_errors`); no Message dialog for field validation | Form error rule | PLN-UI-02, PLN-UI-06 | Done | Register + Plan Item editor + UI-08 return comment (`planning-plan-review.spec.ts`) |
...
| PLN-AC-004 | Complete method/schedule/lotting + optional preference designation in editor (source at UI-04) | PLN-UI-06 | PLN-UI-06, PLN-SVC-005 | Done | `test_update_plan_item` preference cases + editor Playwright |
| PLN-AC-005 | HoD submits unit contribution (drawer, not separate workspace) | PLN-UI-07 | PLN-UI-07, PLN-SVC-008 | Done | Builder overlay (not page); SVC-008 + `planning-contribution-drawer.spec.ts` |
| PLN-AC-006 | Consolidate contributions; resolve aggregation candidates | PLN-SVC-006 + review | PLN-SVC-006, PLN-UI-08 | Done | Aggregation SVC-006 + UI-08 review DTO/items/statutory; `get_plan_review` + Playwright |
| PLN-AC-007 | Review / AO / Designated Approver per configured route | `record_plan_decision` | PLN-SVC-010, PLN-UI-08 | Done | `test_record_plan_decision` + Reviewer/Approver Playwright rail modes |
| PLN-AC-008 | Approval locks version; Draft allocations effective once | `approve_plan_version` | PLN-SVC-011 | Done | `test_approve_plan_version_gate05` happy path Effective-once |
...
| PLN-AC-015 | Cannot approve while statutory minimums unmet | validation + approve | PLN-SVC-007, PLN-SVC-011 | Done | `approve_plan_version` requires validation Ready; `test_approve_plan_version_gate05` + validate Ready gate |
...
| PLN-AC-025 | Aggregation / public-value not shown as realised without evidence | UI + DTO asserts | PLN-UI-06, PLN-UI-09 | Partial | Planned set-aside note only; no realised claims; UI-09 later |
...
| PLN-NFR-002 | Approval / supersession / effective allocations transactional + idempotent | concurrency / double-submit | PLN-SVC-011, PLN-AC-008 | Done | Gate 05 `approve_plan_version` + Gate 01 Effective-once / concurrency token |
...
| PLN-ABS-020 | No user-maintained statutory percentages on Plan Items | PLN-UI-06 | Done | Preference designation only; coverage derived on builder; retired questionnaire cleared |
...
| PLN-GATE-05 | `ui-planning-approval-gate` | PLN-UI-07…08 + roles | PLN-UI-07…08 | Done | [GATE_05_VALIDATION_AND_APPROVAL.md](GATE_05_VALIDATION_AND_APPROVAL.md); SVC-009…011 + `planning-plan-review.spec.ts` |

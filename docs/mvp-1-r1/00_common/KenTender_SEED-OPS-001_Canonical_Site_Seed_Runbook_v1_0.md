...
make seed-canonical SITE=kentender.midas.com WIPE=True FORCE=True                    # full wipe + rebuild from nothing, §4
make seed-canonical SITE=kentender.midas.com WIPE=True FORCE=True RESEED=False       # full wipe, nothing rebuilt — empty database
```
...
| `reseed` | `True` | `False` stops immediately after clearing/wiping — no stage is rebuilt, and `through`/`validate` are ignored (there is nothing left to validate against). Combined with `wipe=True` this is the actual "empty database, no data at all" mode: everything KenTender's seed owns is gone and nothing replaces it, leaving the site in the same unconfigured state CFG-CHG-002 calls CONFIG-FIRST. Exposed on `make seed-canonical` as `RESEED` since v1.4. |
...
| `wipe`/`WIPE=True` runs clean, but Strategy/Budget/Needs/Planning/etc. all still show data afterwards | That's `reseed` defaulting to `True` — every documented example through v1.3 reseeds the full chain right after clearing it, by design (`through` controls how far it reseeds, not whether). For an actually empty database, add `reseed=False` (`RESEED=False` on the make target), added in v1.4. |

...
| Supersedes | v1.8, proposed 11 September 2026 — superseded in full by this amendment; re-approval required |
...
> **Editorial note.** v1.6 changes no decision made in v1.5. It records the version bump caused by extracting shared standards material into KT-STD-001 and by that standard moving to v1.1. Every domain model rule, business rule, service contract, error code, acceptance criterion and its identifier is unchanged. **References elsewhere to AUTH-ADR-001 v1.5 — including NDS-CHG-001 v1.4 and CFG-CHG-002 — remain semantically correct and need no edit.**
...
While the derived status (§4.6) is **Scheduled**, an administrator permitted to grant both the current and the new responsibility may change `user`, `business_role`, `organisation_unit`, `appointment_type`, `authority_reference`, `effective_from` and `effective_to`; nothing has been decided under the assignment yet, so there is no historical authority to protect. Once the derived status has ever been Active, Expired or Revoked, none of these fields changes again: the assignment is revoked and replaced. The record itself enforces this, whatever path attempts the write.
...
**Correction.** That paragraph states the right; nothing in this document, until now, stated the mechanism, and BUD-CHG-001's Budget workspace was found scoping its detail-read route to Active, Superseded and Closed versions only — Draft and Submitted for approval returned Not Found for Administrator, silently, because no rule here said they must not. The right was never in question. Its absence of a stated mechanism is what let a module built against this document still fail to honour it.
...
| `SearchTechnicalRecords(query)` | Administrator/System Manager only, otherwise Not found. Returns up to 25 rows of reference, record type, title, status and route matching `query` by reference or title, across every registered module, exact reference match first. |
| `ResolveTechnicalReference(reference)` | Administrator/System Manager only, otherwise Not found. Returns the one route for an exact KenTender reference. |
...
| `PreviewResponsibilityAssignment(user, role, ou, appointment, dates, assignment?)` | Return required fields, exact scope and descendant count, overlap and exclusivity findings and the human summary. Create nothing. When `assignment` names the Scheduled record being changed, that record is left out of the overlap and exclusivity findings and every other overlap is a finding — a change can never merge into another record. |
...
| `UpdateScheduledResponsibility(id, user, role, ou, appointment, dates, authority_reference, expected_version)` | Only while the derived status is Scheduled; refused with `AUTH_STATE_CHANGED` once the assignment has started. Recompute the preview with the record itself excluded, validate, apply the change, append one history event carrying the before and after of every field that moved, and re-synchronise the Role projection for the previous and the new holder — atomically. An unchanged request writes nothing. |
| `GetResponsibilityAssignment(id)` | Full authorised detail, audit, diagnostics and the actions the state allows (revoke; change while Scheduled). |
...
- For a **Scheduled** assignment only, a secondary button **Edit scheduled assignment** to its left. It opens AUTH-DES-04/05 prefilled with the current values, titled **Edit scheduled assignment**, with the primary **Save changes** and, above the fields, the notice: *This assignment has not started yet, so any of its details can still be changed. Clear Effective from to bring it into force now.*

A change appears in the Administrative history as **Scheduled assignment changed**, with the changed fields beneath it as `Label: before → after` (for example *Effective from: 1 Dec 2026, 00:00 EAT → now*).

Do not show an Edit action for an Active, Expired or Revoked assignment, a Procuring Entity row, a Financial Year row or an approval trail.
...
| Forbidden | Heading **You do not have access to System setup**; text **This area needs Administrator or System Manager access. Ask your KenTender administrator to grant it.**; no table or action. |
...
### 13.10 AUTH-DES-09 — Technical record search

**Fixture context — outside the artboard:** Administrator · `administrator@moh.example.test` · Technical only · 1 Dec 2026, 09:00 EAT · Frappe header breadcrumb: **Home > Technical record search**.

**Page header**

- Eyebrow: **TECHNICAL ACCESS**
- Title: **Technical record search**
- Description: **Find any KenTender record by reference or title. Read-only; technical access grants no business action.**

**Search**

- One search input, placeholder: **Search by reference or title**. Submit on Enter. No other filter.

**Results table**

| Reference | Record type | Title | Status | |
|---|---|---|---|---|
| NDS-MOH-2027-0001 | Departmental Need | National digital health infrastructure upgrade | Accepted for planning | Open |
| DPP-MOH-00187-2027-001 | Departmental Plan | Digital Health | Accepted | Open |
| PLN-MOH-2027-001 | Annual Plan | Annual Procurement Plan FY 2027/28 | Awaiting statutory approval | Open |

At most 25 rows. An exact reference match is listed first. **Open** is a right-aligned link per row.

**States**

| Variant | Exact visible content |
|---|---|
| Empty (before any query) | Heading **Enter a reference or title**; text **Any KenTender record — need, plan, budget, requisition or tender — opens read-only from here.** |
| No match | Heading **No record matches "{query}"**; text **Check the reference and try again.** |
| Forbidden | Heading **You do not have access to Technical record search**; text **This area needs Administrator or System Manager access. Ask your KenTender administrator to grant it.** (KT-STD-001 §3A.4 technical variant) |
| Error | Per the shared load-error component. |

### 13.11 Existing Frappe and KenTender controls

Frappe supplies the Desk header, breadcrumb, session controls, route lifecycle, dialogs, toasts, the tree control and accessibility primitives. KenTender supplies the established tokens and shared Vue components. Claude Design supplies only the page content defined in sections 13.2–13.10.
...
- A Scheduled assignment additionally shows **Edit scheduled assignment**. It reuses the assign dialog (§14.3) prefilled with the current values; every field may change; the server preview runs with the record itself excluded; saving calls `UpdateScheduledResponsibility` with the detail's version stamp, and a blank Effective from brings the assignment into force immediately. A concurrent change reloads the current state rather than overwriting it.
- There is no Edit action for an Active, Expired or Revoked assignment. An assignment that has been in force is revoked and replaced so historical authority is never rewritten, and the server refuses a change once the assignment has started even if a client presents one.
...
Defined in KT-STD-001 §3. Loading, empty, missing-root, forbidden and error states use the exact copy in AUTH-DES-08.
...
- Administrative history on an assignment is append-only. A change made while the assignment was Scheduled is one appended event carrying the actor, the instant and the before and after of every field that moved.
...
The site, Organisation Units, actors, fiscal years and seed execution rules are defined in KT-STD-001 §8 and are not repeated here.

This document additionally requires that seeds create every User Responsibility Assignment through the administration command in §9.2 rather than by writing the DocType, and that the Cartesian-product fixture in KT-STD-001 §8.3 is present so the regression test in §18.2 has a concrete subject.

The exclusive-office conflict shown in AUTH-DES-05 is an artboard-only fixture used to render the blocked state. It is not a seed record and a seed-versus-artboard comparison shall not report it as a mismatch.
...
| AUTH-AC-030 | An assignment that has ever been in force (Active, Expired or Revoked) cannot be edited, from any path; revoke-and-replace preserves the original assignment and its audit history. |
...
| AUTH-AC-041 | Every module registers its record types and read entry points with the Technical record search and the conformance gate. |
| AUTH-AC-042 | A Scheduled assignment can be changed in any attribute before it starts, re-validated exactly as a new grant would be without conflicting with itself; the change is one appended history event with before and after, and the Role projection follows the holder. |
...
16. Idempotent grant, revoke, change-before-start (every field; refusal once in force and from the record itself; self-exclusion from overlap; the exclusive-office recheck; stale version; no-op), role synchronisation and migration.
...
22. Component tests for tree section, add dialog, assignment dialog variants including the edit variant, descendant preview, Acting fields and validation copy.
23. Register and detail tests for filters, derived status, append-only history, diagnostics, the state-dependent Edit and Revoke actions, and revoke-and-replace.
...
### 18.3 Verification and release evidence

The TDD protocol and release-evidence requirements are in KT-STD-001 §5–6. Because this document defines shared infrastructure, its release gate additionally requires affected-module tests and cross-app contract tests, and its schema scan must prove that `procuring_entity` is absent from Organisation Unit and User Responsibility Assignment, and that the cleanup dry-run output enumerating exactly which User Permission rows would be deleted has been reviewed before execution.
...
AUTH-ADR-001 v1.9 is proposed for product-owner approval. On approval it supersedes v1.8, v1.7, v1.6, v1.5, v1.4, v1.3, v1.2, v1.1 and v1.0 in full and becomes the only KenTender authorization document to consult. v1.7 was approved on 11 September 2026; v1.8 (proposed the same day) and this amendment supersede that approval — v1.8 because v1.7 left the direct-search surface unowned and unnamed, and this version because the owner decided on 21 September 2026 that an assignment which has not yet come into force may be changed rather than revoked and re-created.

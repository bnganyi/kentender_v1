...
| Corrects | Contradictory scope authorities, unbound Role and scope grants, inert organisation hierarchy, per-user Financial Year access, browser-context authority and fragmented setup administration |
| Implementation owner | `kentender_core` for the shared assignment, resolver and unified System setup surface; each domain app for record-state and business-rule checks |
...
This complete document supersedes AUTH-ADR-001 v1.3, v1.2, v1.1 and v1.0 in full.
...
Administrator and System Manager are authorized to create and maintain PE, operating-year, Organisation Unit and User Responsibility Assignment configuration through **System setup**. These direct administrative writes require no submission, review, recommendation or approval state. They are validated and audited.

Setup authority does not authorize a transactional business mutation. To create, certify, approve, reject, submit or otherwise exercise a module business responsibility, the person must have the same active User Responsibility Assignment and pass the same state and segregation checks as any other user.
...
Authorization administration shall be embedded in the single Configuration and Governance **System setup** surface defined by CFG-CHG-002 v0.4:

| Surface | Route | Purpose |
|---|---|---|
| System setup | `/app/system-setup` | Create and maintain PEs, operating years, Organisation Units and role-bound user responsibilities without leaving the page. |

This is a configuration page, not a work queue. It is the only setup entry under **Configuration and Governance**. Raw `/app/procuring-entity`, `/app/financial-year`, `/app/pe-fiscal-year-context`, `/app/organisation-unit`, `/app/user-responsibility-assignment`, `/app/user-permission`, legacy operational-access and scope-assignment pages are not normal operating journeys.

Administrator and System Manager may use this surface without a PE/OU/FY assignment. No setup submission, approval level or assignment-approval task is introduced.
...
### 12.1 AUTH-UI-01 — Organisation structure section

#### 12.1.1 Page purpose and layout

This section manages the selected PE's organisational tree without exposing nested-set internals or leaving `/app/system-setup`.

The shared page header is defined by CFG-CHG-002 v0.4. The section contains:
...
- the shared, changeable **Procuring Entity** selector in the page header.
...
|---|---|
| No PE exists | The System setup page opens **Create procuring entity** automatically. No separate route or dead-end link is shown. |
...
### 12.2 AUTH-UI-02 — Users and responsibilities section
...
The register initially shows assignments for the PE selected in the shared System setup header plus Global assignments. Its visible filters are optional, changeable and non-authoritative. No remembered PE/FY browser context grants or restricts authority.
...
The System setup page shall:

- be the single setup entry under **Configuration and Governance**;
- use the existing KenTender/Frappe shell and header;
- register in `cl_surface_registry` and `STITCH_DESK_SURFACES`;
- use the stable canonical route `/app/system-setup` and preserve selected PE and section on refresh/back/forward;
- provide `data-testid="back-to-workbench"`;
- not create a second shell, separate setup pages, global authority selector or role-specific menu; and
- return to Configuration and Governance rather than raw `/desk`.

Legacy product routes `/app/organisation-structure` and `/app/user-responsibilities` shall redirect to `/app/system-setup` with the corresponding section selected. They shall not remain separate maintained surfaces.
...
Fixture: Administrator · PE-MOH — Ministry of Health · 1 Sep 2026, 10:00 EAT · breadcrumb metadata **Home > Configuration and Governance > System setup**. Show the Organisation structure section within the complete setup page.
...
Fixture: Administrator · 1 Sep 2026, 10:10 EAT · breadcrumb metadata **Home > Configuration and Governance > System setup**. Show the Users and responsibilities section within the complete setup page.
...
| AUTH-AC-016 | The System setup Users and responsibilities section completes a normal grant or revocation without raw User Permission or seed-script work. |
...
| AUTH-AC-023 | Configuration and Governance exposes one System setup page containing Organisation structure and Users and responsibilities sections. |
...
| AUTH-AC-033 | System setup works without a remembered PE/FY browser context and provides a visible, changeable PE selector. |
| AUTH-AC-034 | Direct route, refresh, back/forward and return to Configuration and Governance preserve a valid page state. |
| AUTH-AC-035 | An administrator can complete the full PE → operating year → structure → assign → verify → revoke journey on one route without raw Frappe administration or seed scripts. |
| AUTH-AC-036 | Setup assignments take effect on save and never enter Submit, Review or Approval states. |
...
23. Route tests for the single System setup menu entry, direct load, section anchors, refresh, browser back/forward and return path.
24. Browser test proving PE creation, OU creation and responsibility assignment occur on `/app/system-setup` without a submission or approval control.
...
- the User Responsibility Assignment DocType and its Organisation structure and Users and responsibilities sections within the unified System setup surface in section 12;
...
AUTH-ADR-001 v1.4 is a complete replacement for v1.3. Implementers shall not retain v1.3's separate Organisation structure and User responsibilities routes as active product surfaces.

...
| Supersedes | v0.4, approved 7 September 2026 — superseded in full by this citation correction; re-approval required |
...
| Change type | Updates the one live citation of AUTH-ADR-001's current authorization model from v1.6 to v1.7, found on a full cross-document citation sweep. No comparative finding, verdict or evidentiary content changes. Earlier: v0.4 corrected §10's stated authorization approach, which named the model AUTH-ADR-001 has since replaced, and added the citation to STD-STD-001 v1.1. Nothing else changed; every comparative finding, the verdict, and the required human validation in v0.3 stood. |
...
Approval of STD-ST-001 v0.3 made its comparative findings and minimal product direction binding for subsequent KenTender design, and that remains true. v0.4's own approval carried the same findings forward, stated against the current authorization model as of that version; this citation-only correction to v0.5 is approved and supersedes that approval, changing no finding:

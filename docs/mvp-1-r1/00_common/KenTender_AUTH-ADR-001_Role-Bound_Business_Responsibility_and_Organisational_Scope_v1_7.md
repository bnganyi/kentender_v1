...
| Supersedes | v1.6, approved 3 September 2026 — superseded in full by this domain correction; re-approval required |
| Change type | Corrects §8, found via a real gap in BUD-CHG-001: the Administrator/System Manager technical-read right was stated but never given a concrete mechanism, and a module built against this document scoped a detail-read route to three statuses, silently excluding the rest for a technical reader. §8 now states the mechanism explicitly — every per-record detail-read route resolves for a technical reader regardless of status — so a future module cannot repeat the same omission. No other decision, model, rule, contract or acceptance criterion changes. Earlier: v1.6 was an editorial successor recording the version bump caused by extracting shared standards material into KT-STD-001; no decision, model, rule, contract or acceptance criterion changed from v1.5.
...
A module is not required to build a bespoke workspace-level filter to satisfy this. A single, sitewide direct search that resolves any record to its technical detail read, for Administrator and System Manager only, satisfies both sentences above by itself.
...
AUTH-ADR-001 v1.7 is approved. It supersedes v1.6, v1.5, v1.4, v1.3, v1.2, v1.1 and v1.0 in full and is the only KenTender authorization document to consult. v1.6 was itself approved on 3 September 2026, as an editorial successor; this domain correction to §8 superseded that approval and required its own — this was the first substantive change since v1.5.

This approval authorises §8's corrected technical-read mechanism, and everything v1.5 already established: the `User Responsibility Assignment` DocType and the two System setup sections in section 12; the code-owned role registry with two scope types; the site Procuring Entity as a Single DocType; `Organisation Unit` as the single site-local nested-set tree without a PE field; the shared predicate registered through both Frappe permission hooks; Frappe Role synchronisation as a projection with expiry reconciliation; adoption of the ERPNext `Fiscal Year`; retirement of Frappe User Permission, `User Scope Assignment` and the older capability stores as KenTender authority; removal of per-user Fiscal Year grants, `PE Fiscal Year Context`, PE selectors and authoritative browser context; the controlled migration and scoped cleanup in section 11; and full successor versions of the affected module documents in section 20.

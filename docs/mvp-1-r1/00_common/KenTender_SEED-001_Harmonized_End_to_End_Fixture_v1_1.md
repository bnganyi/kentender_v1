...
| Change type | Corrects §1.1, §4.3 and SEED-AC-003, which stated this scenario's authorisation produces one reservation, `RSV-MOH-2027-033-001`. BUD-CHG-001 §8.2A and REQ-CHG-001 v1.6 §16.4 both establish one reservation per source allocation; with two source allocations (HRMD, DHI) here, authorisation correctly produces two — `RSV-MOH-2027-033-001` (HRMD, KES 20,000,000) and `RSV-MOH-2027-033-002` (DHI, KES 30,000,000) — matching REQ-CHG-001's own fixture, which already had this right. Also updates §1.2, §4 and §5, which described REQ-CHG-001 and TPR-CHG-001 as not yet reflecting this document's target identifiers: both now do, in their own approved v1.6 and v0.6 text, checked directly rather than assumed. Updates §7 to record that KT-STD-001 v1.3 already added the requested fixture-instant row, at §8.4A rather than §8.5. No other fact in this document changes. |
...
**REQ-CHG-001 and TPR-CHG-001 no longer say `PE-KEBS` anywhere.** v1.0 of this document named this as a tracked dependency, blocking on both documents' larger realignment to AUTH-ADR-001 v1.6. That realignment is done: REQ-CHG-001 v1.6 and TPR-CHG-001 v0.6 are both approved and both use the exact identifiers in §4 and §5 below, checked directly against their current text rather than assumed from this document's own account of them.
...
Reflected in REQ-CHG-001 v1.6's own text since its cross-document accuracy pass.
...
Authorisation is the event that creates `RSV-MOH-2027-033-001` (HRMD, KES 20,000,000) and `RSV-MOH-2027-033-002` (DHI, KES 30,000,000) on `MOH-BL-HWD-2027`, one reservation per source allocation per BUD-CHG-001 §8.2A, matching REQ-CHG-001 v1.6 §16.4's own fixture. Nothing before this step touches a Budget balance.

---

## 5. Downstream — TPR-CHG-001's seed of record

Reflected in TPR-CHG-001 v0.6's own text.
...
Approval makes this document the canonical fixture every downstream module's own seed contract must match. It binds the identifiers in §3–§5, including the two-reservation correction in §4.3, as the one scenario exercised end to end; it does not itself authorise implementation, which remains gated by the modules it seeds (per TPR-CHG-001 §19's required walkthrough and STD-TPL-001's own release evidence).

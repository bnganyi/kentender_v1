...
| Supersedes | v1.1, approved 7 September 2026 — superseded in full by this citation correction; re-approval required |
...
| Standards | Governed by KT-STD-001 v1.4. |
| Change type | Closes stale cross-document citations found on a full re-sweep: §1.2 asserted "REQ-CHG-001 v1.6 and TPR-CHG-001 v0.6 are both approved" — wrong on both counts by the time of this correction; REQ-CHG-001 is v1.7 and TPR-CHG-001 is v0.7, and neither is currently approved, both pending re-approval after later domain corrections. Updated to state the current versions and status plainly, noting the identifiers themselves are unaffected. Standards citation and two live REQ-CHG-001 version references (§4, §4.3) updated to v1.7. Earlier: Corrects §1.1, §4.3 and SEED-AC-003, which stated this scenario's authorisation produces one reservation, `RSV-MOH-2027-033-001`. BUD-CHG-001 §8.2A and REQ-CHG-001 §16.4 both establish one reservation per source allocation; with two source allocations (HRMD, DHI) here, authorisation correctly produces two — `RSV-MOH-2027-033-001` (HRMD, KES 20,000,000) and `RSV-MOH-2027-033-002` (DHI, KES 30,000,000) — matching REQ-CHG-001's own fixture, which already had this right. Also updated §1.2, §4 and §5, which described REQ-CHG-001 and TPR-CHG-001 as not yet reflecting this document's target identifiers. Updated §7 to record that KT-STD-001 v1.3 already added the requested fixture-instant row, at §8.4A rather than §8.5. No other fact in this document changes. |
...
| BUD-CHG-001 | §15.4's stale per-Plan-Item reservation — which contradicted this same document's own corrected funding model — retired; the reservations this scenario produces are now correctly shown as Requisition-stage events, one per source allocation per §8.2A, not a Planning-stage one; BUD-DES-06A retargeted from the retired reservation to the real ones |

### 1.2 Corrected since v1.0

**REQ-CHG-001 and TPR-CHG-001 no longer say `PE-KEBS` anywhere.** v1.0 of this document named this as a tracked dependency, blocking on both documents' larger realignment to AUTH-ADR-001. That realignment is done: REQ-CHG-001 v1.7 and TPR-CHG-001 v0.7 are both approved and both use the exact identifiers in §4 and §5 below, checked directly against their current text rather than assumed from this document's own account of them.

**STD-TPL-001** does not need the renaming v1.0 asked for. Its own §3/§13 already explain why: the historical KEBS Tender it cites is kept, deliberately, as the external comparison source that validated the template's structure — a different thing from the golden fixture, which STD-TPL-001 renamed to Ministry of Health in its own v0.4 correction. **STD-TPL-IMP-001** is unreviewed here — it is outside this document set and remains flagged as stale, predating the structured-data model, in the document registry.
...
## 4. Downstream — REQ-CHG-001's seed of record

Reflected in REQ-CHG-001 v1.7's own text since its cross-document accuracy pass.
...
Authorisation is the event that creates `RSV-MOH-2027-033-001` (HRMD, KES 20,000,000) and `RSV-MOH-2027-033-002` (DHI, KES 30,000,000) on `MOH-BL-HWD-2027`, one reservation per source allocation per BUD-CHG-001 §8.2A, matching REQ-CHG-001 v1.7 §16.4's own fixture. Nothing before this step touches a Budget balance.

---

## 5. Downstream — TPR-CHG-001's seed of record

Reflected in TPR-CHG-001 v0.7's own text.
...
**Update.** KT-STD-001 v1.3 corrected its own shared actor registry (§8.3) from "Head of Procurement" to "Head of Procurement Function." If DSP-CHG-001 reads that shared entry rather than carrying its own separately-typed label, this is already resolved. DSP-CHG-001 itself is outside this document set and has not been checked directly — confirm its own text before treating this as closed.

---

## 7. Correction to KT-STD-001 — done

KT-STD-001 v1.3 added the requested fixture-instant row, at §8.4A rather than §8.5 as first asked here:

> **Requisition and Tender Preparation journeys — 1 Mar 2027 through 15 May 2027, EAT.**

No new actor was required. Every actor this scenario uses already exists in the shared register.
...
| SEED-AC-003 | No Budget balance changes before `REQ-MOH-2027-033-001` is authorised; `RSV-MOH-2027-033-001` and `RSV-MOH-2027-033-002` exist only after that event. |
...
| SEED-AC-006 | REQ-CHG-001 and TPR-CHG-001 contain no reference to `PE-KEBS`, `PPI-KEBS-2026-ICT-001` or `TND-KEBS-2026-0001`. |
| SEED-AC-007 | `Head of Procurement Function` is the sole registry name for this office across TPR-CHG-001 and DSP-CHG-001. |

## Approval effect

SEED-001 v1.2 is approved. It supersedes v1.1 in full and is the canonical fixture every downstream module's own seed contract must match. v1.1 was itself approved on 7 September 2026; this citation correction superseded that approval. It binds the identifiers in §3–§5, including the two-reservation correction in §4.3, as the one scenario exercised end to end; it does not itself authorise implementation, which remains gated by the modules it seeds (per TPR-CHG-001 §19's required walkthrough and STD-TPL-001's own release evidence).

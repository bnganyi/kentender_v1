...
| Supersedes | v0.1, approved 10 September 2026 — superseded in full by this domain correction |
...
| Change type | Corrects three real gaps found on review, not cosmetic ones: (1) Addendum was a single free-text `content` field — the same unstructured-blob pattern STD-TPL-001 v0.4 existed specifically to remove from the technical specification. Replaced with `affected_area`, `affected_reference`, `previous_text`, `revised_text` and `reason`, mirroring TenderEvidenceRequirement's link-to-something-real pattern (TPR-CHG-001 §7.4) instead of describing something real in prose. `previous_text` is now checked against the live value at issuance (`TPUB_ADDENDUM_STALE_REFERENCE`), which the old model had no way to do. (2) The Authorise Publication artboard gave the Accounting Officer four shallow fields — a reference, a value, a channel list, a period — before an irreversible statutory act, when the handoff she's acting on carries a full readiness result, a warning set, the rendered documents and an approval trail. TPR-CHG-001's own fixture already has a real Warning on this Tender ("confirm that manufacturer authorisation is proportionate") that v0.1 never surfaced anywhere. (3) That same screen's "what is being procured" section still only went as far as a goods table plus a bare count — "1 technical requirement, 0 related services" — after having just argued a reviewer shouldn't be forced to click through to the full document to decide. Replaced with the same six read-only panels TPR-CHG-001 §13.5 already shows the Head of Procurement Function at his own approval — Goods and delivery, Technical requirements, Warranty and support, Related services, Acceptance requirements, Supporting materials, each with its actual content or explicit empty state — so the Accounting Officer sees what he saw, not a rollup of it. The same panels were added to the published-Tender detail view, since addenda are drafted from that screen against this content. Three acceptance criteria (TPUB-AC-006A, TPUB-AC-006B, TPUB-AC-006C) and one error code make all three fixes verifiable rather than asserted. Earlier: v0.1 established the module. |
...
1. **Awaiting authorisation** — one row: `TND-MOH-2027-033` · Business laptops, 250 Each · Ministry of Health Headquarters, Afya House, Nairobi · authorised value KES 50,000,000.00 · approved by Charles Mutiso, 20 Apr 2027 · readiness **0 Blocking · 1 Warning** · action **Authorise Publication**. Visible only to the Accounting Officer.
2. **Published** — empty in this fixture instant.
...
2. **What is being procured** — read-only, drawn from the handoff's inherited snapshot, not retyped. The same six panels TPR-CHG-001 §13.5 shows the Head of Procurement Function at approval — the Accounting Officer sees exactly what he saw, not a rollup of it:
   ...
   | Related services | Empty state — none. |
   | Acceptance requirements | Empty state — none beyond delivery. |
   | Supporting materials | Empty state — none. |
...
3. **Addenda** — one row, expandable:

   | Field | Value |
   |---|---|
   | Addendum | `ADD-MOH-2027-033-001` · issued by Charles Mutiso · 22 May 2027, 09:00 EAT |
   | Affected area | Goods/delivery schedule — `SRC-MOH-033-001`, `SRC-MOH-033-002` |
   ...
   | Deadline extension | **Not triggered** |
4. **Inquiries** — one row under the addendum above: answered by Brian Wafula, 23 May 2027, 11:00 EAT. Badge **Not published to all candidates**.
...
### 12.8 Dialogs and common states

- **Empty workspace.** Both sections in §12.3 render their own stated empty text when no row exists; no illustration, no call-to-action beyond the section's own meaning.
- **Cancelled Tender.** §12.5's artboard, loaded against a cancelled TenderPublication, replaces the status badge with **Cancelled**, replaces the Actions row with the cancellation's ground, reason and decision timestamp, and removes both action controls — a cancelled Tender accepts no further command, per TPUB-AC-013.
- **Loading and error states** follow KT-STD-001 §3's common page-behaviour standard, unchanged and not restated here.
...
TPUB-CHG-001 v0.2 is approved. It supersedes v0.1 in full and is the Tender Publication requirements document for the first IT-equipment slice, closing the seam TPR-CHG-001 §9.5 has held open since v0.5.

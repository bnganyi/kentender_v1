...
| Starts from | One immutable `TenderPublicationHandoff v1.1`, per TPR-CHG-001 v0.7 — approved |
...
No Frappe User Permission, capability profile or operational-scope assignment participates in any authorization decision in this module, matching every document in this chain since AUTH-ADR-001 v1.7.
...
This section's own six-panel "What is being procured" tables carried exactly this defect until correction — "Business laptops · 250 Each · Ministry of Health Headquarters, Afya House, Nairobi · latest delivery 30 Sep 2027" as one line under one caption, shipped to a rendered screen before anyone caught it. The general rule against that is now centralised in KT-STD-001 §2.2, per that standard's own controlling decision that a rule repeated verbatim across change units is deleted from them once centralised — it is inherited here, not restated.
...
2. **What is being procured** — read-only, drawn from the handoff's inherited snapshot, not retyped. The same six panels TPR-CHG-001 §13.5 shows the Head of Procurement Function at approval — the Accounting Officer sees exactly what he saw, not a rollup of it. Each panel is its own labeled table, not one line of dot-separated facts pretending to be one:
   ...
   **Technical requirements**
   ...
   | Field | Value |
   |---|---|
   | Requirement | `TRQ-MOH-033-001` |
   | Description | Mid-range processor, 16 GB memory, 512 GB solid-state storage, pre-loaded security configuration, three-year warranty and on-site support. |

   This description is stored and rendered as the one inherited text the Requisition carries — it is not decomposed into separate processor/memory/storage fields here, because the domain model does not store them separately, and inventing that split would violate KT-STD-001 §2.2's rule against adding structure not stated. What genuinely is separate data gets separate fields; what is one stored sentence stays one sentence, clearly labelled and set off from anything else, never run into an adjacent fact with a dot.
...
1. **What is being procured** — the same six panels as §12.4's Authorise Publication screen, current as of publication; an officer drafting an addendum from this screen is looking at what they would actually be correcting, not a stale copy. Same labeled-field structure as §12.4 — not restated table-by-table here; see §12.4 for the exact field layout each panel uses.
...
Governed by SEED-001 v1.2. Continues the exact chain: `TND-MOH-2027-033`, approved by Charles Mutiso, 20 Apr 2027.
...
3. **AUTH-ADR-001 v1.7** for role-bound responsibility assignment and registered permission hooks;
4. **CFG-CHG-002 v0.9**, extended per §21, for the effective-dated advertising threshold this document reads but never defines;
5. **STD-TPL-001 v0.6 - Approved**, for the Invitation and issued Tender this document publishes unchanged;
6. **TPR-CHG-001 v0.7 - Approved**, for the exact `TenderPublicationHandoff v1.1` contract and the `AcknowledgeTenderPublicationConsumed`/§9.5 seam this document closes; and
7. **SEED-001 v1.2**, for the exact harmonized identifiers §15's fixtures extend.
...
TPUB-CHG-001 v0.3 is approved. It supersedes v0.2 in full and is the Tender Publication requirements document for the first IT-equipment slice, closing the seam TPR-CHG-001 §9.5 has held open since v0.5. v0.2 was itself approved on 10 September 2026; this domain correction superseded that approval.

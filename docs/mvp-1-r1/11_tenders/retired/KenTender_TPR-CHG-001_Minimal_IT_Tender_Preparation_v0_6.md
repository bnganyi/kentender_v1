...
| Starts from | One authorised Requisition handoff, per REQ-CHG-001 v1.6 — approved, realignment complete |
...
| Implementation authority | Gated by a successful Ministry of Health walkthrough and completed template 1.1 release evidence, per STD-TPL-001 v0.5 |
| Change type | Synchronises this document to REQ-CHG-001 v1.6's actual, approved state, which v0.5 had fallen behind: (1) the compatibility test now names the real handoff, `AuthorisedRequisitionHandoff v1.3`, not the assumed and superseded v1.2; (2) v1.3's two added fields, Strategic Objective (with ancestor path) and plan horizon (with Multi-year justification), are added to the inherited snapshot, the responsibility boundary and Task 1's read-only context, as internal policy context only — visible to the Procurement Officer and Head of Procurement Function, never rendered in the Invitation, the issued Tender or any supplier-visible output; (3) §22's two "required correction" claims against REQ-CHG-001 are removed because REQ-CHG-001 v1.6 already delivered both — the PE/FY-scope/User-Permission removal and a fully designed upstream-correction mechanism at §7.4A — and are replaced with the genuine remaining dependency, an inbound `PlanItemCorrectionRequest` handler in PLN-CHG-001, which REQ-CHG-001 itself already names; (4) the PLN-CHG-001 citation is updated from the stale v1.13 to the current v1.14; and (5) three dangling references to a non-existent "§17" corrections section are corrected to §22, where that content actually lives. One acceptance criterion (TPR-AC-044) and one smoke test (TPR-SMOKE-20) are added to make the internal-only rendering rule verifiable. No task, control, role, lifecycle state, error code or domain scope from v0.5 changes. Earlier: v0.5 removed `pe_fy_context_id` and the native-Frappe-Role/User-Permission authorization model, replacing both with AUTH-ADR-001 v1.6's role-bound `User Responsibility Assignment` and registered permission hooks; added the milestone-actual write-back Planning now depends on; added reservation-category and lotting rendering; renamed the golden fixture from Kenya Bureau of Standards to Ministry of Health and corrected a segregation-of-duties error the fixture had carried; cited STD-STD-001 v1.1 in place of restating its principles. Domain scope — Open Tender, one award package, IT goods and closely related services — is unchanged, and every sound mechanism from v0.4 (the five-task shell, the one-approver model, the digest-everywhere immutability, the upstream-correction route) is carried forward. |
...
| Approved technical PDF published as Section V | Unchanged from v0.4: generate Section V from structured items, technical rows, services and acceptance rows. This correction is now delivered — STD-TPL-001 v0.5 is the template's own version 1.1 release. |
...
| Preference/reservation and lotting carried as inherited data with no rendering rule | Added. Section III renders the inherited reservation category and lotting indicator, per STD-STD-001 v1.1 §8–9 and STD-TPL-001 v0.5 §6.1. |
...
- the structured Requisition handoff, per REQ-CHG-001 v1.6's approved `AuthorisedRequisitionHandoff v1.3`.
...
**This correction is complete.** STD-TPL-001 v0.5 is `IT-EQUIPMENT-OPEN-V1` version 1.1, and it does what this section required:
...
|---|---|
| Handoff version | `AuthorisedRequisitionHandoff v1.3`, per REQ-CHG-001 v1.6 |
| Product pattern | `IT Equipment` |
| Method | Open Tender |
| Currency | KES |
| Award package | One |
| Lotting indicator | `Single lot` |
| Reservation category | `None`, `Youth`, `Women`, `Persons with disabilities` or `Other disadvantaged group` — the categories STD-TPL-001 v0.5 §6.1 supports rendering. Any other value is incompatible in this release. |
...
The fixture uses the exact stable row IDs and content from REQ-CHG-001 v1.6's approved Ministry of Health fixture (§16).
...
4. render tests for structured Section V, reservation-category rendering, supplier-response schema, Invitation/issued-Tender separation, and confirming Strategic Objective and plan horizon are absent from both rendered outputs;
...
The result is **GO**, **SIMPLIFY FURTHER** or **REJECT**. A **GO** walkthrough authorises implementation, provided the template 1.1 release evidence — already delivered by STD-TPL-001 v0.5 — also exists before code binds it.
...
6. **STD-TPL-001 v0.5**, for the reviewed PPRA Goods source, locked text, forms, structured Section V and the completed version 1.1 release evidence;
7. **PLN-CHG-001 v1.14**, as the consumer of the milestone-actual write-back this document supplies in §9.5, and as the document still missing the inbound handler named below; and
8. **SEED-001 v1.1**, for the exact harmonized identifiers this document's fixture uses, including the two-reservation correction in its own §4.3.

Where earlier Tender Preparation or template material treats an attachment as the primary technical requirement, or PE/FY scope and User Permission as the authorization model, this structured boundary and role-bound authorization control. The official PPRA source controls locked standard wording. REQ-CHG-001 v1.6 — approved, and already realigned to AUTH-ADR-001 v1.6, its own compatibility test, the Strategic Objective and plan-horizon fields, and the §7.4A upstream-correction mechanism — controls authorised business requirements. This document controls the Tender Preparation journey and downstream mappings.

**Required correction in another document.** REQ-CHG-001 v1.6 already carries every correction an earlier draft of this paragraph asked for — the PE/FY-scope and User-Permission removal, the Ministry of Health fixture rename, and a fully designed upstream-correction mechanism at §7.4A. What REQ-CHG-001 cannot finish alone is receipt: §7.4A's `PlanItemCorrectionRequest` has an outbound half only. **PLN-CHG-001 needs an inbound handler for `PlanItemCorrectionRequest`, exposed as a Procurement Planner task**, per REQ-CHG-001 §7.4A and §9.1. Until it exists, both REQ-CHG-001's and this document's upstream-correction routes end at a request Planning has no designed way to receive. This is the next document in sequence.
...
TPR-CHG-001 v0.6 is approved. It supersedes v0.5 and all earlier versions in full and is the single Tender Preparation requirements document for the first IT-equipment slice.

Approval authorises everything v0.5 introduced — removal of `pe_fy_context_id` and the native-Frappe-Role/User-Permission model, replaced by AUTH-ADR-001 v1.6's registered permission hooks; the `PublishTenderMilestoneActual` write-back to Planning and its `AcknowledgeTenderPublicationConsumed` trigger; reservation-category and lotting rendering per §7.7; and the Ministry of Health fixture, corrected for the segregation-of-duties error the earlier draft carried — plus, new in v0.6: binding the compatibility test and domain model to the actual `AuthorisedRequisitionHandoff v1.3`; carrying Strategic Objective and plan horizon as internal-only inherited context, per TPR-AC-044; and every citation and cross-reference correction in §1.2 and §22.

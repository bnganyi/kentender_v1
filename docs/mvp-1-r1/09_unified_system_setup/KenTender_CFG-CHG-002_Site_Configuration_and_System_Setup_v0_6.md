...
| Change type | Complete successor to v0.5, which was withdrawn before approval |
...
|---|---|
| Site Procuring Entity; System setup shell, tabs and routing | This document |
...
On approval, CFG-CHG-002 v0.6 supersedes v0.5 and all earlier versions in full, withdraws CTX-CHG-001, and becomes the only KenTender site-configuration document to consult.

Approval authorises: the site Procuring Entity as a Frappe Single DocType with first-run configuration and atomic root-unit creation; adoption of ERPNext `Fiscal Year` with the namespaced flag pattern and optional close instant; ERPNext `UOM` as the sole unit source; the `/app/system-setup` shell with four tabs; the commands in §7; removal of `PEFiscalYearContext`, the KenTender `FinancialYear` DocType, `ProcuringEntityVersion`, the PE register, the PE/FY selector, the `Reference Data Manager` role and every `/app/reference-data/*` route; and the seed and acceptance contracts in §13 and §14.

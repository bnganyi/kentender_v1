...
| Supersedes | v1.5; v1.3 remains withdrawn for cause |
...
| Technical specification PDF as the primary requirement | Unchanged from v1.2: removed. Structured rows are authoritative. TPR-CHG-001 v0.5 is now the corrected consumer this required — that correction is delivered, not still pending. |
| Custom Capability Profiles, Operational Scope Assignments, native Frappe Roles, Workflow permissions and User Permissions | **This is now the defect.** AUTH-ADR-001 v1.6 established role-bound `User Responsibility Assignment`, resolved through registered `permission_query_conditions` and `has_permission` hooks, as the sole authorization mechanism. This is not a new custom layer; it is the one mechanism every other approved KenTender module already uses. |
...
| Reservation category | `None`, `Youth`, `Women`, `Persons with disabilities` or `Other disadvantaged group` — the categories STD-TPL-001 v0.4 §6.1 supports rendering. Any other value is incompatible in this release, checked against the same list Tender Preparation uses, not a second, independently-maintained one. |
...
16. Reservation category and lotting indicator are read-only throughout; only `Single lot` and the reservation categories STD-TPL-001 v0.4 §6.1 renders are compatible with this release.
...
| Administrator / System Manager | — | Technical administration only, per AUTH-ADR-001 v1.6 §8; no Requisition decision. |
...
`GetRequisitionEligiblePlanItem` is the only starting projection. It returns the exact approved facts PLN-CHG-001 v1.13 defines, including reservation category, lotting indicator and, where the Plan Item is a cross-department combine, every contributing Organisation Unit.
...
**This correction is complete.** TPR-CHG-001 v0.5 is the corrected consumer this section previously said had to exist before implementation could join the modules. It no longer treats any attachment as the primary technical requirement.
...
**Renamed from Kenya Bureau of Standards, per SEED-001 v1.0.**
...
- Supporting line: **Clinical training and deployment laptops for digital health rollout · Digital Health, HR Management and Development · KES 50,000,000.00**
...
- Planning drawdown — 2 source lines · 250 Each · KES 50,000,000.00;
- Equipment — 2 items;
- Technical requirements — 11 confirmed rows;
- Related services — 0 rows;
- Acceptance — 5 rows;
- Supporting materials — 0 files.
...
- suitability: **Straightforward IT equipment · Goods · Compatible with first released product · Reservation category None · Lotting Single lot** — every row from §5A's compatibility test, not a single collapsed pass/fail;
...
- a record outside the actor's authority and a genuinely nonexistent record both show **Requisition not found**, per AUTH-ADR-001 v1.6's non-disclosure rule; and
...
| REQ-AC-038 | TPR compatibility receives structured items, technical rows, services and acceptance rows rather than one primary specification PDF — confirmed against TPR-CHG-001 v0.5, not merely asserted as a future requirement. |
...
6. Contract tests against `GetRequisitionEligiblePlanItem` and the current Tender handoff shape TPR-CHG-001 v0.5 consumes.
...
- Register Departmental Author and Head of User Department with `scope_type = Organisation Unit`; register Head of Procurement Function, Procurement Planner, Procurement Officer and Auditor with `scope_type = Site-wide`, per AUTH-ADR-001 v1.6 §4.4.
...
- No production Requisition is authorised until the AUTH-ADR-001 v1.6 registration and the Budget-contract suite both pass.
...
5. **PLN-CHG-001 v1.13** for the eligible Plan Item, its combine rule, and the `PlanItemCorrectionRequest` this document's §7.4A now depends on as a required correction there;
6. **BUD-CHG-001 v1.6** for the `check_funding`/`reserve_funding` contracts this module now actually calls;
7. **TPR-CHG-001 v0.5** for the corrected handoff consumer; and
...
REQ-CHG-001 v1.6 supersedes v1.5, v1.4, v1.3 (withdrawn) and v1.2 (approved 28 August 2026) and all earlier versions in full and becomes the Procurement Requisitions requirements document.

This approval authorises everything v1.3 introduced — removal of `pe_fy_context_id` and the native-Frappe-Role/User-Permission model; the Budget reservation call in §9.1A, closing the gap that has existed since BUD-CHG-001 v1.4; the narrow cross-department relaxation in §2.1, scoped exactly to a Plan Item Planning itself combined; the upstream-correction mechanism in §7.4A; reservation-category and lotting carriage through the handoff; and the Ministry of Health fixture — now correctly delivered alongside the complete v1.2 requirements content it must not have replaced.

This approval also authorises the accuracy corrections in this version: the compatibility test in §5A; the `procurement_category`, Strategic Objective and `plan_horizon` fields added to §5.1 and carried into the handoff, artboards and decision tasks; and the matching correction to PLN-CHG-001's `RequisitionEligibilityProjection`.

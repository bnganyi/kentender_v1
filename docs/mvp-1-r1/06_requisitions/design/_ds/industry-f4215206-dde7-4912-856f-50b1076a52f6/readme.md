...
**`.kt-label`** is the small caption under a figure or beside a value (e.g. "Drawdown lines",
"Plan Item"). Use it instead of dimming caption text with `opacity` — at 11px, opacity-based
dimming on this ground falls to roughly 2.5:1 contrast. `.kt-label` uses a solid neutral-ramp
step (`--color-neutral-700`) plus 600 weight instead, so captions stay legible next to the
bold value above or beside them.

...
| Version | 1.10 |
| Change type | Fixes the two internals the v1.9 rename left implicit and aligns the workspace states with the live build: Revision records keep their generated `-V{nnn}` name suffix (§4.3) and `DepartmentalNeedAccepted.v2` keeps its wire keys (§7.1), so no data migration and no contract bump ride on a label change; the departmental plan's counter is cited as a **Submission** per PLN-CHG-001 v1.16; §11.15 gains the filtered-empty variant the workspace already renders. Earlier (v1.9): Renames the Need's own revision counter from "Version" to "Revision" throughout — the DocType (`DepartmentalNeedVersion` → `DepartmentalNeedRevision`), every field (`version_number` → `revision_number`, `need_version_id` → `need_revision_id`, and five others), every command name, and every UI label ("Draft Version 1" → "Draft Revision 1"). Found via a developer's own confusion reading the live Procurement Planning screen: the Ministry-level Annual Plan and a department's own Need both call their revision counter "Version," and the two appear side by side on the same screen with nothing distinguishing them — confirmed here by checking NDS-CHG-001 §4.3 and PLN-CHG-001 §4.8 directly, both independently defining a field named `version_number` for two different, nested entities. "Version" is kept for the Plan, per PLN-CHG-001, since it already carries the connotation of a formally numbered release at the level this whole document family uses "Version" for. `record_version` (the optimistic-concurrency token), `planning_dependency_version`, and every cross-reference to Planning's own Version (§5.3, §7.2) are unchanged — this is the Need's own counter only. Earlier (v1.8): Corrects `NDS-MOH-2027-0003` and `NDS-MOH-2027-0004`'s quantities (200→100, 300→150) and carries both to Accepted, so they correctly feed the harmonized combined Plan Item in PLN-CHG-001, per SEED-001 v1.0. Earlier versions: v1.7 adopted KT-STD-001 v1.2 §3A page-state rules with no domain change; v1.6 was the complete consolidated successor to v1.4, with v1.5 an interim draft never issued. |
| Date | 11 September 2026 |
| Status | **Proposed for product-owner approval** |
| Supersedes | v1.9, approved 11 September 2026, and all earlier versions, in full; re-approval required |
...
| KenTender-governed ERPNext `UOM` catalogue | Replace with ERPNext `UOM` per CFG-CHG-002 v0.9 §4.4. Needs read only records where `enabled = 1`. |
| `Procurement Budget Line` as the Budget & Funding record name | Rename to `Procurement Budget Line` per BUD-CHG-001 v1.7 §1.1. The identifiers themselves are unchanged. |
...
At most one Fiscal Year may have the flag enabled. Administrator or System Manager changes both fields directly through the Fiscal Years section of System setup, governed by CFG-CHG-002 v0.9. The server records who changed them and when.
...
| Field | Operational purpose and system effect |
|---|---|
| `need_id` / `accepted_revision_id` | Fixes the source revision represented by the projection. |
...
| Update submitted | Accept | Successor becomes Accepted; earlier accepted revision becomes Superseded; new accepted event is published atomically. |
| Update submitted | Do not take forward | Successor becomes Not taken forward; earlier accepted revision remains effective. |

There may be only one open successor per Need. No command edits, deletes or rewrites an Accepted, Superseded, Submitted, Returned, Not-taken-forward or Withdrawn revision.

### 5.3 Accepted withdrawal lifecycle

| Current request state | Live Planning dependency | Decision | Result |
|---|---|---|---|
| Awaiting review | No Active Plan inclusion | Approve | Need and accepted revision become Withdrawn; withdrawal event is published. |
...
| NDS-BR-013 | When Procurement Planning consumes a Need, it consumes only the current Accepted revision and cannot edit, split, partially include or inflate its quantity. This rule does not prohibit a Planning-owned direct departmental requirement. |
...
| NDS-BR-016 | An accepted withdrawal remains pending while the exact Need revision is represented in an Active Plan. |
...
- Direct requirements are Planning-owned and editable only while the applicable DPP Submission (PLN-CHG-001 v1.16 §4.3) is Draft or Returned. The HoD's certification of the complete DPP is the departmental governance control.
- A DPP may contain accepted-Need entries, direct entries or both. A DPP may consist entirely of direct entries.
- A Plan Item may be formed from one or more accepted DPP entries of either source origin. Every source retains its exact DPP-entry lineage; only accepted-Need entries additionally retain Need/revision/hash lineage.
...
- Planning publishes `NeedPlanningUsageChanged.v1` only when an Active Plan starts or stops representing the accepted Need revision.
...
- every current accepted Need must still be accounted for exactly once and at its full accepted quantity in the DPP, unless it is withdrawn or superseded and is no longer the current Accepted revision;
- the DPP may also contain any number of direct departmental requirements authorised within its department and context;
- Plan Item source lineage terminates at the DPP entry for a direct requirement and at the accepted Need revision for a Need-origin entry; and
...
Earlier Budget & Funding text stating that Departmental Needs owns the selected Procurement Budget Line is superseded at this boundary, and BUD-CHG-001 v1.7 §3 records the same ownership. Procurement Planning owns the Procurement Budget Line and indicative amount on each DPP entry; Budget & Funding remains authoritative for Procurement Budget Line identity, eligibility and funding control.
...
| `get_departmental_need` | Need reference and optional accepted revision | Authorised detail, current accepted source, open successor and Planning usage. No mutation. |
...
| `check_accepted_need_withdrawal_dependency` | Need ID and accepted revision | Current Active Plan dependency result and revision token. No mutation. |
...
| `NDS_MAKER_CHECKER` | Revision maker attempted its decision. |
| `NDS_STATE_CONFLICT` | Command is invalid for the current Need/revision state. |
| `NDS_OPEN_SUCCESSOR_EXISTS` | Another accepted successor is already open. |
| `NDS_STALE_WRITE` | Record revision or decision token is stale. Overwrite nothing. |
...
| `NDS_SOURCE_STALE` | Requested accepted revision/hash is no longer current. |
| `NDS_NOT_ACCEPTED` | No current accepted revision exists. |
...
The Needs-submission flag is maintained only in the Fiscal Years section of `/app/system-setup` under CFG-CHG-002 v0.9. Departmental Needs exposes no configuration route or page action.
...
- Eyebrow: **NDS-MOH-2027-0003 · REVISION 2**
...
- Eyebrow: **NDS-MOH-2027-0002 · REVISION 1**
...
- Text: **This revision is read-only while it is in the department review queue.**
...
- Eyebrow: **DEPARTMENTAL REVIEW · NDS-MOH-2027-0002 · REVISION 1**
- Title: **Digital health workforce certification programme**
- Status badge: **Submitted**
- No header action button
...
- Eyebrow: **NDS-MOH-2027-0001 · ACCEPTED REVISION 1**
- Title: **National digital health infrastructure upgrade**
...
- Text: **This accepted revision is included in the Active Annual Procurement Plan.**
- Reference: **PPI-MOH-2027-021 · National digital health infrastructure upgrade**
- Right-aligned secondary button: **View Plan Item**

All fields are read-only. Do not show a revision-history table, source hash, Plan classification, Strategic Objective or Finance position.
...
- Eyebrow: **NDS-MOH-2027-0001 · REVISION 2**
...
- Heading: **Accepted Revision 1 remains in use**
- Text: **The proposed Revision 2 is shown below.**
...
**Fixture context — outside the artboard:** Dr Peter Kimani · `peter.kimani@moh.example.test` · Head of User Department · OU-MOH-DHI — Digital Health · FY 2027/28 · 15 Dec 2026, 10:05 EAT · Frappe header breadcrumb: **Home > Departmental Needs > NDS-MOH-2027-0001 > Revision 2 > Review**

**Page content header**

- Eyebrow: **DEPARTMENTAL REVIEW · NDS-MOH-2027-0001 · REVISION 2**
- Title: **National digital health infrastructure upgrade**
...
| Current accepted revision | Revision 1 |
| Proposed revision | Revision 2 |

**Changes card**

| Field | Accepted Revision 1 | Proposed Revision 2 |
|---|---|---|
...
- Heading: **Departmental decision**
- Prompt: **Should Revision 2 replace Accepted Revision 1 for Procurement Planning?**

**Sticky page footer**

- Left-aligned secondary button: **Return for correction**
- Right-aligned destructive secondary button: **Do not take forward**
- Right-aligned primary button: **Accept updated revision**
...
The former Intake window artboard is withdrawn. No replacement Departmental Needs artboard, route or page action is authorised. The simple **Needs submission** Open/Closed control appears in the Fiscal Years section of the single System setup surface defined by CFG-CHG-002 v0.9.

### 11.12 NDS-DES-11 — Withdrawal request dialog

**Fixture context — outside the artboard:** Grace Wanjiku · `grace.wanjiku@moh.example.test` · NDS-MOH-2027-0001 · Accepted Revision 1 · 5 Jan 2027, 10:20 EAT · Frappe header breadcrumb: **Home > Departmental Needs > NDS-MOH-2027-0001**
...
Produce these variants using the NDS-DES-01 shell, page header and context position.
...
| Filtered empty | Heading **No needs match your filters**; text **Adjust the search or status filter, or clear the filters.**; secondary button **Clear filters**. Shown only when rows exist but none match. |
...
Under the single Needs-submission flag (§4.1) the **Needs submission closed** and **No open Fiscal Year** variants are one server state — no Fiscal Year currently carries the flag — so the strip shows **Needs submission · Closed** and the sentence **Needs submission is currently closed. You can continue viewing existing needs.** appears in both; `get_needs_submission_state` does not distinguish them.
...
- **Continue** and **Correct** route to the actor's editable current revision. **View** routes to NDS-UI-04.
...
- First save requires resolved context and a valid title. It creates the root and Draft Revision 1, then replaces the route with the generated Need reference.
...
- Detail resolves the stable Need, current root state, current accepted revision, open successor state and Planning usage independently.
...
- Return opens the exact Return dialog; decline opens the exact Do-not-take-forward dialog. Accept opens a standard confirmation dialog containing Need reference, revision and the fixed statement **Acceptance makes this revision available to Procurement Planning. It does not approve expenditure or create procurement authority.**
...
- Closing intake disables **Create need** and **Submit**, and leaves **Save draft** enabled on an existing Draft or Returned revision.
...
- Planning events preserve Need/revision/hash lineage. Projection events never alter Need content or decision history.
...
NDS-MOH-2027-0001 Revision 1 is accepted by Dr Peter Kimani on 24 Nov 2026 at 14:00 EAT. NDS-MOH-2027-0002 is submitted by Grace Wanjiku on 24 Nov 2026 at 12:20 EAT.
...
An integration profile projects NDS-MOH-2027-0001 Revision 1 to:
...
A second integration profile projects both NDS-MOH-2027-0003 Revision 2 and NDS-MOH-2027-0004 Revision 1 to:
...
| NDS-AC-009 | Submit creates one immutable revision/hash, one scoped review task and one notification effect atomically and idempotently. |
| NDS-AC-010 | A revision maker cannot decide that revision; expired or cross-scope review assignments fail closed. |
| NDS-AC-011 | Return requires a reason, preserves the submitted revision and creates one copied correction Draft. |
...
| NDS-AC-014 | When Planning consumes a Need, it uses only the current accepted revision at the full accepted quantity. Partial Need usage does not exist. |
| NDS-AC-015 | Planning usage is independent of lifecycle and is only `Not included` or `Fully included`. |
| NDS-AC-016 | An accepted revision is immutable; an open successor does not replace it before acceptance. |
| NDS-AC-017 | Successor acceptance atomically supersedes the earlier revision and publishes exact old/new lineage. |
| NDS-AC-018 | Successor decline leaves the earlier accepted revision current. |
...
| NDS-AC-028 | Stale, duplicate and concurrent commands create no overwritten revision, duplicate task, duplicate event or duplicate decision. |
...
| NDS-AC-033 | Cancelling a Draft accepted successor withdraws only that successor and leaves the earlier accepted revision current. |
...
| NDS-AC-037 | Accepted-Need entries retain Need/revision/hash lineage; direct entries retain DPP-entry lineage and are never presented as accepted Needs. |
| NDS-AC-038 | Expected operational result is present in the immutable Accepted revision and `DepartmentalNeedAccepted.v2`. |
...
| NDS-AC-054 | Closing Needs submission blocks Create need and Submit, leaves Save draft enabled on an existing Draft or Returned revision, and changes no Accepted, Withdrawn or historical record. |
...
1. KT-STD-001 v1.4 for document structure, design closed-input rules, the artboard shell, the shared fixture register, common page behaviour, the verification protocol, release evidence, seed conventions, universal prohibitions and error conventions;
2. AUTH-ADR-001 v1.7 for role-bound User Responsibility Assignments, the site-local Organisation Unit tree, shared scope resolution, non-authoritative module filters, no per-user Fiscal Year assignment and administrator inspection;
3. CFG-CHG-002 v0.9 for the one site PE, the ERPNext Fiscal Year, the Needs-submission flag and its close instant, Organisation Unit identity and the ERPNext `UOM` catalogue;
4. BUD-CHG-001 v1.7 for Procurement Budget Line identity, funding source and currency as consumed by Procurement Planning, subject to the ownership correction in section 7.3;
5. this document for Need capture, review, revisions and withdrawal;
6. STR-CHG-001 v1.7 for Strategic Objectives; and
...
NDS-CHG-001 v1.10 is proposed for product-owner approval. On approval it supersedes v1.9, v1.8, v1.7 and all earlier versions in full and becomes the only Departmental Needs requirements document to consult. It records that Revision record names keep their `-V{nnn}` suffix and that `DepartmentalNeedAccepted.v2` keeps its wire keys (§4.3, §7.1), adds the filtered-empty workspace variant (§11.15) and cites the departmental plan's counter as a Submission. It further authorises the Version-to-Revision rename throughout §4, §8, §10 and §11, and the quantity and acceptance corrections to `NDS-MOH-2027-0003` and `NDS-MOH-2027-0004` in §14.3, per SEED-001 v1.0. Implementation shall adopt AUTH-ADR-001 v1.7 User Responsibility Assignments and the shared resolver; use the site PE implicitly; remove every older scope authority, Fiscal Year user grant, `NeedsIntakeWindow`, PE/FY Context, PE selector, authoritative browser context and pre-entry selector; consume the ERPNext Fiscal Year Needs-submission flag and its optional close instant read-only; keep existing Draft and Returned revisions editable when intake closes; source units from ERPNext `UOM`; adopt the KT-STD-001 v1.4 shared fixture register and standards; and keep pending review work inside the ordinary Departmental Needs workspace rather than a separate **Review tasks** menu.

...
| Change type | Complete consolidated successor to approved v1.2 |
...
This complete document is the single implementation authority for Departmental Needs. It consolidates all approved v1.2 content with the role-bound assignment and organisation-tree rules in AUTH-ADR-001 v1.2 and replaces NDS-CHG-001 v0.2, NDS-CHG-002 v0.1, NDS-CHG-003 v0.1 and NDS-CHG-001 v1.0–v1.2.
...
| Separate Needs Configuration Manager role | Remove. A PE-scoped Procurement Planner responsibility maintains the PE/FY intake window. |
| Separate Role, User Permission, User Scope Assignment or capability records as authority | Remove. Use one role-bound User Responsibility Assignment and the AUTH-ADR-001 v1.2 resolver. |
| Financial Year or PE/FY Context assigned to each user | Remove. Assign durable PE and department scope once. Derive the Need year from eligible Needs intake windows. |
...
- clear PE and department authority with Financial Year owned by each Need and its intake window;
...
- Configuration & Governance owns PE, Organisation Unit, Financial Year, timezone and unit catalogue. `kentender_core` owns the business-role registry, User Responsibility Assignment, Organisation Unit scope resolution and administration surface. Departmental Needs never assigns Financial Years to users.
...
|---|---|---|
| PE, OU, FY, timezone and unit | Configuration & Governance | Resolve exact governed records; do not invent fallbacks. |
| Business responsibility and organisational scope | `kentender_core` under AUTH-ADR-001 v1.2 | Resolve the exact active role-bound assignment and OU subtree; resolve FY eligibility from configured Needs intake windows. |
...
The task is available only to users with an active Head of User Department responsibility assignment whose PE/OU subtree contains the Need. The task routes eligible work but grants no authority. Its Financial Year comes from the Need; it is not a user permission or assignment dimension. The task is not described as assigned to a named person until a decision actor completes it. There is no claim, release, priority, score, due-date or free-text task note.
...
|---|---|
| NDS-BR-001 | Every Need resolves to one authorised PE and OU plus one target FY selected from an eligible Needs intake window. Missing or ambiguous PE/OU authority or an ineligible FY fails closed. |
...
| Procurement Planner | Procuring Entity | Maintain Needs intake windows for assigned PEs; read current accepted Need versions through the typed source contract and exact read-only deep link; no Need decision. |
| Auditor | Procuring Entity or approved oversight scope | Read scoped Needs, versions, decisions and lineage; no business mutation. |
| Administrator / System Manager | Technical read-all under AUTH-ADR-001 v1.2 | Inspect all Needs, versions, tasks and technical metadata read-only; no Need decision unless the person also has the applicable business responsibility assignment. |

User Responsibility Assignment is the sole source of the role-to-PE/OU relationship. Frappe Roles are synchronized framework projections and Frappe User Permission, User Scope Assignment, Capability Profile and Operational Scope Assignment grant no Departmental Needs authority. Organisation Unit assignment includes that node and its descendants within the same PE. Financial Year eligibility derives from the Needs intake window for creation and submission, or from the existing Need for later reads and decisions.
...
A temporary acting HoD receives the same Head of User Department responsibility through a dated Acting User Responsibility Assignment for the exact PE/OU. Do not create a delegate role or another approval level.
...
|---|---|---|
| `resolve_needs_scope` | Actor and required Departmental Needs responsibility | Exact authorised PE/OU pairs and matching assignment IDs from the AUTH-ADR-001 v1.2 resolver. No FY permission or fallback authority. |
| `list_needs_financial_years` | Actor and optional authorised PE filter | Configured Financial Years available for browsing existing Needs in the authorised PE scope. This is not the create-eligibility result. |
| `list_need_create_targets` | Actor and server clock | Authorised PE/OU/FY combinations whose Needs intake window is Open. Zero, one or several results drive the exact Create behaviour in section 12.1. |
| `get_needs_workspace` | Optional PE, OU, FY, status, search and paging filters | Authorised role-specific rows and counts across the actor's durable scope from one predicate. Filters are optional and non-authoritative. |
...
| `save_needs_intake_window` | Create or update one PE/FY window after an active PE-scoped Procurement Planner responsibility, UTC and concurrency checks. No Frappe User Permission or FY assignment is consulted. |
...
|---|---|
| `NDS_CONTEXT_REQUIRED` | No authorised PE/OU or eligible target intake window can be resolved. Create no record. |
| `NDS_SCOPE_DENIED` | Actor lacks the exact current role-bound User Responsibility Assignment for the record's PE/OU scope. Disclose no protected record data. |
...
An effective Procurement Planner opens **Intake windows** from a secondary page action inside the same workspace. It is not a separate business-flow menu entry.
...
| NDS-UI-08 Intake windows | `/app/departmental-needs/intake-windows` | Maintain only PE/FY open and close instants; reached from the workspace page action. |
...
| No authorised scope | Heading **Departmental Needs is not available**; text **You have not been assigned to a Procuring Entity and department for Departmental Needs.**; no table or action. |
| No open intake | Retain authorised existing rows; show text **There is no open Needs intake window. You can continue working with existing needs.**; omit **Create need**. |
...
Produce one 520 px standard modal over NDS-DES-01 only for the fixture state in which the actor has more than one eligible Open intake target.

- Title: **Create need for**
- Text: **Choose the department and Financial Year for this need.**
- Procuring Entity select: **Ministry of Health**; omit the field entirely when only one PE is eligible.
- Department select: **Digital Health**
- Financial Year select: **FY 2027/28**
- Footer buttons: **Cancel** and primary **Continue**

Every option is returned by the server from an authorised PE/OU and an Open Needs intake window. Do not show closed, scheduled or unauthorised options. Do not add a global-context checkbox, remember-context option, date field or administrative instruction.
...
- Load all of the actor's authorised own Needs across their assigned PEs, departments and Financial Years. If the actor is an HoD, also load the in-page decision section under the same route. Search matches title or reference; PE, department, Financial Year and status are optional local filters.
- One PE or department may display directly. Several remain available through ordinary changeable filters; they do not block page entry. The last valid filter may be remembered for convenience but is never authority and always has a visible reset.
- Derive create targets by combining active Departmental Author PE/OU assignments with Needs intake windows whose derived state is Open at the server clock instant. Do not use the current calendar FY, a Financial Year permission, the list's current FY filter or a browser-stored context.
- If exactly one create target exists, **Create need** opens NDS-UI-03 immediately with that PE, department and FY. If several exist, show the standard **Create need for** dialog defined in NDS-DES-15. If none exists, omit the action and show the no-open-intake message while retaining existing rows.
...
- Intake close removes that target from new creation and initial submission; it does not hide existing authorised records or trap the user in that year.
...
- The in-page decision section returns only Open tasks in the actor's exact effective PE/OU review scope after maker-checker exclusion, across every FY represented by those tasks.
- The department register returns authorised Needs in that same PE/OU scope. Optional local filters may narrow by FY; they do not grant actions outside an Open task.
- An acting HoD sees only tasks within the exact PE/OU subtree and effective period of the dated Acting assignment. The Frappe role label alone grants no cross-department access.
...
- Resolve PEs from the Procurement Planner's active PE-scoped User Responsibility Assignments. Resolve Financial Years from configured PE/FY Context records for those PEs. Do not require or inspect Frappe User Permission or a Financial Year assignment.
...
|---|---|
| `grace.wanjiku@moh.example.test` · Grace Wanjiku | Two Departmental Author User Responsibility Assignments: PE-MOH / OU-MOH-DHI and PE-MOH / OU-MOH-HRMD; no Financial Year assignment |
...
| `julia.njeri@moh.example.test` · Julia Njeri | Acting Head of User Department assignment for PE-MOH / OU-MOH-DHI from 1 Oct to 30 Nov 2026 with authority reference |
| `amina.hassan@moh.example.test` · Amina Hassan | PE-scoped Procurement Planner assignment for PE-MOH; may maintain its Needs intake windows across configured Financial Years |
| `mercy.kilonzo@moh.example.test` · Mercy Kilonzo | Procurement Planner for PE-MOH; accepted-source contract and exact detail link only |
| `auditor.moh@example.test` · MOH Auditor | Read-only audit scope for PE-MOH across Financial Years |
...
| NDS-AC-002 | PE/OU authority is explicit and server-authorised; target FY is derived from an eligible intake window or the existing record. First-record and current-FY fallbacks do not exist. |
...
| NDS-AC-021 | Search, counts, rows, detail, export and service access use the same server-side PE/OU scope and record-FY eligibility predicates. |
| NDS-AC-022 | Departmental Author, Head of User Department, acting-HoD, Procurement Planner and auditor permissions match section 6; System Administrator can inspect all records read-only but cannot make a Need decision without the business role and PE/OU scope. |
...
| NDS-AC-044 | One role-bound User Responsibility Assignment and the shared AUTH resolver enforce durable PE/OU scope without Frappe User Permission, User Scope Assignment or a Financial Year grant as authority. |
...
| NDS-AC-048 | With several eligible Open intake targets, the user chooses only PE when needed, department and Financial Year in one compact dialog; closed, scheduled and unauthorised targets are absent. |
| NDS-AC-049 | A Departmental Author assigned to one PE/OU can create in every applicable Open Needs intake window without annual access provisioning. |
| NDS-AC-050 | Choosing or filtering a future, closed or different Financial Year never binds later visits, hides other authorised records or prevents creation in another Open window. |
...
| Permission unit | Own vs department vs acting-HoD vs Planner contract vs auditor, role/scope Cartesian-product denial, OU-descendant and cross-OU/PE denial, multi-FY eligibility without annual assignment, administrator read-only inspection and maker-checker. |
...
1. Replace every Departmental Needs use of Frappe User Permission, User Scope Assignment or module-local scope logic with the shared AUTH-ADR-001 v1.2 resolver and role-bound assignment ID.
2. Remove `Financial Year` from `required_dimensions()`, remove every FY grant check and remove any `allowed_years` identity gate. Retain record/window, state and maker-checker controls.
3. Make `selectable_financial_years()` return governed browsing options under authorized PE/OU assignments. This function supplies filters only.
4. Implement `list_need_create_targets()` separately. It combines active Departmental Author assignments with Open Needs intake windows and returns only eligible PE/OU/FY targets.
...
9. Verify Grace can browse FY 2026/27 and FY 2027/28 without an annual permission edit, can create only against an Open Needs intake window, and is never trapped by a remembered year.
10. Verify a parent-OU HoD assignment covers its two named descendants but never a sibling outside that subtree or an OU in another PE.
...
1. AUTH-ADR-001 v1.2 for role-bound User Responsibility Assignments, the PE-bounded Organisation Unit tree, shared scope resolution, non-authoritative module filters, no per-user FY assignment and administrator read-only inspection;
2. Configuration & Governance for PE/FY/OU/unit identity;
...
| Minimum role-bound responsibilities | Four registered responsibilities cover the module; only Author and HoD make Need lifecycle decisions. PE/OU responsibility assignments are durable and Financial Year is derived from the record/window. |
| No premature abstraction | No generic requirement model is introduced. |

## 20. Approval effect

On approval, NDS-CHG-001 v1.3 supersedes v1.2 in full and becomes the only Departmental Needs requirements document to consult. Implementation shall adopt AUTH-ADR-001 v1.2 User Responsibility Assignments and the shared resolver, remove every older scope authority and Financial Year user grant, remove authoritative browser context and the pre-entry selector, and keep pending review work inside the ordinary Departmental Needs workspace rather than a separate **Review tasks** menu.

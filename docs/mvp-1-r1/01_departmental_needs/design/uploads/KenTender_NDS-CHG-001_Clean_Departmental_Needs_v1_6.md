...
| Supersedes | v1.4 and all earlier versions, in full. v1.5 was an interim draft and was not issued. |
...
Approved 3 September 2026. NDS-CHG-001 v1.6 supersedes v1.4 and all earlier versions in full and is the only Departmental Needs requirements document to consult. Implementation shall adopt AUTH-ADR-001 v1.6 User Responsibility Assignments and the shared resolver; use the site PE implicitly; remove every older scope authority, Fiscal Year user grant, `NeedsIntakeWindow`, PE/FY Context, PE selector, authoritative browser context and pre-entry selector; consume the ERPNext Fiscal Year Needs-submission flag and its optional close instant read-only; keep existing Draft and Returned versions editable when intake closes; source units from ERPNext `UOM`; adopt the KT-STD-001 v1.1 shared fixture register and standards; and keep pending review work inside the ordinary Departmental Needs workspace rather than a separate **Review tasks** menu.

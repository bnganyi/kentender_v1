...
**Status:** Phase 0 complete. Phase 1 complete (schema, 25/25). Phase 2 complete (§5 command set, 40/40; DEBT-04 closed). Phase 3 complete (per-doctype §6 permissions, acting-HoD, 27/27). **Phase 4 complete** (exact §8 contract names, closed §9 code set, §4.7 projection, 23/23). **115 tests green across the module.** Phases 5–10 not started. Three carried debts remain, the largest being that the UI is dark until Phase 7.
**Started:** 2026-08-29
...
| DEBT-03 | **Planning still reads Departmental Needs tables directly**, violating the firm D1 boundary. | Phase 5 | `procurement_planning/services/need_allocations.py` and `plan_need_allocation.py` query `Departmental Need` / `Departmental Need Version` directly. Both carry an explicit boundary note. NDS-910 fails until they go through the published handoff contract. **Narrowed in Phase 4 (NDS-406):** the reverse direction is now clean — Departmental Needs no longer reads `tabPlan Need Allocation` for either usage or the withdrawal dependency, and `get_current_accepted_need` gives Planning the typed source it needs. What remains is Planning's own two modules. |
...
| NDS-AC-024 | Payload includes expected result; excludes the named 9 concepts | 4, 5 | Partial | `get_current_accepted_need` returns the §7.1 field set including `expected_operational_result`, and `test_the_payload_carries_nothing_the_spec_excludes` asserts the absence of all twelve excluded concepts. The **event** form of the same payload is Phase 5 |

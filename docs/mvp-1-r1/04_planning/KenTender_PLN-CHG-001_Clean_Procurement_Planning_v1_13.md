...
**Required correction in another document.** TPR-CHG-001 must publish a milestone-actual projection consumable by `RecordTenderMilestoneActual` at minimum for tender issuance and bid opening — the two milestones an existing module could plausibly own today. No such projection exists in TPR-CHG-001 v0.4. Until it is added, `actual_invitation_date` and `actual_bid_opening_date` remain null in production, identically to the other five milestones, which additionally require a future Evaluation and Award module and a future Contract Management module before they can ever be populated. This is named here as a tracked dependency rather than left to be rediscovered.
...
PLN-CHG-001 v1.13 supersedes v1.12 and all earlier versions in full and, on approval, becomes the only Procurement Planning requirements document to consult. It further authorises the correction in §1.1: `PPI-MOH-2027-033` integrated into the live Active Version on one shared Procurement Budget Line, and the retired `PPI-KEBS-2026-ICT-001` fixture, per SEED-001 v1.0.

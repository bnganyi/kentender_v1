...
| DEBT-02 | `planning_context.py` still resolves PE eligibility through `user_scope_rows` (the scope-assignment read) with the §6 role set as an interim filter | 2 | Opened in Phase 1 (PLN-113). PLN-207 re-bases eligibility onto native User Permission via `permitted_procuring_entities` and deletes the interim read, closing the AUTH-ADR-001 `plan.*` dual-path finding completely. |

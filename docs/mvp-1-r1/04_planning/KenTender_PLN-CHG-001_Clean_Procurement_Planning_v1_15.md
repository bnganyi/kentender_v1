...
| `GetPlanVersion` | Complete Version, all Plan Items, source allocations, Finance state, governance history, current commands and, for an Active Version, the same schedule-health count and a per-item behind-baseline flag. |
...
| `PublishAnnualPlan` | Transmit the exact approved payload and activate only on acknowledged response. Activation seeds every Plan Item's seven forecast fields equal to its baseline fields, in the same transaction. |
...
| DPP 1 | `DPP-MOH-HRMD-2027-001` · Human Resources Management and Development · Version 1 — the department's first DPP |
...
| DPP 2 | `DPP-MOH-DHI-2027-001` · Version 1 — the same DPP root as the first source pair, carrying a second entry |
...
- NDS-CHG-001 v1.9 — optional consultation, direct departmental planning path, six Need values, role-bound departmental scope and Planning-owned DPP funding specification; and
...
PLN-CHG-001 v1.15 is approved. It supersedes v1.14, v1.13, v1.12 and all earlier versions in full and is the only Procurement Planning requirements document to consult. v1.14 was itself approved on 4 September 2026; this domain correction superseded that approval. It further authorises the Need-revision rename in §4.14 and every fixture citing a Need's revision; the correction in §4.14: `RequisitionEligibilityProjection` now explicitly enumerates every field REQ-CHG-001 depends on; and the correction in §1.1: `PPI-MOH-2027-033` integrated into the live Active Version on one shared Procurement Budget Line, and the retired `PPI-KEBS-2026-ICT-001` fixture, per SEED-001 v1.0.

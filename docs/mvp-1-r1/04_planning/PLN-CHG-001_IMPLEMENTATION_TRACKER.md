...
**Status:** In progress. v1.23 closed both v1.22 blockers, so no gate is blocked.
**Started:** —
...
|---|---|---|---|
| PLN22-G00 | Phase 0: plan + tracker written; v1.18 docs retired; `evidence/v1_22/` created with `FRAMES.md`; baseline per-module counts recorded; owner Q1/Q2 raised; checkpoint commit of the uncommitted v1.18 UI; no product code changed | Planned | — |
| PLN22-G01 | Phase 1A: classification provenance and correction — record, command, read, projection, allocation snapshot, four recovery outcomes, four error codes; `test_dpp_classification_correction` green | Planned | — |
| PLN22-G02 | Phase 1B–1C: forecast/cascade/reminder exposure withdrawn; residues removed with patch; `test_planning_v122_schema` planted-violation-proven | Planned | — |
| PLN22-G03 | Phase 1 exit: `make planning-domain-gate` green; three clean `bench migrate`; cross-module checkpoint (Budget, NDS, REQ, TPR) green one module per run | Planned | — |
| PLN22-G04 | Slice 2A — U01 workspace + U21 shared states (22 artboards) per rule 6 | Planned | — |
| PLN22-G05 | Slice 2B — U02–U05 departmental preparation and certification (13 artboards) per rule 6 | Planned | — |
| PLN22-G06 | Slice 2C — U06 validation + classification correction (8 artboards) per rule 6 | Planned | C1 closed by v1.23 |
| PLN22-G07 | Slice 2D — U07 annual plan preparation + U08 add requirements (11 artboards) per rule 6 | Planned | — |
...
| PLN22-G14 | Phase 3: §10.2/§13 seed rebuilt under the frozen clock, idempotent, validate green twice; every blank artboard value supplied; isolated §13.3 profiles; REQ + TPR stages re-run on the new Plan; persona browser pass as each §10.2 actor | Planned | — |

...
| Version | 1.16 |
| Change type | Gives the departmental plan's counter its own word. Three counters reached users under the one word "Version" — a Need's, a departmental plan's and the Annual Plan's — and v1.15 renamed only the Need's, while the counter the developer actually saw beside **Active Version 1** on the workspace was the departmental plan's (`DPPVersion.version_number`). Every actor now reads a departmental plan's counter as **Submission {n}** (§4.3, §5, §8, §9, §11, §12, §14, §15); the model name and field are unchanged because `DPPSubmission` already names the certified snapshot, and `submission_number` now equals the number of the Submission it certifies (§4.5). Also re-bases the text on the live build: v1.15 was written from the committed v1.14 and dropped five approved clauses the code implements and the browser tests pin — restored verbatim in §9, §11.2, §11.18, §12.1, §12.2 and §12.7 — and corrects label and contract-name drift where the build is right (§5.1, §5.2, §8, §11.18, §12.2). Field-table drift is recorded in §1.1 for a later cycle. Earlier (v1.15): Renames every reference to a Need's own revision counter from "Version" to "Revision," matching NDS-CHG-001 v1.9's rename of the underlying field and DocType — `need_version_id` → `need_revision_id`, and every fixture line citing a Need's "Version N" now reads "Revision N." This document's own Plan-level Version — `PlanVersion`, `version_number`, "Active Version 1" — is unchanged; the collision being corrected is between that concept and the Need's, not within this document's own model. Found via a developer's direct confusion reading the live Procurement Planning workspace, where both appeared on one screen under the same word. Earlier: Corrects §4.14's `RequisitionEligibilityProjection` to explicitly enumerate every field REQ-CHG-001 depends on — contributing Organisation Units, Strategic Objective and path, procurement category, reservation category, plan horizon and lotting indicator — none of which the prior text actually named, found during REQ-CHG-001's cross-document accuracy pass. Earlier still: corrects the combined "laptops" Plan Item to one shared Procurement Budget Line, integrates it into the live Active Version as `PPI-MOH-2027-033`, and retires the disconnected `PPI-KEBS-2026-ICT-001` fixture, per SEED-001 v1.0. Earlier still: adds mandatory cascading reforecast, a portfolio schedule-health indicator, a proactive baseline-approaching nudge, category/method-scaled buffer defaults, a reordered schedule card and an explicit live-recompute requirement; and a UX consistency pass rebuilding the PLN-DES-01 workspace, correcting PLN-DES-11's table header and splitting PLN-DES-09's package card into three focused cards, inherited by PLN-DES-09A. Earlier versions: v1.11 replaced the seven flat officer-typed schedule dates with a governed baseline/forecast/actual model and closed contradictions the v1.8–v1.9 corrections left standing (§3, §12.12, §17); v1.10 adopted KT-STD-001 §3A page-state rules with no domain change; v1.9 implemented LAW-REG-001 v1.0 in full. |
| Date | 11 September 2026 |
| Status | **Proposed for product-owner approval** |
| Supersedes | v1.15, approved 11 September 2026 — superseded in full; v1.15 was authored from the committed v1.14 text and omitted five approved clauses the live build implements, restored here; re-approval required |
...
This complete document is the single implementation authority for Procurement Planning. It consolidates approved v1.4 with the one-site-one-PE, role-bound assignment, site-local Organisation Unit and ERPNext Fiscal Year rules in AUTH-ADR-001 v1.7, and replaces the Planning documents listed in section 18 wherever they conflict with it.
...
| Separate Frappe Role, User Permission, User Scope Assignment or capability records as authority | Remove. Use one role-bound User Responsibility Assignment and the AUTH-ADR-001 v1.7 resolver. |
...
|---|---|
| `pe_fy_context_id` on `DPPSubmissionWindow`, `DepartmentalProcurementPlan` and `AnnualProcurementPlan` | **Replace with `fiscal_year`**, referencing the ERPNext `Fiscal Year` governed by CFG-CHG-002 v0.9 §4.2. One site is one Procuring Entity, and `PEFiscalYearContext` no longer exists. |
...
| Business responsibilities scoped to a Procuring Entity | **Site-wide.** Procurement Planner, Accounting Officer, the statutory approver and the Planning Auditor are registered with `scope_type = Site-wide` under AUTH-ADR-001 v1.7 §4.4. Departmental Author and Head of User Department remain Organisation Unit scoped. |
| `Budget Officer` as the Finance confirmation actor | **Rename to `Finance Confirmation Officer`.** BUD-CHG-001 v1.7 §7 owns the role and distinguishes it from Budget Officer, which authors budget versions and holds no Planning task. |
...
| `Procurement Budget Line` as the Budget & Funding record name | Rename to `Procurement Budget Line` per BUD-CHG-001 v1.7 §1.1. Identifiers are unchanged. |
| `ListEligibleBudgetLines` and `ListEligibleStrategicObjectives` taking a Procuring Entity argument | Remove the argument. Budget lines resolve by Fiscal Year and source Organisation Unit; Strategic Objectives resolve by date or Fiscal Year only, per STR-CHG-001 v1.7 §7. |
| KenTender-governed unit catalogue | Replace with ERPNext `UOM` per CFG-CHG-002 v0.9 §4.4. |
...
New in v1.16 — one word per counter, and re-basing on the live build:

| Earlier item | Disposition in v1.16 |
|---|---|
| Three counters shown to users under the one word "Version" — a Need's (NDS-CHG-001 §4.3), a departmental plan's (§4.3) and the Annual Plan's (§4.8) — side by side on the workspace, the departmental plan and the Plan Item editor | **Correct.** Each counter has its own word: a Need's is a **Revision** (NDS-CHG-001 v1.10), a departmental plan's is a **Submission**, and the Annual Plan's remains a **Version**. v1.15 corrected only the Need's; the counter the developer actually saw beside "Active Version 1" on the workspace was the departmental plan's. The `DPPVersion` model name and `version_number` field are unchanged (§4.3) because `DPPSubmission` already names the certified snapshot; only the displayed word changes. `DPPSubmission.submission_number` now equals the number of the Submission it certifies (§4.5). |
| v1.15 authored from the committed v1.14 text rather than the working copy, silently dropping five approved clauses the live build implements and the browser tests pin — FU-14 header links (§12.2, §12.7), FU-15 supporting lines (§11.2), the always-reachable Annual Plan and **Prepare plan update** row (§12.1), the masked "Not available" state (§9, §11.18) and the "name the outcome" action-row rule (§12.1) | **Restore** every clause verbatim, reworded only where it names the departmental counter. A document authored from review is diffed against the working tree before it becomes authority. |
| Copy and contract names where the build is right and the text had drifted: **Begin plan update** as a button label (§5.2), **Open departmental plan** as a control (§5.1, §11.18, §12.2), a separate **Reopen departmental plan** command (§5.1), and §8 read/command names that never matched the whitelisted API | **Correct the text.** The button reads **Prepare plan update** (command `BeginPlanUpdate` unchanged); the control reads **Start departmental plan**; reopening is the same command; §8 names follow the API and list every whitelisted command. |
| §11.18's Publication-failed control read **Retry publication** while §11.15 and the build read **Retry exact approved payload** | **Correct §11.18** to §11.15's words; the build already matches. |
| Field-table drift recorded, not corrected here: `indicative_amount_minor_units` (§4.4, §4.10) against a Currency `indicative_amount`; `source_line_id` (§4.4) never stored; `variance_baseline_days` / `variance_forecast_days` (§4.9) computed per read with sign actual − baseline; `scope` on `PlanGovernanceTask` (§4.12) not stored; the Finance-shortfall and No-validation-tasks states (§11.18) and the **Withdraw departmental submission** / **Cancel update** controls (§5) not yet built | **Record** in FOLLOW_UPS for a later correction cycle; none changes a user-facing outcome in the current build. |

New in v1.12 — UX consistency pass, plus cascading reforecast and digitisation additions:

| Item | Disposition in v1.12 |
|---|---|
| PLN-DES-01 structured as a bordered Financial Year card followed by two disconnected, equally-weighted sections ("Your work" and "Departmental plans") | **Restructured.** Every other screen in this document uses one header, one or two focused cards and a clear action — PLN-DES-01 was the sole outlier. The Financial Year control is now a plain inline filter, not a bordered card competing for attention. The single actionable item is now one prominent headline-plus-button card, matching the summary-strip language used on PLN-DES-07 and PLN-DES-14, not a generic table implying a queue that does not yet exist. The departmental-plans table is retitled to state its relationship to that card rather than reading as an unrelated second dataset. |
| The excluded-Needs fact shown as small grey footer text | **Promoted** to the same amber notice treatment already used for the readiness notice on PLN-DES-02, since a materially important fact should not read as less important than routine table rows above it. |
| PLN-DES-11's immutable Plan table described its final header by giving the original columns and then instructing a column be inserted, and named the same column "Finance" in one place and "Funding" in another | **Corrected.** The table now states its complete, final ten-column header directly, with one consistent name, "Funding." |
| PLN-DES-09's Procurement package card held eleven fields in one flat list | **Split into three focused cards** — Identity, Classification and method, and Preference and structure — so a Planner scans three short, purposeful groups instead of one long undifferentiated list on a screen visited for every Plan Item. |

New in v1.12 — cascading reforecast and digitisation additions:

| Item | Disposition in v1.12 |
|---|---|
| Forecast revision required one manual edit per downstream milestone | **Replaced with mandatory cascade proposal.** Revising any one forecast date always computes and proposes the same shift for every subsequent milestone that has no actual yet. The Planner reviews the full proposed set, may deselect or individually override any row, and confirms once. This is not an optional convenience — the cascade is always computed and always offered; the Planner's only choice is which proposed rows to keep. |
| No portfolio-level view of schedule health | **Added.** The workspace and the Active Plan overview both show how many Plan Items are currently forecast behind their baseline. The underlying data already existed; this is a rollup, not a new capability. |
| Baseline dates recorded with no operational follow-through | **Added a proactive nudge.** A scheduled check compares the current date against each Active Plan Item's next unforecast-passed milestone and raises a notification through the existing shared My Work/notifications surface when the milestone is approaching. It creates no task, no state and no blocking condition — it is a reminder, not a workflow. |
| `award_approval_buffer_days` and `notification_buffer_days` defaulted to one flat value regardless of procurement category, method or value | **Changed to a governed default** sourced from Configuration & Governance's effective-dated reference register, keyed by procurement category and method, so a Low Value item and a full Open Tender no longer receive the same assumption. Named as a required correction to CFG-CHG-002 in §18, since that register does not yet hold this data. |
| The schedule card showed six input controls before any computed result | **Reordered.** The computed baseline table is now the prominent element; the five period inputs collapse into a closed-by-default **Adjust periods** disclosure, since the governed defaults are correct for most Plan Items. |
| The computed schedule implied a save-then-view step | **Stated explicitly as a live recalculation** — every input change updates the computed table before any save, in both PLN-DES-09 and the cascade-review dialog. |

New in v1.11 — governed schedule model:

| Item | Disposition in v1.11 |
|---|---|
| Seven flat, independently officer-typed schedule dates, freely editable with no validation against statutory gaps | **Replaced by a three-tier baseline/forecast/actual model.** The Third Schedule's own column semantics already treat several of these as derived rather than independently entered — column 11 (bid evaluation) is capped at 30 calendar days, column 15 (total time to contract signature) and column 16 (days to completion) are stated as derived day-counts, not free dates. Two further statutory gaps were verified against the primary text and were previously unvalidated anywhere in this document: regulation 86 sets a **minimum seven-day** tendering period for national and county open tenders; section 135(3) sets a **minimum fourteen-day** standstill between notification of award and contract signing, tied to the fourteen-day review window in section 167(1). Nothing in the prior model checked either gap. |
| Baseline computed once and never freely re-typed | **Added.** One Planner-set anchor (`baseline_invitation_date`) plus the Requisition-inherited delivery boundary, carried forward through three governed periods (tendering, evaluation, standstill) and two explicitly-flagged non-statutory planning buffers (award-approval, notification), produce the full baseline schedule. Locked the moment the Plan Version leaves Draft, exactly like every other Draft-then-locked field in this system. Changing it requires a formal Plan successor, never a routine edit. |
| Forecast as a distinct, explicitly tracked layer | **Added.** Meaningful only once the Version is Active; starts equal to baseline; any change is one audited event — old value, new value, reason, actor, timestamp — never a silent overwrite. This is management discipline layered on top of the statutory minimum, not itself a Third Schedule requirement. |
| Actual dates populated by manual entry | **Removed as a design option entirely.** No actor, anywhere in this module, may type an actual date. Actual dates are populated only by a write from the module that owns the real-world event. For MVP-1 that write path does not exist for any of the seven milestones — see the required correction to TPR-CHG-001 in §18. |
| Three internal contradictions surviving the earlier corrections | **Closed.** §3's ownership line, §12.12's interaction description, and §17's blanket prohibition on "lotting or actual milestone fields" all still asserted, verbatim, positions the domain model and acceptance criteria had already reversed. All three are rewritten in this version to say one consistent thing. |
| `plan_horizon`, `multi_year_justification`, `aggregation_indicator`, `lotting_indicator`, `lot_count` defined in the domain model and blocking per invariant 24b, but absent from every artboard and from the readiness card | **Fixed.** Added to PLN-DES-09 and PLN-DES-09A as real controls with fixture values, and to the PLN-DES-07 readiness card as a checked row. |
...
| `DPPSubmissionWindow` as a separate DocType with `opens_at`, `closes_at` and derived states | **Remove.** Replaced by `kentender_dpp_submission_open` and `kentender_dpp_submission_closes_at` on the ERPNext Fiscal Year, following the flag pattern CFG-CHG-002 v0.9 §4.2 requires of every module flag. The window is configuration and belongs in Configuration & Governance. |
...
| Baseline anchor, three governed periods and two labelled planning buffers | Derive the seven baseline schedule dates once, lock them at Version submission, and enforce the two statutory gaps (regulation 86, section 135(3)) that nothing previously validated. |
| Seven forecast dates and their revision log | Let the Planner track a schedule that has slipped from baseline, with every change reasoned and audited, without ever overwriting the baseline itself. |
| Seven actual dates and derived variance | Hold the Third Schedule's column 8 requirement even though, for MVP-1, no write path yet populates them; the fields exist so the future projection in §18 has somewhere to land. |
...
- Departmental Needs owns accepted Need identity, accepted revisions and the six requirement facts supplied to Planning.
...
- Procurement Requisitions owns the Requisition and its drawdown. Procurement Planning owns the Plan Item's baseline and forecast schedule; it never accepts a manually typed actual date. Tender Preparation, and later Contract Management, own the real-world events themselves and are the only permitted writers of `actual_*_date`, through the projection contract named in §18 — a write Planning receives, never solicits by giving anyone an entry field.
...
| Business responsibility and organisational scope | `kentender_core` under AUTH-ADR-001 v1.7 | Resolve the exact active role-bound assignment and PE/OU scope; never infer one from a separate Role or permission row. |
...
| Direct departmental requirement | Procurement Planning | Create and edit only inside a Draft or Returned DPP Submission. |
...
Identifier convention: a field written `x_id` here is the Frappe Link or Data field `x` in the implementation, and a record's own `*_id` is its Frappe `name`. The mapping is one-to-one and is not a divergence.
...
There is **no `DPPSubmissionWindow` DocType**. Departmental plan intake is governed by two namespaced fields on the canonical ERPNext `Fiscal Year`, owned by Configuration & Governance and following the flag pattern that CFG-CHG-002 v0.9 §4.2 requires of every module flag:
...
| `current_version_id` | Points to the current Draft, Returned or Submitted Submission. |
| `current_accepted_version_id` | Points to the latest accepted Submission. Empty until acceptance. |
...
| `based_on_version_id` | Identifies the accepted Submission copied for an update. Empty on Submission 1. |
...
Every actor sees a `DPPVersion` as **Submission {n}** — the department's numbered submission into the Annual Plan — never as "Version", which is reserved for the Annual Plan's counter (§4.8); a Need's counter is a **Revision** (NDS-CHG-001 v1.10 §4.3). The model name and `version_number` are unchanged because `DPPSubmission` (§4.5) already names the certified snapshot. A Draft Submission is mutable. Certification locks its snapshot. A return creates a copied Draft correction and preserves the certified Submission. One accepted Submission may coexist with at most one open successor.

### 4.4 DPPEntry

One departmental requirement in one DPP Submission (§4.3).
...
| `dpp_version_id` | Fixes the containing Submission. |
| `source_origin` | `Accepted Departmental Need` or `Direct departmental requirement`. Required and immutable after creation. |
| `need_id` / `need_revision_id` | Fix the accepted Need source. Required only for Need-origin entries; empty for direct entries. |
...
The immutable HoD-certified snapshot of one DPP Submission.

| Field | Operational purpose and system effect |
|---|---|
| `dpp_submission_id` | Immutable submission and queue reference. |
| `dpp_version_id` | Fixes the certified Submission. |
| `submission_number` | Equals the `version_number` of the Submission it certifies, so the number an actor reads on screen and the number on the certified snapshot never differ. |
...
| `baseline_invitation_date` | The one genuine Planner-set anchor. No statutory rule fixes when a tender should be issued, so this is the sole schedule date a Planner chooses directly. Recalculated on every Draft save; **locked the moment the Plan Version leaves Draft**, exactly like every other Draft-then-locked field in this system. |
| `tendering_period_days` | Planner-set, governed floor **7**, per regulation 86's minimum tendering period for national and county open tenders. The system rejects a value below the floor; it does not silently raise it. |
| `evaluation_period_days` | Planner-set, governed ceiling **30**, per the Third Schedule's evaluation-stage limit. The system rejects a value above the ceiling. |
| `award_approval_buffer_days` | Planner-set. Defaults from Configuration & Governance's effective-dated reference register, keyed by procurement category and method — not one flat value for every item. Falls back to **5** only where the register has no entry for the resolved category and method. This is an internal planning assumption, not a statutory figure — no provision in the Act or Regulations fixes the gap between evaluation completion and award approval. Labelled as an assumption everywhere it is shown. |
| `notification_buffer_days` | Planner-set. Defaults from the same category/method-keyed register as the award-approval buffer, falling back to **2** only where no entry exists. Also an internal planning assumption, labelled as such; no statutory gap governs it. |
| `standstill_period_days` | Planner-set, governed floor **14**, per section 135(3)'s minimum gap between notification of award and contract signing, tied to the fourteen-day review window in section 167(1). The system rejects a value below the floor. |
| `baseline_bid_opening_date` | Derived: `baseline_invitation_date + tendering_period_days`. |
| `baseline_evaluation_completion_date` | Derived: `baseline_bid_opening_date + evaluation_period_days`. |
| `baseline_award_approval_date` | Derived: `baseline_evaluation_completion_date + award_approval_buffer_days`. |
| `baseline_award_notification_date` | Derived: `baseline_award_approval_date + notification_buffer_days`. |
| `baseline_contract_signing_date` | Derived: `baseline_award_notification_date + standstill_period_days`. Blocked by readiness if this would fall outside the Tender's eventual validity period once known; not independently validated against it at Plan stage. |
| `baseline_delivery_completion_date` | Equal to the Requisition-inherited `latest_delivery_date` — the department's own requirement, not independently derived. Readiness blocks where `baseline_contract_signing_date` leaves no sensible implementation allowance before it (see invariant 12a). |
| `forecast_invitation_date` … `forecast_delivery_completion_date` | One forecast date per baseline milestone above. **Null on every Draft, Submitted or Superseded Version.** Seeded equal to the corresponding `baseline_*` field in the same transaction that activates the Version — a system write, the only one this field ever receives automatically. Every write after that goes through the cascade preview-and-confirm commands in §8.2 (`PreviewForecastCascade`, `ConfirmForecastCascade`), always proposed for every later not-yet-actual milestone, and requires a reason. |
| `actual_invitation_date` … `actual_delivery_completion_date` | One actual date per milestone. **No control anywhere in this module accepts a manually typed actual date.** Each is populated only by a write from the module that owns the real-world event — see §18 for the required TPR-CHG-001 correction this depends on. For MVP-1, every actual date is null for every Plan Item, because that write path does not yet exist for any of the seven milestones. |
| `variance_baseline_days` (×7) | Derived, one per milestone: `baseline − actual`, in days. Populated only once the corresponding actual exists. This is the variance the Third Schedule's column 8 requires. |
| `variance_forecast_days` (×7) | Derived, one per milestone: `forecast − actual`, in days. Populated only once the corresponding actual exists. An internal management metric additional to, and never presented as equivalent to, the statutory variance above. |
...
Quantity, unit, planned value, funding source and Procurement Budget Line breakdown are derived from source allocations. The item's **planned value** is the sum of its source indicative amounts, each of which is the full estimated cost of that requirement including insurance, clearing and forwarding, demurrage, warehousing, advertisement and other incidental costs where applicable. A planning estimate net of incidentals is invalid, because the affordability check and every downstream reservation are computed from this figure. Note 7 to the Third Schedule adds that the estimated cost **shall be established through market surveys**, and section 54(2A) makes the head of the procurement function responsible for carrying those surveys out. The plan records the estimate; the market price index and the entity's own survey are its evidence. **Source correction required** is derived when an allocation no longer points to the current accepted DPP entry/version for its stable departmental source.

Contract period, Value Commitment, recommended method and generic basis genuinely do not exist on the Plan Item — those remain correctly excluded. **Lotting and the schedule's actual-date fields do exist**, per the tiered model above; do not read this paragraph as excluding them. Lot specifications — the content of each lot — still belong to Tender Management and are never modelled here; only the lotting indicator and lot count are Plan content.

### 4.9A PlanItemForecastRevision

Append-only audit trail for every forecast change. Exists only once a Plan Item's Version is Active; a Draft Plan Item has no revision history because its forecast has not yet diverged from anything.

| Field | Purpose and rule |
|---|---|
| `revision_id` | Immutable generated reference. |
| `plan_item_id` | Owning item. |
| `milestone` | Which of the seven forecast fields changed. |
| `previous_forecast_date` | The value before this revision. |
| `new_forecast_date` | The value after this revision. |
| `reason` | Required, 20–500 characters. A forecast change with no stated reason is rejected. |
| `cascade_id` | Shared across every row written from the same cascade confirmation, so the audit trail shows a set of related shifts as one event rather than unrelated coincidental edits. Null for a standalone single-milestone revision where every downstream row was deselected. |
| `revised_by` / `revised_at` | Server-set. |

A forecast revision never touches the corresponding baseline field, and never touches `actual_*_date`. Revising a forecast changes no Budget position and creates no downstream event.
...
| `need_id` / `need_revision_id` | Preserves Need lineage only when the source has it. |
...
This is not an inferred approval stage under AUTH-ADR-001 v1.7 §5.8. That rule bars inventing an approver where an approved source names the accounting officer as final authority. Here a regulation names a different authority, so the stage is a legal requirement rather than an inference.
...
Read-only contract containing:

- Plan, Version, Plan Item and allocation lineage;
- **every contributing Organisation Unit** — plural, since a combined Plan Item may have more than one, per its own combine record;
- the Strategic Objective and its full ancestor path, per the immutable snapshot §7 creates at Plan Item approval — carried explicitly, not left to be re-derived by chasing `plan_item_id` back into this module;
- `procurement_category`, `requirement_type_id`, `reservation_category`, `plan_horizon` and, where Multi-year, `multi_year_justification`;
- `lotting_indicator` and, where packaged into lots, `lot_count`;
- approved quantity/value; authoritative drawdown quantity/value; remaining quantity/value;
- current funding state; Active status; and evaluation time.

It creates no Requisition and stores no duplicate operational status on the Plan Item. This is the one contract Requisitions is authorised to depend on; a field this list omits is not available to Requisitions regardless of what that document's own text might claim.
...
|---|---|---|---|
| No DPP | Start departmental plan | DPP root and Draft Submission 1 | Departmental Author or HoD |
...
| Submitted | Accept departmental plan | Submission Accepted with classifications; accepted entries appear in the open Draft Annual Plan | Procurement Planner |
| Returned | Save / Resubmit | Corrected Draft saved or submitted as the next submission | Departmental Author; HoD submits |
| Accepted; change required | Create update | One Draft successor copied from the accepted Submission | Departmental Author or HoD |
| Draft successor | Submit update | Same validation route; Active Plan unchanged | HoD or acting HoD using the same role |
| Draft / Returned with no downstream consumption | Withdraw departmental submission | Submission Withdrawn; prior accepted Submission remains current when one exists | HoD or acting HoD using the same role |
| Withdrawn with no accepted predecessor; initial window Open | Start departmental plan | Same DPP root; next numbered Draft Submission — the same command as the first row, not a separate one | Departmental Author or HoD |
...
| Active; no successor | Prepare plan update | Draft successor; Active Version unchanged | Procurement Planner |
...
| Approved / Publication failed | Publish / Retry exact approved payload | Active only on exact acknowledgement; otherwise failed/indeterminate | System; technical retry by System Manager if required |
...
12. Baseline dates are computed, never independently typed: `baseline_bid_opening_date` shall be at least `tendering_period_days` (minimum 7, regulation 86) after `baseline_invitation_date`; `baseline_evaluation_completion_date` shall be at most `evaluation_period_days` (maximum 30, Third Schedule) after `baseline_bid_opening_date`; `baseline_contract_signing_date` shall be at least `standstill_period_days` (minimum 14, section 135(3)) after `baseline_award_notification_date`.
12a. `baseline_contract_signing_date` shall leave a sensible implementation allowance before `baseline_delivery_completion_date`; a schedule that does not is a Blocking readiness finding, not a silently accepted plan.
12b. Baseline is locked the moment the Plan Version leaves Draft. No command may alter a locked baseline field; only a Plan successor may.
12c. A forecast date changes only through the preview-then-confirm cascade commands in §8.2, only on an Active Version, only with a required reason, and every changed milestone is recorded as its own `PlanItemForecastRevision` row.
12c-i. Revising one forecast milestone always computes a proposed shift, equal to the same day-delta, for every subsequent milestone on that Plan Item whose actual date is not yet set. This proposal is never skipped or hidden; the Planner may include or exclude any proposed row before confirming, including excluding every row to produce a standalone single-milestone change. A row is included or excluded, never independently retyped inside the same confirmation — a genuinely different value for one milestone is achieved by excluding it here and running a separate cascade action on it afterward.
12c-ii. A milestone with an existing `actual_*_date` is never included in a cascade proposal and never receives a forecast revision. What has already happened is not reforecast.
12c-iii. A uniform accepted shift preserves every gap between consecutive forecast milestones exactly as it stood before the shift, so the governed periods that produced the baseline continue to hold without being separately re-validated. Where excluding some rows would leave an included later milestone closer to its immediate predecessor than a governed floor or ceiling permits, that later row is re-validated against the same rules as baseline derivation (regulation 86, the Third Schedule, section 135(3)) and a breach is rejected with the same error code baseline derivation uses.
12c-iv. Every forecast row confirmed in the same action shares one `cascade_id`. A standalone single-milestone revision, with every downstream row deselected, has a null `cascade_id`.
12d. No command, role or interface accepts a manually entered `actual_*_date` anywhere in this module. An actual date is written only by the projection contract named in §18, and is absent for every milestone until that contract exists.
12e. Every forecast field is null until Version activation. `PublishAnnualPlan` seeds all seven forecast fields on every Plan Item equal to their baseline counterparts, in the same transaction that activates the Version — this is the only system-initiated forecast write, and it happens exactly once per Version.
...
| Head of User Department | Organisation Unit | All Author work plus certify, submit, resubmit and withdraw the departmental Submission. Only an effective HoD assignment covering the DPP's OU may act at command time. |
...
| Finance Confirmation Officer | Site-wide | Open the one plan-level Finance task, confirm that the consolidated plan sits within the approved budget, or return it with a reason. Confirmation creates no reservation. This is the role defined by BUD-CHG-001 v1.7 §7; Budget Officer authors budget versions and holds no Planning task. |
...
| Administrator / System Manager | Technical read-all under AUTH-ADR-001 v1.7 | Inspect all Planning records, tasks, files and evidence; no Planning decision without the applicable business responsibility assignment. |
...
`DepartmentalNeedAccepted.v2` supplies event identity, Need/revision lineage, PE/OU/FY, title, description, expected operational result, quantity, unit and required-by date. Projection is idempotent.
...
- `NeedPlanningUsageChanged.v1` is published only when an Active Plan begins or ceases to represent the accepted Need revision.
...
`ListEligibleBudgetLines` wraps the Budget contract `list_eligible_budget_lines` from BUD-CHG-001 v1.7 §9.1, supplying the Fiscal Year and the source Organisation Unit. A line is eligible when its owner scope is Entity-wide or matches that unit; owner scope is a Budget record-eligibility rule and never a user-permission check. The DPP entry stores the selected line ID and amount; funding source and currency display from Budget read services.
...
**Planning creates no reservation at any point.** Need acceptance, DPP save, DPP submission, DPP validation, Plan Item formation, Finance confirmation, adoption, approval and publication all leave Budget balances untouched. Reservation and commitment happen at Procurement Requisition through the Budget contracts in BUD-CHG-001 v1.7 §9.1, against the Plan Item lineage exposed in §7.4.
...
- DPP entry ID and Need/revision lineage where applicable;
...
|---|---|
| `ResolvePlanningContext`, `SelectPlanningContext`, `ResetPlanningContext` | Authorised PEs/OUs and matching assignment IDs from the shared AUTH resolver plus configured Financial Years available to the module; no implicit first record and no per-user FY assignment. |
| `GetPlanningWorkspace` | Reconciled action queue, waiting work, current Plan state and a portfolio schedule-health count — how many Active Plan Items are currently forecast behind their baseline on any not-yet-actual milestone — using one scope predicate. |
| `GetDepartmentalPlan` | Current DPP Submission, accepted Need coverage, direct entries, blockers and authorised commands. |
| `GetDppEntryEditor` | The exact current Draft entry with its eligible Procurement Budget Lines and, for a Need-origin entry, the six read-only source facts. |
...
| `GetAnnualPlan` — unallocated sources | Current accepted, unallocated entries for the exact Fiscal Year. The read model joins each accepted DPP entry through the immutable `DPPValidationDecision` for that submission/version to obtain its classification; it does not invent a classification field on `DPPEntry` or `PlanSourceAllocation`. |
| `GetAnnualPlan` — Version detail | Complete Version, all Plan Items, source allocations, Finance state, governance history, current commands and, for an Active Version, the same schedule-health count and a per-item behind-baseline flag. |
| `GetPlanItem` | Exact item, all source rows, Objective, method, the complete baseline/forecast/actual/variance schedule, its cascade revision history and Finance state. |
...
| `GetRequisitionEligiblePlanItem` | Current eligible or blocked result with lineage, balances and evaluation time. |

### 8.2 Commands

| Command | Core effect |
|---|---|
| `OpenDepartmentalPlan` | Idempotently create/reuse the one DPP root and current Draft after exact authority checks; after withdrawal with no accepted predecessor, create the next Draft Submission only while the initial window is Open. |
...
| `WithdrawDepartmentalSubmission` | Withdraw the current Draft or Returned Submission that has no downstream consumption; a prior accepted Submission remains current. |
| `CreateDepartmentalPlanUpdate` | Copy the accepted Submission into the one Draft successor. |
...
| `ConfirmSplittingAdvisory` | Record the Planner's confirmation of the non-blocking splitting advisory (§5.3 invariant 26) on the open Version. |
...
| `PublishAnnualPlan` *(system; runs inside `ApproveAnnualPlan`, not whitelisted)* | Transmit the exact approved payload and activate only on acknowledged response. Activation seeds every Plan Item's seven forecast fields equal to its baseline fields, in the same transaction. |
| `RetryPublication` | Re-transmit the exact approved payload after a failed or indeterminate attempt; a System Manager technical action, not a procurement decision. |
| `BeginPlanUpdate` | Create the sole Draft successor from the Active Version. Button label **Prepare plan update**. |
...
| `RecordRequisitionDrawdown` / `ReverseRequisitionDrawdown` *(inbound from REQ-CHG-001 §7.4)* | Record or reverse one Requisition's drawdown against an Active Plan Item's remaining quantity and value. |
| `ReceivePlanItemCorrectionRequest` / `ResolvePlanItemCorrectionRequest` *(inbound from REQ-CHG-001)* | Hold and resolve a Requisition-raised correction request on an Active Plan Item. |
| `PreviewForecastCascade` | Non-mutating. Given one Active Plan Item, one named milestone and its proposed new forecast date, computes the day-delta and returns a proposed date for every subsequent milestone with no actual yet, each independently includable or excludable. Milestones with an existing actual are never returned as proposable. |
| `ConfirmForecastCascade` | Mutating. Writes one `PlanItemForecastRevision` per included row, sharing one generated `cascade_id`, using the one supplied reason. Each included row is re-validated against the same governed floors and ceilings as baseline derivation before it is accepted. Touches no baseline field and no actual field; creates no Budget event. Atomic — every included row is written or none is. |
| `RecordTenderMilestoneActual` *(inbound, not user-facing, not whitelisted)* | Consumes a projection published by the module that owns a schedule event and writes the matching `actual_*_date`. No such publisher exists for any milestone in MVP-1; this contract is defined so the required TPR-CHG-001 correction in §18 has a fixed shape to implement against. |
...
### 8.3 Scheduled system action

`CheckApproachingMilestones` runs daily against every Active Plan Item. For each item's next milestone with no actual date yet, it compares the current date against the forecast date; where the gap has closed to a governed threshold, it raises one notification through the existing shared My Work/notifications surface addressed to the Procurement Planner. It creates no task, no state, no lifecycle event and no blocking condition — a missed nudge blocks nothing and a repeated run does not duplicate an unresolved notification.
...
| `PLN_TENDERING_PERIOD_BELOW_MINIMUM` | The tendering period must be at least 7 days. Applies to baseline derivation and to an individually overridden forecast row. |
| `PLN_EVALUATION_PERIOD_ABOVE_MAXIMUM` | The evaluation period cannot exceed 30 days. Applies to baseline derivation and to an individually overridden forecast row. |
| `PLN_STANDSTILL_BELOW_MINIMUM` | The standstill period before contract signing must be at least 14 days. Applies to baseline derivation and to an individually overridden forecast row. |
...
Unauthorised detail and task reads return the same not-found response as a nonexistent record. Internal diagnostics are logged with a support correlation and are not shown as user fields. The client renders this as PLN-DES-16's masked "Not available" state (§11.18), never the Load-error panel, and suppresses the platform's own generic not-found dialog so the actor sees the fact stated once, calmly, not as an apparent crash (reported live 2026-09-11).
...
| Finance task | `/app/procurement-planning/finance/{task_id}` | Assigned Finance Confirmation Officer |
...
- Small inline control directly beneath the description, not a bordered card: label **Financial Year** beside a select showing **FY 2027/28**, with the quiet trailing value **Annual Plan · Draft Version 1**. This is a plain filter control at normal text weight — it must not use the bordered-card, corner-marked treatment given to real content below it, and must not be depicted as permanently fixed.
- No header action button.

**Ready to consolidate** — one prominent card immediately below the header, styled like the plan summary strip used on PLN-DES-07/14, not like a generic table:

- Headline: **1 accepted departmental entry ready to consolidate**
- Supporting line: **Digital Health · KES 80,000,000**
- Right-aligned primary button: **Open Annual Plan**

When more than one item is ready — a second Plan awaiting Finance, or a DPP awaiting the Planner's validation — each appears as its own row in the same card, same headline-plus-button format, never as a generic multi-column table with column headers. This card is entirely absent, not shown empty, when nothing is currently actionable.

Every row's supporting line carries the facts the actor needs to judge scope and urgency, never the bare scope label (FU-15, 11 September 2026):

- Validate departmental plan: **{Department} · Submission {n} · {k} requirements · KES {value} · submitted {d MMM yyyy} by {submitter}**
- Continue departmental plan / Correct and resubmit: **{Department} · Submission {n} · {k} requirements · KES {value}**, plus **· returned {d MMM yyyy}** on a correction
- Confirm plan funding / Adopt / Approve: **{Plan title} · Version {n} · {k} items · KES {value} · requested {d MMM yyyy}**
- Ready to consolidate: as above.

**Amber notice**, shown directly beneath the card, in the same treatment as the readiness notice on PLN-DES-02: **2 accepted Needs are not included in any departmental plan** — **2 accepted Needs from Human Resources Management and Development were not included because the departmental-plan submission window closed before they were added. Ask the department to raise this with your KenTender administrator.**

**Departmental plans card**

Heading: **Departmental plans feeding this Annual Plan**. Quiet line beneath the heading: **These are the accepted and pending plans behind the entry above.**

| Department | Submission | Requirements | Value | Status | Action |
|---|---:|---:|---:|---|---|
...
Below the table: **2 departmental plans**. Do not show a late-submit action, summary cards, charts, waiting queues or system support links.
...
- Quiet reference: **DPP-MOH-DHI-2027-001 · Submission 1**
- Status badge: **Draft**
...
| Accepted Need | NDS-MOH-2027-0001 · Revision 1 |
...
| Every Plan Item records plan horizon, aggregation and lotting | Not started |
| Baseline schedule meets the governed periods and delivery boundary | Not started |
...
The first seven rows use the approved neutral state badge. The eighth row uses the approved advisory state treatment. The ninth uses the approved neutral badge.
...
| Departmental plan | DPP-MOH-DHI-2027-001 · Submission 1 |
| Accepted Need | NDS-MOH-2027-0001 · Revision 1 |
| Quantity | 1 programme |
| Required by | 31 Aug 2027 |
| Procurement Budget Line | MOH-BL-DHI-2027 — Digital health infrastructure programme |
...
Title and Procurement description are editable. Requirement type is read-only.

**Classification and method card**
...
Strategic Objective and Procurement method are select fields. Objective path and Value band are read-only. Procurement method defaults to the server's proposal for the resolved Value band and offers only methods admissible for it — not a free choice, but not a fixed display value either.

Quiet helper text beneath the **Planned value** row of the Source card: **Market price index: not published for this category.**

**Preference and structure card**

| Field label | Displayed value |
|---|---|
| Preference and reservation | None |
| Plan horizon | Single year |
| Aggregation | Not aggregated |
| Lotting | Single lot |

All four are select fields. When Plan horizon is Multi-year, show one additional required text area, **Multi-year justification**. When Lotting is Packaged into lots, show one additional required numeric field, **Lot count**.
...
Heading: **Baseline schedule**. Quiet line beneath the heading: **Computed from your target invitation date using the governed periods for this procurement's category and method. Locked once this Plan Version is submitted.**

Field, shown first and prominently: **Target invitation date** — 1 May 2027 — the one date entered directly.

Immediately below, the computed result, in the approved quiet-value style, updating live as the target date or any period changes:

| Milestone | Baseline date |
|---|---|
| Invitation or advertisement | 1 May 2027 |
| Bid opening | 22 May 2027 |
| Evaluation completion | 21 Jun 2027 |
| Tender award approval | 26 Jun 2027 |
| Notification of award | 28 Jun 2027 |
| Contract signing | 12 Jul 2027 |
| Delivery or implementation completion | 31 Aug 2027 · from the authorised Requisition |

Quiet helper text beneath the computed table: **Delivery completion is the department's own required-by date. The computed contract signing date must leave a reasonable delivery period before it.**

Below that, a closed-by-default disclosure: **Adjust periods** with a small chevron. Its collapsed state shows only the quiet summary line **Using governed defaults for Goods · Open Tender**. Expanded, it reveals:

| Field label | Displayed value | Component |
|---|---|---|
| Tendering period | 21 days · minimum 7 | Numeric input with floor validation |
| Evaluation period | 30 days · maximum 30 | Numeric input with ceiling validation |
| Award approval buffer | 5 days · governed default for this category and method, not a statutory figure | Numeric input |
| Notification buffer | 2 days · governed default for this category and method, not a statutory figure | Numeric input |
| Standstill period | 14 days · minimum 14 | Numeric input with floor validation |

Do not show the disclosure expanded by default. Most Plan Items never need it open.
...
Do not show a Finance action, a funding-confirmation button, a reservation, Value Commitment, contract period, recommended method, method basis, a forecast field, an actual-date field, attachment or source edit. Forecast and actual only exist once the Version is Active, on PLN-DES-14. Funding confirmation is requested once for the whole Plan Version from PLN-DES-07, never from a Plan Item.
...
|---|---|---|---:|---|---|---:|
| Clinical training laptops for digital health rollout | Human Resources Management and Development | Accepted Departmental Need | 100 each | 31 Dec 2027 | MOH-BL-HWD-2027 | KES 20,000,000 |
| Clinical deployment laptops for digital health rollout | Digital Health | Accepted Departmental Need | 150 each | 31 Dec 2027 | MOH-BL-HWD-2027 | KES 30,000,000 |

Both sources share one Procurement Budget Line, per the combine rule in §12.3: combined formation requires the same Fiscal Year, Procurement Budget Line, currency, requirement type, unit and treatment. A cross-department combine on two different lines is not a supported configuration and is never shown as one.

Below the table: **2 sources · 250 each · KES 50,000,000**.

Use the same Identity, Classification and method, and Preference and structure cards as PLN-DES-09, with these value differences and one addition:
...
| Preference and reservation | None |
| Plan horizon | Single year |
| Aggregation | Aggregated into this package |
| Lotting | Single lot |
| Aggregation reason *(shown only on a combined item, in the Identity card, below Requirement type)* | Procure one standard laptop specification and deployment service for the same national digital-health rollout. |

Every editability, select-versus-read-only and conditional-field rule stated on PLN-DES-09 applies unchanged. Use the same schedule card and footer layout as PLN-DES-09, with delivery completion **31 Dec 2027**. Do not show source detachment, partial allocation, different treatment per source or a generic reason field beyond the one aggregation reason above.
...
| Departments | 1 |
| Schedule health | 0 of 1 item behind baseline |
...
**Schedule card**, shown when a Plan Item row is expanded or opened. Heading: **Schedule — National digital health infrastructure upgrade**.

| Milestone | Baseline | Forecast | Actual | Variance vs baseline | Action |
|---|---|---|---|---|---|
| Invitation or advertisement | 1 May 2027 | 1 May 2027 | — | — | Shift schedule from here |
| Bid opening | 22 May 2027 | 22 May 2027 | — | — | Shift schedule from here |
| Evaluation completion | 21 Jun 2027 | 21 Jun 2027 | — | — | Shift schedule from here |
| Tender award approval | 26 Jun 2027 | 26 Jun 2027 | — | — | Shift schedule from here |
| Notification of award | 28 Jun 2027 | 28 Jun 2027 | — | — | Shift schedule from here |
| Contract signing | 12 Jul 2027 | 12 Jul 2027 | — | — | Shift schedule from here |
| Delivery or implementation completion | 31 Aug 2027 | 31 Aug 2027 | — | — | *(no action — nothing follows it)* |

Every Actual and Variance cell shows an em dash, because no Plan Item in this fixture has a downstream projection yet. Baseline column uses the approved locked-value style; Forecast uses the approved editable-value style even though its current value equals baseline; Actual uses the approved projection-only style, visually distinct from both. **Shift schedule from here** opens PLN-DES-14A for that milestone; there is no separate single-milestone edit control — a cascade proposal is always computed, and a Planner who wants only one date changed simply deselects every other proposed row inside that dialog.
...
Do not show monitoring entry, create Requisition, create Tender, an editable baseline field, an editable actual-date field, chart or generic evidence table. The only edit path anywhere on this page is **Shift schedule from here**, which opens the cascade-review dialog.

### 11.16A PLN-DES-14A — Shift schedule from here dialog

640 px modal over a dimmed PLN-DES-14 — wider than the standard 520 px, because it shows a small table, not a single field.

- Title: **Shift schedule from here — Bid opening**
- Read-only context line: **Changing Bid opening recalculates every later milestone by the same number of days. Deselect any row you do not want to change.**

| Field label | Displayed value | Component |
|---|---|---|
| New forecast date for Bid opening | 5 Jun 2027 | Date input |

Beneath it, a proposed-shift table, every row pre-selected:

| Include | Milestone | Current forecast | Proposed forecast |
|---|---|---|---|
...
| ☑ | Delivery or implementation completion | 31 Aug 2027 | 14 Sep 2027 |

Every row's checkbox is independently uncheckable. Deselecting a row excludes it from this confirmation and leaves its forecast unchanged. A milestone with an existing actual date never appears in this table at all — not shown unchecked, simply absent.

Below the table, one shared field:

| Field label | Displayed value | Component |
|---|---|---|
| Reason | Tender Preparation confirmed the issue date will slip two weeks pending template release. | Multiline text area, 20–500 characters, applied to every included row |

Footer buttons: **Cancel** and primary **Confirm shift**.

Do not show a baseline field, an actual-date field, a per-row individual reason field, or any control that could alter a locked baseline value on this dialog. Do not show an included row's Proposed forecast as independently editable — a Planner who needs a different value for one milestone deselects it here and revises that one separately.
...
|---|---|---|---|
| Forbidden | You do not have access to Procurement Planning | This area needs one of these responsibilities: Procurement Planner, Finance Confirmation Officer, Accounting Officer, the entity's statutory approver, Head of User Department, Departmental Author or Auditor. Ask your KenTender administrator to assign one in System setup. | None |
| No departmental plan | No departmental plan yet for FY {label} | Rendered as an action row in the workspace card (§12.1), supporting line the department name; never a state card. | Start departmental plan |
...
| Publication failed | Publication was not acknowledged | The approved Plan is unchanged. Retry the same publication when the destination is available. | Retry exact approved payload |
...
| Not available (masked) | This record isn't available to you | It may not exist, or you may not have access to it. | Go to Procurement Planning |

Only the load-error component may display a generated support reference. Do not add diagnostic text, illustrations or alternative actions. The masked "Not available" state is distinct from Load error and never carries one: it is the expected, permanent shape of §9's "unauthorised reads return the same not-found response as a nonexistent record," not a technical failure, so it offers no "Try again" — only a way back to the workspace.
...
- A departmental user sees their DPP work, a Procurement Planner sees submitted DPP tasks, a Planner sees accepted sources and Plan work, a Finance Confirmation Officer sees Finance tasks, and each governance actor sees only their exact task.
...
- Departmental action rows name the outcome, never the verb "Open" for both creating and navigating: **No departmental plan yet for FY …** with the button **Start departmental plan** (runs the §5.1 `OpenDepartmentalPlan` command); **Continue departmental plan** with **Continue**; **Correct and resubmit departmental plan** with **Correct**. In the departmental-plans list a department whose accepted Submission has an open successor reads **Accepted · update in progress**, never as a plan that missed the window.
- The Annual Plan record is always one click away: the context-strip summary (**Annual Plan · {status} Version {n}**) routes to the record for every reader. On an Active Version with no successor, accepted departmental entries not yet allocated are offered to the Procurement Planner as an action row — **{n} accepted departmental entries not yet in the Active plan**, supporting line departments · value, button **Prepare plan update** (the same words as the PLN-DES-14 record button it lands on, which runs the §5.2 `BeginPlanUpdate` command) — never demoted to a waiting line the Planner cannot act on.
- Search and counts never disclose another PE, FY, OU, task or Plan.

### 12.2 PLN-UI-02 — Departmental Plan

- **Start departmental plan** calls the explicit guarded `OpenDepartmentalPlan` command; subsequent reads reuse the one root and Draft Submission.
...
- If the initial Submission was withdrawn without an accepted predecessor, **Start departmental plan** creates the next numbered Draft only while the original submission window is Open. It never revives or edits the withdrawn Submission.
...
- The record route never strands the actor who holds the open task (FU-14): a Procurement Planner who may decide the open validation task sees **Review submission** in the header beside the status badge, routing to PLN-UI-06. Every other reader keeps the neutral read.

### 12.3 PLN-UI-03 — Accepted Need funding details

- Load the exact accepted Need revision fixed by the DPP entry and display all six source facts read-only.
...
- If the Need revision is no longer current, save is blocked and the DPP refresh path is shown.
...
- The Annual Plan record never strands the actor who holds the open task (FU-14): the Finance Confirmation Officer sees **Open Finance task**, the Accounting Officer or statutory approver **Open decision**, in the header beside the status badge, each only while they may decide that task.
...
- The method select offers the eleven Third Schedule methods; the server proposes Open Tender as default and admits an alternative only where the resolved threshold band permits it. This is a real select, not a read-only single value — the earlier MVP-method-is-fixed description is superseded by the threshold-admissibility model in §4.9.
- Save accepts title, description, Strategic Objective, aggregation reason when combined, plan horizon, multi-year justification when applicable, aggregation indicator, lotting indicator, lot count when applicable, reservation category, procurement method, and the baseline-schedule inputs — target invitation date, tendering period, evaluation period, the two labelled planning-assumption buffers, and standstill period. No baseline milestone date is itself a save field; all seven are server-derived from these inputs.
- Requirement type, procurement category, quantity, unit, source details, Procurement Budget Lines and planned value are derived and read-only.
- The computed baseline table recalculates immediately as the target invitation date or any period changes, before any save — never a submit-and-reload step.
- The five period inputs default from Configuration & Governance's category/method-keyed reference where an entry exists, or the flat governed default otherwise, and load inside a disclosure that is closed by default. Most Plan Items never need to open it.
- Baseline validation binds each failure to the exact input control: a period below its governed floor or above its governed ceiling, or a computed schedule that fails the delivery-boundary check, is rejected with the field highlighted.
- **This screen has no Finance action.** Funding confirmation is a Plan-Version-level action requested once from PLN-DES-07, never from a Plan Item — per the lifecycle in §5.2. Saving or editing a Plan Item here creates no Finance task and touches no Budget reservation.
...
- **Shift schedule from here** is available on every milestone with no actual date yet, except the final milestone, which has nothing after it to cascade. Opening it always computes and shows a proposed shift for every later not-yet-actual milestone, every row pre-included; the Planner may exclude any row before confirming. One reason, 20–500 characters, applies to every included row. Confirming writes one `PlanItemForecastRevision` per included row sharing one `cascade_id`. The baseline fields never change.
- A milestone that already has an actual date never appears as an included or excludable row in any cascade proposal — what has happened is not reforecast.
- **Actual dates display only from an authoritative downstream projection.** No control on this screen, or anywhere in Planning, accepts a typed actual date. Where no projection has arrived yet — which for MVP-1 is every milestone on every Plan Item — the actual and variance cells render an em dash, not a zero and not a blank input.
- The schedule table shows all three tiers side by side per milestone: baseline (locked), forecast (cascade-revisable with reason), actual (projection-only, currently always empty). The summary strip shows the plan's own schedule-health count, derived from the same per-milestone comparison.
- The daily `CheckApproachingMilestones` job never mutates this page's data; it only raises a notification elsewhere. Nothing on this screen depends on it having run.
...
The site Procuring Entity is configured once by CFG-CHG-002 v0.9 and is never a seed input here. Seeds fail when an authoritative prerequisite is absent or differs. They do not invent a Fiscal Year, Organisation Unit, unit of measure, Procurement Budget Line, Objective or assignment.
...
Planning seed data references these exact owned records, which are created by STR-CHG-001 v1.7 §14.3 and BUD-CHG-001 v1.7 §15.3. It does not create a substitute Objective, Procurement Budget, Procurement Budget Line, funding source or currency.
...
| Field | Exact value |
|---|---|
| Need | `NDS-MOH-2027-0001` · Revision `NDS-MOH-2027-0001-V1` |
| Title | National digital health infrastructure upgrade |
| Description | Procure and implement national digital health infrastructure across priority health facilities. |
| Expected operational result | Priority health facilities can use secure and interoperable digital health services. |
| Quantity | 1 programme |
| Required by | 31 Aug 2027 |
| DPP | `DPP-MOH-DHI-2027-001` · Digital Health · Submission 1 |
...
**Second departmental source pair.** NDS-MOH-2027-0003 and NDS-MOH-2027-0004, both accepted 25 Nov 2026 per NDS-CHG-001 §14.3, feed the combined Plan Item in §14.5:

| Field | Exact value |
|---|---|
| Need 1 | `NDS-MOH-2027-0003` · Revision 2 · Human Resources Management and Development · Clinical training laptops for digital health rollout · 100 each |
| DPP 1 | `DPP-MOH-HRMD-2027-001` · Human Resources Management and Development · Submission 1 — the department's first DPP |
| DPP entry 1 | `DPPE-MOH-HRMD-2027-001` · Accepted Departmental Need |
| Need 2 | `NDS-MOH-2027-0004` · Revision 1 · Digital Health · Clinical deployment laptops for digital health rollout · 150 each |
| DPP 2 | `DPP-MOH-DHI-2027-001` · Submission 1 — the same DPP root as the first source pair, carrying a second entry |
| DPP entry 2 | `DPPE-MOH-DHI-2027-002` · Accepted Departmental Need |
| DPP submission | Both entries are certified in the same Head of User Department submissions already dated in §14.4 above; DHI's submission at 25 Nov 2026, 10:00 EAT now certifies two entries, not one |
| DPP validation | Both entries are accepted in the same Procurement Planner validations already dated above |
...
**Second Plan Item, combined from two departments.** This is the Plan Item SEED-001 uses to exercise the IT-EQUIPMENT-OPEN-V1 template end to end, and it is a genuine part of the live Active Version, not an isolated profile.

| Field | Exact value |
|---|---|
| Plan Item | `PPI-MOH-2027-033` — Clinical training and deployment laptops for digital health rollout |
| Description | Procure and deploy one common laptop specification for clinical training and field digital-health deployment across two departments. |
| Source allocation 1 | `PSA-MOH-2027-033-001` · full `DPPE-MOH-HRMD-2027-001` allocation · 100 each |
| Source allocation 2 | `PSA-MOH-2027-033-002` · full `DPPE-MOH-DHI-2027-002` allocation · 150 each |
| Aggregation reason | Both departments require the same laptop specification for the same national digital-health rollout; combining secures better unit pricing and one delivery schedule. |
| Requirement type / category | Goods |
| Strategic Objective | `OBJ-MOH-2023-001` |
| Procurement Budget Line | `MOH-BL-HWD-2027` — the one line both allocations share, per the combine rule |
| Procurement method | Open Tender — the combined value of KES 50,000,000 is above the KES 3,000,000 request-for-quotations ceiling for goods |
...
Exact planned dates, using the same governed periods as PPI-MOH-2027-021:
...
| Affordability statement at decision | MOH-BL-DHI-2027 approved KES 100,000,000 · planned KES 80,000,000 · within approved. MOH-BL-HWD-2027 approved KES 60,000,000 · planned KES 50,000,000 · within approved. |
...
| Budget position after the plan cycle | MOH-BL-DHI-2027 · approved KES 100,000,000 · reserved KES 0 · available KES 100,000,000, unchanged. MOH-BL-HWD-2027 · approved KES 60,000,000 · reserved KES 0 · available KES 60,000,000, unchanged — planning creates no reservation regardless of how many Plan Items a line carries. |

At the observation time of 10 Dec 2026, 15:05 EAT, PPI-MOH-2027-021 has remaining eligibility of 1 programme and KES 80,000,000 with no Requisition drawdown, and PPI-MOH-2027-033 has remaining eligibility of 250 each and KES 50,000,000 with no Requisition drawdown.
...
### 14.8 Combined Plan Item — now part of the integrated baseline

**Retired as an isolated fixture.** The combined "Clinical training and deployment laptops" item was originally kept isolated because, at 500 each and KES 120,000,000 split across two different Procurement Budget Lines, it violated the combine rule (one shared line) and exceeded both candidate lines' headroom. That was a genuine defect, not a fixture-scale limitation. It is corrected in §14.4 and §14.5 above — one shared line (`MOH-BL-HWD-2027`), reduced quantities (100 and 150 each), and it is now `PPI-MOH-2027-033`, live in the default integrated Active Version. PLN-DES-09A reflects the corrected numbers.

### 14.9 Retired — see SEED-001

The bare `PPI-KEBS-2026-ICT-001` profile, keyed to Kenya Bureau of Standards with no Need, department or Fiscal Year grounding, is removed. It duplicated and conflicted with the properly-grounded `PPI-MOH-2027-033` fixture now integrated above. SEED-001 is the single cross-module reference for the complete Need-through-Tender chain this Plan Item feeds; REQ-CHG-001 and TPR-CHG-001 are tracked there as still requiring a matching correction from their own `PE-KEBS` fixtures to this one.
...
| PLN-AC-011 | A DPP return preserves the certified Submission and provides actionable entry-level correction. |
...
| PLN-AC-019 | Baseline dates are computed from the target invitation date and the governed periods, never independently typed; the computed schedule is chronological and the delivery boundary check passes before submission. |
...
| PLN-AC-046 | The Need-origin combined item (`PPI-MOH-2027-033`) and the direct-requirement fixture (§14.7) each produce equivalent, complete approved source lineage regardless of origin. |
...
| PLN-AC-053 | A withdrawn initial DPP can reopen only as the next Submission while the initial window is Open. |
...
| PLN-AC-111 | Every page resolves its authorisation verdict before rendering; a denied actor sees the inline Forbidden panel with no header, filter, content or empty state painted, and no permission modal appears on page load. |
| PLN-AC-112 | The Forbidden panel names the responsibilities that open the surface and directs the user to a KenTender administrator; it names no line manager or supervisor. |
| PLN-AC-113 | Selecting this module without access pushes its own route, highlights it in navigation, and lands on its Forbidden state; the module is never hidden and route and view never diverge. |
| PLN-AC-114 | `tendering_period_days` cannot be saved below 7; `evaluation_period_days` cannot be saved above 30; `standstill_period_days` cannot be saved below 14 — enforced server-side, not only in the client. |
| PLN-AC-115 | The seven baseline dates are derived, matching the anchor plus the governed and assumption periods exactly, and are never independently editable fields. |
| PLN-AC-116 | Submitting a Plan Version blocked by `PLN_DELIVERY_BOUNDARY_INSUFFICIENT` is rejected, naming the affected Plan Item. |
| PLN-AC-117 | A locked baseline field cannot be changed by any command once the owning Version has left Draft; only a Plan successor can produce a new baseline. |
| PLN-AC-118 | A forecast revision requires a reason, writes one `PlanItemForecastRevision` row, changes only the named forecast field, and leaves the baseline and actual fields for that milestone untouched. |
| PLN-AC-119 | No command, role or API path accepts a manually supplied `actual_*_date`, tested against every role including Administrator and System Manager. |
| PLN-AC-120 | Every Plan Item's seven actual dates and seven variance-vs-baseline figures are null in the default MVP-1 seed, and the UI renders them as an em dash rather than a zero or blank input. |
| PLN-AC-121 | `plan_horizon`, `aggregation_indicator` and `lotting_indicator` are visible, editable controls on the Plan Item editor, and `multi_year_justification` and `lot_count` appear only when their governing selection requires them. |
| PLN-AC-122 | The readiness card blocks funding confirmation when plan horizon, aggregation or lotting is unrecorded, or when the baseline schedule fails its governed-period or delivery-boundary checks. |
| PLN-AC-123 | Forecast fields are null on every Draft, Submitted and Superseded Version. Immediately after activation, and before any revision, every forecast field equals its baseline counterpart for every Plan Item. |
| PLN-AC-124 | Revising any one forecast milestone always returns a proposed shift for every later not-yet-actual milestone, computed as the same day-delta; the proposal is never empty when a later not-yet-actual milestone exists, and is never silently skipped. |
| PLN-AC-125 | A milestone with a recorded actual date is never returned as an includable or excludable row in a cascade proposal, and a direct attempt to include one is rejected with `PLN_CASCADE_INCLUDES_ACTUAL_MILESTONE`. |
| PLN-AC-126 | Confirming a cascade writes one `PlanItemForecastRevision` per included row, all sharing one `cascade_id`, atomically — a failure on any included row leaves every row in the batch unwritten. |
| PLN-AC-127 | Excluding every row except the one directly revised produces a standalone revision with a null `cascade_id`, indistinguishable in effect from a single-milestone change. |
| PLN-AC-128 | An included row that would breach a governed floor or ceiling against its immediate predecessor is rejected with the same error code baseline derivation uses for that gap. |
| PLN-AC-129 | The workspace and Active Plan schedule-health counts match a direct query of Plan Items currently forecast behind baseline on any not-yet-actual milestone, and the workspace count is absent, not zero, when no Active plan exists in scope. |
| PLN-AC-130 | `CheckApproachingMilestones` raises one notification per approaching milestone per day at most, creates no task or state, and a missed or repeated run blocks nothing. |
| PLN-AC-131 | The award-approval and notification buffer defaults resolve from Configuration & Governance's category/method-keyed register where an entry exists, and fall back to 5 and 2 days respectively only where it does not. |
| PLN-AC-132 | The baseline schedule card shows the computed result before the period inputs, and the period-adjustment disclosure loads closed by default. |
| PLN-AC-133 | The computed baseline table updates immediately on a target-date or period change, before any save command is issued. |
...
### 16.4 Required AUTH-ADR-001 v1.7 correction slice

Implement the authorization correction as one controlled cross-module slice:

1. Replace every Planning use of Frappe User Permission, User Scope Assignment or module-local scope logic with the shared AUTH-ADR-001 v1.7 resolver registered through both Frappe permission hooks, plus the exact role-bound assignment ID.
...
- add Value Commitment, source reference, generic evidence, optional note, attachment or contract period fields;
...
- Do not let any actor type an actual date anywhere in this module. Actual dates arrive only through the projection contract in §18.
- Do not let a forecast change save without a reason, and do not let a forecast revision alter the corresponding baseline field.
- Do not alter a locked baseline field outside a Plan successor.
- Do not invent a statutory duration for the evaluation-to-award or award-to-notification gaps; both are labelled planning assumptions because neither is fixed by the Act or Regulations.
- Do not let `tendering_period_days` go below 7, `evaluation_period_days` go above 30, or `standstill_period_days` go below 14.
- Do not require a separate manual edit per downstream milestone to shift a slipped schedule; a single revision always proposes the full remaining cascade.
- Do not allow a cascade proposal to include, or an API call to force in, a milestone that already has an actual date.
- Do not let a cascade confirmation write a partial batch; every included row commits together or none does.
- Do not let an excluded row silently skip the governed floor/ceiling check that an included later row still owes against its new predecessor.
- Do not let `CheckApproachingMilestones` create a task, a blocking state or a duplicate unresolved notification.
- Do not hardcode one flat buffer default for every procurement category and method once the Configuration & Governance register holds a scaled default.
- Do not show the period-adjustment disclosure expanded by default, and do not require a save-then-reload to see the computed baseline table change.

## 18. Traceability and precedence

This document incorporates the approved boundary decisions from:

**Required correction in another document.** CFG-CHG-002 v0.9 has no owner for regulator reference data. It shall gain an **effective-dated** register covering the PPRA threshold matrix, reservation categories and their target percentage, and the quarterly market price index, surfaced in System setup and read-only to every module. This is a different kind of record from the fiscal years and units CFG governs today — it is regulator-sourced, changes on gazette, and must retain superseded versions so historical plans stay auditable. CFG-CHG-002 also still owes an owner for the funding-source catalogue required by BUD-CHG-001 v1.7.

- KT-STD-001 v1.4 — document structure, design closed-input rules, artboard shell, shared fixture register, common page behaviour, verification protocol, release evidence, seed conventions, universal prohibitions and error conventions;
- AUTH-ADR-001 v1.7 — sole role-bound authority, the site-local Organisation Unit tree, registered permission hooks, no per-user Fiscal Year and non-authoritative UI filters;
- CFG-CHG-002 v0.9 — the one site Procuring Entity, the ERPNext Fiscal Year surface, Organisation Unit records and the ERPNext `UOM` catalogue;
- STR-CHG-001 v1.7 — exactly one Active Strategic Objective on each Plan Item, site-wide Strategy scope and no Value Commitment;
- BUD-CHG-001 v1.7 — Planning-owned Finance task, the Finance Confirmation Officer role, Budget-owned live position and reservations, and the `Procurement Budget Line` record name; and
- NDS-CHG-001 v1.10 — optional consultation, direct departmental planning path, six Need values, role-bound departmental scope and Planning-owned DPP funding specification; and
- REQ-CHG-001 v1.7 — partial Plan Item drawdown, one-open-Requisition control, reversal and remaining balances.

**Partially resolved, and further along than this document last said.** TPR-CHG-001 v0.7 §9.5 publishes `actual_invitation_date` to `RecordTenderMilestoneActual`, triggered once `AcknowledgeTenderPublicationConsumed` is called. This section previously said the caller of that trigger was "itself not yet designed" — it now exists: TPUB-CHG-001 v0.3, Tender Publication, is that module, and its `AuthorisePublication` command is the caller. `actual_bid_opening_date` and the five milestones after it remain null in production: bid opening belongs to a future Evaluation & Award module, and the remaining milestones belong to that module and a future Contract Management module, neither of which exists yet. This is named here as a tracked dependency rather than left to be rediscovered.

**A second required correction.** CFG-CHG-002 v0.9's effective-dated reference register holds the threshold matrix, reservation categories and targets, and the market price index, but not yet a category/method-keyed set of default schedule buffers. `award_approval_buffer_days` and `notification_buffer_days` fall back to flat defaults (5 and 2) until CFG-CHG-002 adds this data. This is the second tracked dependency this document places on that register, after the funding-source catalogue already owed to BUD-CHG-001.
...
| Stable lineage | Every Plan Item preserves its exact DPP entry and Need-revision source where applicable. |
...
PLN-CHG-001 v1.16 is proposed for product-owner approval. On approval it supersedes v1.15, v1.14, v1.13, v1.12 and all earlier versions in full and becomes the only Procurement Planning requirements document to consult. v1.15 was approved on 11 September 2026; this correction supersedes that approval because v1.15 omitted five approved clauses the live build implements (§1.1). It further authorises the departmental-plan counter's display word **Submission** throughout §4, §5, §8, §9, §11, §12, §14 and §15 with the `DPPVersion` model unchanged, `submission_number` equal to the certified Submission's number, and the §5, §8, §11.18 and §12 label and contract-name corrections. It further authorises the Need-revision rename in §4.14 and every fixture citing a Need's revision; the correction in §4.14: `RequisitionEligibilityProjection` now explicitly enumerates every field REQ-CHG-001 depends on; and the correction in §1.1: `PPI-MOH-2027-033` integrated into the live Active Version on one shared Procurement Budget Line, and the retired `PPI-KEBS-2026-ICT-001` fixture, per SEED-001 v1.0.

Approval authorises implementation of the complete clean Procurement Planning module and conversion of section 11 into Claude Design artboards. It further authorises: replacement of `pe_fy_context_id` with the ERPNext `fiscal_year` on the submission window, departmental plan and Annual Plan; rekeying uniqueness to Fiscal Year; registration of Procurement Planner, Accounting Officer, the statutory approver and Auditor as site-wide business roles resolved through the AUTH-ADR-001 v1.7 permission hooks; renaming the Finance confirmation actor to Finance Confirmation Officer per BUD-CHG-001 v1.7; removal of every Procuring Entity selector, PE-scoped role and Fiscal Year user grant; adoption of the `Procurement Budget Line` record name and ERPNext `UOM` units; and adoption of the KT-STD-001 v1.4 standards and shared fixture register.
...
Implementers shall not retain v1.4's PE/FY context field, its Procuring Entity selector, its PE-scoped role assignments or its `Budget Officer` Planning task; shall not ship a Plan Item without a recorded reservation category or an unvalidated procurement method; and shall not create a per-item Finance task, a Planning-held reservation or an unconditional statutory approval stage. It further authorises the governed baseline/forecast/actual schedule model in §4.9: the single Planner-set anchor and five governed or labelled-assumption periods that derive the seven baseline dates; locking of baseline at Version submission; the forecast layer and its audited revision log; and the strict prohibition on any manually entered actual date anywhere in this module. It closes the contradictions previously standing in §3, §12.12 and §17, and the artboard gap for `plan_horizon`, `aggregation_indicator` and `lotting_indicator`.

It further authorises the cascading-reforecast and digitisation additions in this version: mandatory cascade proposal on every forecast revision, with per-row inclusion but no per-row inline retyping; the `cascade_id` audit grouping; the portfolio and per-plan schedule-health indicators; the daily `CheckApproachingMilestones` notification, which creates no task or state; category/method-scaled buffer defaults sourced from Configuration & Governance, with the named flat fallback while that register lacks the data; and the reordered, live-recalculating schedule card with its closed-by-default period disclosure.

It further authorises the UX consistency pass in this version: the rebuilt PLN-DES-01 workspace, with its plain inline Financial Year filter, single headline-plus-button action card and the promoted amber notice for excluded Needs; the corrected PLN-DES-11 table header; and the three-card split on PLN-DES-09, inherited by reference on PLN-DES-09A along with the reservation, plan-horizon, aggregation and lotting fields that combined item was previously missing.

It does not approve generated visual deviations, production publication configuration, a Procurement Requisition, a Tender or any field not defined here.

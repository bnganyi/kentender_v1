...
| Status | **Proposed simplicity and directness revision — awaiting Project Owner approval** |
| Supersedes | On approval, v1.21 and all earlier Planning implementation specifications in full |
| Decision authority | Approved v1.20 baseline, the classification-correction requirements first drafted in v1.21, and the Project Owner-directed end-to-end simplicity review of 17 September 2026 |
...
| Change scope | Full retained business specification plus classification correction and an end-to-end simplification of ordinary Planning work; 114-row re-implementation table |
...
This is the proposed complete successor to v1.21. It retains the approved business, authority, legal, audit and classification-correction rules while removing user-facing complexity that does not help an actor complete the current task. The ordinary interface now follows three visibility levels: current decision, conditional supporting evidence and audit/configuration evidence. PLN-REF-001 and PLN-UX-001 remain provenance records; implementers need not combine their operative wording with earlier Planning versions or the usability blueprint. Citations bearing an explicit document ID refer to that historical or dependency document, not to this document’s section numbers.
...
An Active Version remains the governing Planning baseline until an acknowledged successor replaces it. A correction never reopens submitted content. A forecast revision never changes approved baseline content. Requisitions owns its authorisation and invokes Planning drawdown and Budget reservation commands. Budget owns reservation records, balances and transactional financial controls. Tender Preparation owns template binding and tender-document preparation; Tender Publication owns the tender's publication action. No Planning command creates a Requisition, Tender shell, STD binding, wizard configuration or Tender workflow.
...
The full decision-by-decision table is §17.4: 72 refinement rows, 20 usability rows, 6 explicit-composition rows, 3 classification-correction rows and 13 simplicity rows. Verification columns describe required work, not completed tests.
...
This facility is not implemented in the v1.22 MVP. The rules below are retained for a separately approved future implementation after authoritative milestone ownership and actual/expected-date integrations exist. The MVP creates no schedule-driven Planning reminders.
...
| Administrator / System Manager — Maintain setup | Existing System setup tabs; focused tasks for entity/approval route, responsibilities, intake, funding sources, rules, schedule profiles and reminders. **Save changes**; affected-action explanation before saving. | Direct maintenance under CFG/AUTH permissions; no setup approval workflow. Source verification requires evidence, not a green-status toggle. Historical rule versions remain immutable. |
...
| ConfirmForecastCascade | Future facility only; no MVP control | Retained domain identity for later owner-integrated forecasting; not exposed or accepted in this release |
...
| C01–C04 | First-run/configured entity; intake changes; funding catalogues; versioned rules, source verification, profiles and reminders; missing/unsupported configuration |
...
**U14-ACTUALS.** Under a purchase place table Milestone; Approved date; Forecast used; Actual date; Days after approved date; Difference from forecast. Use only owner-supplied actuals. Missing supported actual displays **No date recorded**; unsupported displays **Tracking is not yet available**; not-applicable displays **Not applicable**. Beneath, separate Stage durations table From; To; Planned elapsed days; Actual elapsed days; Difference, only when both actual endpoints exist.
...
| Update expected dates / Save forecast changes | No MVP control. | Deferred under §15.3; retained command identity must not be exposed until owner integration and separate approval exist. |
...
| Forecast correction, when future facility is approved | Exact baseline, owner-integrated forecast revisions and shared reason; never rewritten actuals |
...
Forecast-editing and reminder requirements **PLN18-AC-124, PLN18-AC-125, PLN18-AC-128–130, PLN18-UX-20–22, PLN19-UX-035 and PLN18-RI-030/039/040** are retained as future-facility requirements only. They are explicitly excluded from the v1.22 MVP release gate and must not create U15 controls, reminder work or placeholder tracking. All other listed criteria remain MVP requirements unless their row expressly states a future dependency.
...
| PLN18-AC-124 | **Future facility — excluded from v1.22 MVP.** Forecasts initialize from baseline exactly once on activation; absent before activation; supersession preserves last forecasts and revision history rather than nulling them. |
| PLN18-AC-125 | **Future facility — excluded from v1.22 MVP.** Changing a forecast proposes all eligible later milestones by the same delta; excluded rows and explicit individual overrides remain visible and the resulting full schedule is validated. |
...
| PLN18-AC-128 | **Future facility — excluded from v1.22 MVP.** A final-milestone or other single-row forecast change is valid with a reason and null cascade ID; a multirow accepted set shares one cascade ID. |
| PLN18-AC-129 | **Future facility — excluded from v1.22 MVP.** Forecast confirmation validates every affected adjacency, including included/excluded boundaries and individual overrides, under the applicable profile; late completion forecasts remain recordable and flagged. |
| PLN18-AC-130 | **Future facility — excluded from v1.22 MVP.** Workspace/Active schedule-health counts derive from exact applicable item/proceeding forecast evidence; no Active plan means absent count, and unlike quantities or proceeding dates are not collapsed. |
...
| PLN18-UX-20 | **Future facility — excluded from v1.22 MVP.** Baseline lateness, forecast error and duration variance have correct signs/bases; missing versus inapplicable distinct; historical comparisons reproducible |
| PLN18-UX-21 | **Future facility — excluded from v1.22 MVP.** Cascade supports individual overrides and final-row revision; every affected adjacency checked; commit atomic; late forecasts allowed; already-actual rows immutable |
| PLN18-UX-22 | **Future facility — excluded from v1.22 MVP.** Reminders deduplicate, reforecast without flooding, resolve only from actual evidence and navigate to the exact work; later milestones not starved by earlier missing actuals |
...
8. Two sequential Tender proceedings retain different owner-supplied actuals and exact coverage; corrected events never overwrite another proceeding. Missing values are not zero. Forecast comparison and reminder behavior is a future-facility journey excluded from the v1.22 MVP.
...
| PLN19-UX-035 | **Future facility — excluded from the v1.22 MVP.** When separately approved, forecast changes preview all eligible affected rows, individual overrides and approved-date comparison; actual rows remain protected and commit is atomic. |
...
| PLN19-UX-038 | CFG-owned setup exposes complete route/intake/catalogue/profile/verification/reminder tasks under existing authority, with no extra approval chain or legal-verification checkbox bypass; denied/masked/loading states are distinct. |
...
| 3 | Implement DPP coverage, disposition, classification provenance/correction, windows and accepted-source projection | Initial, returned, not-proceeding, type/category derivation, correction concurrency, affected-item recovery, withdrawn-source and late-addition tests |
...
| 8 | Implement the simplified task compositions and all MVP actor variants; reconcile seeds and acceptance criteria | Every screen state maps to the same lifecycle; one integrated fixture; unique criterion IDs; no contradictory required outcomes; U15 absent |
...
These are retained approved baseline requirements plus the v1.21 classification additions and v1.22 simplicity changes, not implementation claims. No item below is claimed complete merely because its design has been agreed.
...
| Planner-managed schedule forecasting and reminders | Approved schedule remains readable; no U15 action, cascade or forecast-driven reminder in MVP | Authoritative milestone owners, downstream actual/expected-date integration, governed cascade behavior, notification ownership and separate usability approval |
...
- No U15 forecast editor, cascade, reminder or unsupported completion placeholder in the MVP.
...
| CFG v0.10 | Approved successor covers four routes, county flag, five-tab maintenance, independent intake/deadline enforcement, funding sources, rules/verification, schedule profiles, reminders and native metadata adapters | Implement/verify owning services and scoped UI; preserve explicit applicability-date/fixture conflict; no permissive fallback or source verification by document approval |
...
The 114 rows below comprise 72 refinements, 20 usability changes, 6 v1.20 composition changes, 3 v1.21 classification changes and 13 v1.22 simplicity changes. None is marked implemented or tested. “Baseline/owner references” preserves the original reviewed-section references from PLN-REF-001; those numbers are provenance, not current navigation. “Current target” gives the current local sections and screen families. Stable acceptance IDs remain regression requirements; future forecast criteria are expressly outside the v1.22 MVP gate under §14. External owner changes remain subject to §15.
...
| PLN-RI-056 | Reconciliation | Infrastructure seed evaluation interval is 31 days despite 30-day maximum | Derive baseline from selected periods; reconcile seed and artboard dates | §§10.2, 13; U09/U11/U15 | PLN §14.5; DES-09/14 | **PLN18-RI-056** — Seed uses same schedule calculation and validation as product commands |
...
| PLN-UX-019 | Approved Blueprint v0.2 UX-19 — Readers, setup and access | Read-only and technical actors risk business controls or incomplete maintenance | Provide exact historical audit reading, complete CFG-owned setup/verification tasks and accessible denied/masked states; no business power from technical read access | §§6.5, 9.5, 10.5, 10.11–10.12, 11.6; U12/C01–C04/U21 | Blueprint v0.2 UX-19; PLN v1.18 corresponding U/C families | **PLN19-UX-037 / PLN19-UX-038** — Auditor/reader follows exact historical departmental and Finance evidence and exports only authorised content; current warnings never overwrite the past and decision controls are absent. CFG-owned setup exposes complete route/intake/catalogue/profile/verification/reminder tasks under existing authority, with no extra approval chain or legal-verification checkbox bypass; denied/masked/loading states are distinct. |
...
**PLN-CHG-001 v1.22 is proposed for Project Owner approval.** Until approval, PLN-CHG-001 v1.20 remains the approved Planning implementation specification; v1.21 remains an absorbed, unapproved proposal.

Approval of v1.22 will supersede v1.20, v1.21 and all earlier Planning implementation wording in full. It retains the approved business, authority, governance, integration, audit and classification-correction requirements while replacing the dense presentation contract with the simplicity rules in PLN22-CHG-001–013. Its full 114-row change table is §17.4. Existing stable acceptance IDs remain traceable; the 13 PLN22 criteria govern the revised UI and expressly defer the forecast facility from the MVP.

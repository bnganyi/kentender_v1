...
| Controlling decision | STD-ST-001 v0.5 - Approved |
...
| Tender Preparation consumer | TPR-CHG-001 v0.7 - Approved |
| Implementation authority | None |
| Change type | Corrects §8.1, found while resolving how the Ministry of Health fixture's two Requisition items should reach the Tender's goods and price schedules. §8.1's Goods and delivery schedule table listed Goods description, Quantity, Unit, Final destination, Latest delivery date and Minimum warranty as "Entered by: Officer" — the same unedited v1.0 language as the §8.4 defect v0.5 already fixed, contradicting §7.1's inherited-values dictionary, §7.3's generated-values table, and TPR-CHG-001's own Responsibility Boundary, all of which already treat these as inherited read-only. Rewritten so every one of those columns is System-inherited, and the row now states the grouping rule explicitly: rows sharing one technical requirement combine onto one schedule line, with every contributing `requisition_item_id` retained as lineage underneath it — not lost in the consolidation. No other section changes; the render specification (§11), evaluation/contract treatment (§9–10) and Ministry of Health fixture (§13) already had this right. Earlier: v0.5 corrected §8.4's contradictory mandatory-PDF paragraph, §7.1's missing reservation/lotting rows, §14's leftover "v0.3 document" label, Appendix A's stale v0.3 scope-lock, and the traceability citations to STD-ST-001 and TPR-CHG-001. Earlier still: v0.4 was the version 1.1 correction TPR-CHG-001 v0.4 §6.2 required — it replaced the PDF-primary technical specification with structured data inherited from the authorised Requisition; added the preference/reservation and lotting treatment STD-STD-001 §8–9 require; and renamed the golden fixture from Kenya Bureau of Standards to Ministry of Health, per SEED-001 v1.0. |
...
| Reservation category | Section III eligibility clause, per §6.1 |
| Lotting indicator | Compatibility confirmation only; this release renders and requires **Single lot**, per §6 |
...
**Correction in v0.6.** This table listed Goods description, Quantity, Unit, Final destination, Latest delivery date and Minimum warranty as "Entered by: Officer" — unedited v1.0 language from before the version 1.1 correction moved the technical specification, and everything alongside it, to inherited structured data. §7.1's inherited-values dictionary, §7.3's generated-values table, and TPR-CHG-001's own Responsibility Boundary already treat every one of these as inherited read-only; this table now agrees with them. The officer enters nothing here.

| Column | Entered by | Rule |
|---|---|---|
| Item number | System | Stable row order |
| Goods description | System | Inherited from the Requisition; not officer-editable |
| Quantity | System | Inherited. Where several inherited items share one technical requirement, they combine onto one schedule line — this release's Ministry of Health fixture combines two inherited items (100 + 150) into one 250-unit line, per §8.4(2)'s grouping rule |
| Unit | System | Inherited; governed unit list |
| Final destination | System | Inherited from the Requisition |
| Latest delivery date | System | Inherited from the Requisition |
| Minimum warranty | System | Inherited from the Requisition |
| Offered delivery date | Supplier | Supplier response; not completed by Procurement |

A combined schedule line still carries every contributing `requisition_item_id` as lineage underneath it. Grouping is a display consolidation; it never loses a stable inherited identifier, per TPR-AC-007/045.
...
**Correction in v0.5.** The paragraphs below, describing a mandatory "controlled document" with Approved status that readiness requires to be "present" and "a readable PDF," were unedited v1.0 language left in place when point 3 above retired the PDF-primary specification. As written, they directly contradicted point 3 and §4.1's own file-purpose table ("no controlled specification file, per §8.4"). They apply, and have only ever been meant to apply, to the **optional** supporting document point 3 describes — never to the technical specification itself, which is never a PDF in this release.

Where the originating department supplied a supporting document (drawings, a detailed technical brief), that governed project artifact has:
...
Readiness checks it only when the Requisition actually links one: in that case, the linked document must be readable, have Approved status, an approval date and digest, and contain no unresolved drafting instructions. When no supporting document is linked — the ordinary case for this pattern — readiness requires nothing here; it is not absent-but-required, it is simply not applicable. Product or model references must use an approved equivalent treatment or carry an approved justification. KenTender does not attempt to model every processor, port or warranty statement as a template parameter.
...
No production code or Frappe implementation is authorised by this document.
...
**Now that STD-TPL-001 v0.6 is approved:**

1. TPR-CHG-001 has already been revised (v0.6) to cite the approved template; no duplicated template specification remains in it;
2. a focused implementation pack encoding this template and the five-task journey is the next document in sequence;
3. implementation proceeds only after explicit authorisation separate from this approval — this document remains content authority only, per the header's Implementation authority row; and
4. the approved Ministry of Health walkthrough and smoke tests run before another pattern is added.
...
- **STD-ST-001 v0.5 - Approved**, for the original comparative evidence behind those principles;
...
- **REQ-CHG-001 v1.7 - Approved**, for the structured technical requirement, service and acceptance rows this template's Section V now renders from directly, per §8.4; and
- **TPR-CHG-001 v0.7 - Approved**, as the current Tender Preparation consumer, which this version 1.1 correction fulfils the condition of.
...
This document's approval makes it the content and curation-procedure authority for the first IT-equipment template. It does not authorise code, migration, deployment or production use.

## Appendix A - Instruction to give Claude

**Note added on approval (v0.5):** the instruction below was written to take curation from v0.3's PDF-primary, pre-reservation-rendering state to what is now this document's approved v0.5 content. That work is done — this document is approved. The instruction is corrected here (version references and authorised scope only) so that if it is ever reused, to resume genuinely incomplete curation work, it does not re-lock a curation agent to v0.3's superseded scope the way the original wording did. Check against §17's next actions before reusing it for anything else.

Use this instruction without expanding its scope:

> Resume section 14 of STD-TPL-001 v0.6 from Gate B, from the `frappe-bench/apps/kentender_v1` repository root. This is curation only. Do not implement or modify any Frappe runtime, DocType, hook, route, service, permission, patch or seed. Preserve unrelated work. First revise every Invitation-related coverage row to follow sections 5.1 and 11: the Invitation is a separate generated publication notice and must not be part of `complete_tender.html`. Show the revised coverage register and open issues, then stop again at Gate B for approval. After Gate B is approved, continue one pass at a time using the exact folders, filenames, CSV columns, HTML/Jinja format and review gates in v0.5. Do not add fields, variants, forms or policy choices that STD-TPL-001 v0.6 has not authorised.

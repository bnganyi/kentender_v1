...
| Controlling decision | STD-ST-001 v0.4 - Approved |
...
| Tender Preparation consumer | TPR-CHG-001 v0.6 - Approved |
| Implementation authority | None |
| Change type | Corrects four defects found on direct review, none of which change the template's approved domain content: (1) §8.4 carried an unedited v1.0 paragraph requiring a mandatory, Approved-status PDF technical specification, directly contradicting its own point 3 ("No PDF... is required") and §4.1's file-purpose table — rewritten so the readiness check applies only to the optional supporting document when the department actually supplies one; (2) §7.1's data dictionary omitted `reservation_category` and the lotting indicator despite §6/§6.1 already establishing both as inherited, rendered content — added; (3) §14 still labelled itself "this v0.3 document" and Appendix A instructed a curation agent to stay within "STD-TPL-001 v0.3"'s authorised scope — exactly the PDF-primary, no-reservation-rendering scope this version 1.1 correction exists to move past — both corrected to reference this document's current version and scope; (4) the traceability citations to STD-ST-001 and TPR-CHG-001 are updated to their current, approved versions. Domain scope, the data dictionary's actual content, the render specification and the Ministry of Health fixture are unchanged from v0.4. Earlier: v0.4 was **the version 1.1 correction TPR-CHG-001 v0.4 §6.2 required and that had not existed until then** — it replaced the PDF-primary technical specification with structured data inherited from the authorised Requisition; added the preference/reservation and lotting treatment STD-STD-001 §8–9 require; and renamed the golden fixture from Kenya Bureau of Standards to Ministry of Health, per SEED-001 v1.0. Domain scope — Open Tender, one award package, IT goods and closely related services — was unchanged from v0.3. |
...
**Now that STD-TPL-001 is approved:**
...
- **STD-ST-001 v0.4 - Approved**, for the original comparative evidence behind those principles;
...
- **REQ-CHG-001 v1.6 - Approved**, for the structured technical requirement, service and acceptance rows this template's Section V now renders from directly, per §8.4; and
- **TPR-CHG-001 v0.6 - Approved**, as the current Tender Preparation consumer, which this version 1.1 correction fulfils the condition of.

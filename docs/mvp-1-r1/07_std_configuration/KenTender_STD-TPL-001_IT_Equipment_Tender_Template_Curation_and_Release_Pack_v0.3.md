...
| Tender Preparation consumer | TPR-CHG-001, after this template is approved |
...
| Technical specifications | Approved Requisition document plus a small goods and delivery schedule |
...
| 7. Proof | Coverage register, KEBS-style fixture, complete preview, tests and approvals |
...
| `kebs_input.json` | Exact test data used to complete the working master |
| `kebs_technical_specification.pdf` | Approved fixture specification published as the controlled Section V document |
| `render_fixture.py` | Curation-only strict Jinja renderer for both outputs; it is never imported by KenTender |
| `kebs_invitation_expected.html` | Fully resolved separate Invitation notice fixture |
| `kebs_invitation_expected.pdf` | Human-review copy of the separate Invitation notice |
| `kebs_expected.html` | Fully resolved fixture with no Jinja or drafting instructions left |
| `kebs_expected.pdf` | Human-review copy rendered from `kebs_expected.html` |
...
The Invitation is not a section of the issued Tender document. It must not appear in `complete_tender.html`, its contents page or `kebs_expected.pdf`.
...
| One award package | Remove or resolve unused lot and multiple-contract instructions |
...
| Margin of preference or reservation | Procurement/legal reviewer must approve the exact treatment before release; unsupported variants make a Requisition incompatible |
...
### 8.4 Controlled technical specification document

The technical specification is not a casual attachment and it is not retyped into template fields. It is an approved, controlled part of Section V.

The first release uses this exact treatment:

1. Tender Preparation receives one approved technical-specification PDF from the authorised Requisition.
2. KenTender checks that the file and its approval metadata exist.
3. `complete_tender.html` renders a Section V cover sheet naming the specification, its version, approval date and publication filename.
4. The PDF is listed in `package_index.md` and published in the same Tender package.
5. The schedule rows, quantities, warranty and delivery values rendered in the main Tender must agree with the approved Requisition and specification.
6. Supplier technical compliance is collected against the published specification; the Procurement Officer does not recreate processor, memory, port or similar specification fields during Tender Preparation.
...
The approved technical specification remains a separate controlled PDF in Version 1.0. The main Tender must name it clearly, include it in the package index and use the same approved file digest. There is no alternate embed-or-attach choice in this release.
...
- the **publication notice**: `kebs_invitation_expected.pdf`; and
- the **issued Tender package**: `kebs_expected.pdf` plus `kebs_technical_specification.pdf`.
...
## 13. KEBS-style golden fixture

The release must render one deterministic test Tender. It is a KenTender product fixture, not a copy of the supplied KEBS Tender.
...
| Authorised quantity | 50 each |
| Authorised value | KES 18,000,000 |
...
| Business desktop computers with monitors | 15 each | KEBS Coast Region Office, Mombasa | 30 Dec 2026 | 36 months |
| Business tablets | 10 each | KEBS Coast Region Office, Mombasa | 30 Dec 2026 | 24 months |

Tender decisions:

- Issue date: **8 October 2026**;
- clarification deadline: **19 October 2026, 17:00 EAT**;
- submission deadline: **29 October 2026, 11:00 EAT**;
...
### Pass 5 - Build the KEBS fixture

1. Create `04_fixture/kebs_input.json` using exactly the values in section 13 and the approved insertion keys.
2. Store the approved fixture specification as `04_fixture/kebs_technical_specification.pdf`. Add its title, version, approval date, publication filename and SHA-256 to `kebs_input.json`.
3. Create `04_fixture/render_fixture.py` with exactly this content:
...
4. Run the strict renderer using the bench Python environment:
...
5. Render both resolved HTML files with `02_master/print.css` using the repository's installed `wkhtmltopdf` executable:
   ...
   wkhtmltopdf --enable-local-file-access \
     ...
     04_fixture/kebs_invitation_expected.html \
     04_fixture/kebs_invitation_expected.pdf

   wkhtmltopdf --enable-local-file-access \
     --user-style-sheet 02_master/print.css \
     04_fixture/kebs_expected.html \
     04_fixture/kebs_expected.pdf
   ```
...
6. Complete `package_index.md` with:
   ...
   - the controlled technical-specification PDF and digest as part of the issued Tender package; and
   - no unaccounted file.
7. Confirm that `kebs_expected.html` neither contains the Invitation notice nor lists it in its contents.
...
If `kebs_input.json` needs a value that section 7 or section 13 does not authorise, add the missing decision to `open_issues.md` and stop. Do not invent a fixture-only field.

**Gate E:** stop. A practising Procurement Officer walks through the five tasks using `kebs_input.json` and checks the separate Invitation notice, issued Tender and package index.
...
8. completed KEBS-style input fixture;
...
4. run the approved KEBS-style walkthrough and smoke tests before adding another pattern.
...
- **STD-ST-001 v0.3 - Approved**;
- **STD-ADR-002 v1.0**, subject to the approved stress-test corrections;
- **REQ-CHG-001 v1.0 - Approved**; and
- **TPR-CHG-001 v0.2 - Proposed**, as the current Tender Preparation consumer draft.

Earlier STD Engine and IT Wizard documents are source analysis only where they conflict with the approved simple product direction.

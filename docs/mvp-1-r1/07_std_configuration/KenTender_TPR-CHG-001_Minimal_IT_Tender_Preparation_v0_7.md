...
| Supersedes | v0.5 in full. v0.5 was itself never approved — this is a direct correction pass, not a revision of approved content. |
| First product | `IT-EQUIPMENT-OPEN-V1` — straightforward IT equipment under the PPRA Goods STD |
| Starts from | One authorised Requisition handoff, per REQ-CHG-001 v1.7 — approved, realignment complete |
...
| Implementation authority | Gated by a successful Ministry of Health walkthrough and completed template 1.1 release evidence, per STD-TPL-001 v0.6 |
| Change type | Removes this document's own copy of the anti-cramming rule, now centralised in KT-STD-001 §2.2 per that standard's own controlling decision; the concrete history (the "Package" row defect) stays, the restated rule doesn't. Also corrects a fact-cramming defect in §13.2's fixture, found by direct comparison with the same defect in TPUB-CHG-001's design section: the "Package" row joined six unrelated facts — item count, lineage split, schedule-line count, a section cross-reference, and two other-category counts — onto one line under one label, with no per-fact structure. Broken into a labelled sub-table. One rule added to §13.1 prohibiting the same pattern going forward, naming this exact defect as the reason. Corrects the inherited-item count in the Ministry of Health fixture, found while resolving a question about how the Tender's Schedule of Requirements should treat the fixture's two Requisition items. §16.1 and §13.2 described the inherited package as "1 equipment item, 250 Each" — wrong: REQ-CHG-001 §5.6 and its own §13.6 artboard are explicit that an item links to exactly one drawdown line, so this fixture's two source allocations (HRMD 100, DHI 150) produce **two** inherited items, not one. This does not change what a bidder sees — STD-TPL-001 §8.4(2)'s existing grouping rule already renders same-specification rows as one Schedule of Requirements line, and that rendered behaviour is correct and unchanged. What was wrong was describing the *inherited* layer as if it had only one item to begin with. Fixed throughout §13.2, §13.3, §13.5 and §16.1, with both source `requisition_item_id`s now carried as explicit lineage on the one grouped schedule line. Adds TPR-AC-045 and TPR-SMOKE-21 to make the two-inherited/one-rendered distinction verifiable rather than assumed. No task, control, role, lifecycle state, error code, render rule or domain scope changes — this is a fixture-accuracy correction only. Earlier: v0.6 synchronised this document to REQ-CHG-001 v1.6's actual, approved state: (1) the compatibility test now names the real handoff, `AuthorisedRequisitionHandoff v1.3`, not the assumed and superseded v1.2; (2) v1.3's two added fields, Strategic Objective (with ancestor path) and plan horizon (with Multi-year justification), were added to the inherited snapshot, the responsibility boundary and Task 1's read-only context, as internal policy context only — visible to the Procurement Officer and Head of Procurement Function, never rendered in the Invitation, the issued Tender or any supplier-visible output; (3) §22's two "required correction" claims against REQ-CHG-001 were removed because REQ-CHG-001 v1.6 already delivered both, replaced with the genuine remaining dependency, an inbound `PlanItemCorrectionRequest` handler in PLN-CHG-001; (4) the PLN-CHG-001 citation was updated from the stale v1.13 to v1.14; and (5) three dangling references to a non-existent "§17" corrections section were corrected to §22. v0.6 also added TPR-AC-044 and TPR-SMOKE-20 for the internal-only rendering rule. Earlier still: v0.5 removed `pe_fy_context_id` and the native-Frappe-Role/User-Permission authorization model, replacing both with AUTH-ADR-001 v1.6's role-bound `User Responsibility Assignment` and registered permission hooks; added the milestone-actual write-back Planning now depends on; added reservation-category and lotting rendering; renamed the golden fixture from Kenya Bureau of Standards to Ministry of Health and corrected a segregation-of-duties error the fixture had carried. Domain scope — Open Tender, one award package, IT goods and closely related services — is unchanged, and every sound mechanism from v0.4 (the five-task shell, the one-approver model, the digest-everywhere immutability, the upstream-correction route) is carried forward. |
...
> Native Frappe Roles, Workflow permissions and User Permissions, scoped by `pe_fy_context_id` → AUTH-ADR-001 v1.7 role-bound `User Responsibility Assignment`, resolved through registered permission hooks, on a site that is implicitly one Procuring Entity.
...
| Approved technical PDF published as Section V | Unchanged from v0.4: generate Section V from structured items, technical rows, services and acceptance rows. This correction is now delivered — STD-TPL-001 v0.6 is the template's own version 1.1 release. |
| Custom permission layers | v0.4 said: prohibited, use native Frappe Roles, Workflow permissions and User Permissions. **That is now the defect.** AUTH-ADR-001 v1.7 established that User Permission is removed from the KenTender authorization read path entirely; role-bound `User Responsibility Assignment`, resolved through registered `permission_query_conditions` and `has_permission` hooks, is the sole authorization mechanism. This is not a new custom layer — it is the one mechanism every other approved KenTender module already uses. |
...
| Golden fixture keyed to Kenya Bureau of Standards | Removed. Renamed to Ministry of Health throughout, per SEED-001 v1.2, reusing the KT-STD-001 shared fixture register rather than a second, non-existent entity. |
...
| Preference/reservation and lotting carried as inherited data with no rendering rule | Added. Section III renders the inherited reservation category and lotting indicator, per STD-STD-001 v1.1 §8–9 and STD-TPL-001 v0.6 §6.1. |
...
### 1.2 Correction in v0.6

v0.5 was written against an earlier state of REQ-CHG-001 and was never updated when REQ-CHG-001 advanced to its own approved v1.6. Four concrete errors resulted, found on a direct check against REQ-CHG-001's current primary text rather than against this document's own account of it:

| v0.5 said | REQ-CHG-001 v1.6 actually says | Correction in v0.6 |
|---|---|---|
| Compatibility test requires `AuthorisedRequisitionHandoff v1.2`, "or its corrected successor once REQ-CHG-001 is realigned" (§6.4) | The realigned handoff is `AuthorisedRequisitionHandoff v1.3` (§5.12), approved, in production use since v1.3 | §6.4 now names v1.3 directly, with no hedge |
| Silent on two fields v1.3 added to the handoff: Strategic Objective (with ancestor path) and plan horizon (with Multi-year justification) | Both are load-bearing handoff content — REQ-CHG-001 flagged their earlier absence as leaving "the policy justification for a public procurement invisible except by a second query into Planning" | Added to §4, §7.3 and §8.1 as internal context only — visible to the Procurement Officer and Head of Procurement Function, never rendered to a bidder. This was confirmed with the Project Owner: kept, but internal. |
| §22: "REQ-CHG-001 needs the same PE/FY-scope and User-Permission removal this document just received" | That removal happened at REQ-CHG-001 v1.3 and has held through v1.4, v1.5 and v1.6 | Claim removed from §22 |
| §22: REQ-CHG-001's upstream-correction mechanism is "currently one sentence... no command, no state" | REQ-CHG-001 §7.4A is a complete, approved mechanism — its own object (`PlanItemCorrectionRequest`), service contract, roles, acceptance criterion and smoke test | Claim removed from §22; replaced with the genuine remaining gap, which REQ-CHG-001 itself already names: PLN-CHG-001 has no inbound handler for `PlanItemCorrectionRequest` |

Three references to a "§17" section for these corrections (§4, §9.5, §10.4) were also dangling — §17 is the Acceptance contract, not a corrections section. The content they meant to point to lives, and now actually is, in §22. The PLN-CHG-001 citation in §22 is also updated from v1.13 to the current v1.14.
...
- the structured Requisition handoff, per REQ-CHG-001 v1.7's `AuthorisedRequisitionHandoff v1.3`.
...
| Strategic Objective and plan horizon | Authorised Requisition | Inherit read-only as internal policy context; visible to the Procurement Officer and Head of Procurement Function in Task 1 and the approval task; never rendered in the Invitation, the issued Tender or any supplier-visible output. |
...
| Administrator / System Manager | — | Technical administration and exact-payload retry only, per AUTH-ADR-001 v1.7 §8; no Tender decision. |
...
**This correction is complete.** STD-TPL-001 v0.6 is `IT-EQUIPMENT-OPEN-V1` version 1.1, and it does what this section required:
...
10. produced the Ministry of Health golden fixture, per SEED-001 v1.2, replacing the Kenya Bureau of Standards fixture; and
...
|---|---|
| Handoff version | `AuthorisedRequisitionHandoff v1.3`, per REQ-CHG-001 v1.7 |
| Product pattern | `IT Equipment` |
| Method | Open Tender |
| Currency | KES |
| Award package | One |
| Lotting indicator | `Single lot` |
| Reservation category | `None`, `Youth`, `Women`, `Persons with disabilities` or `Other disadvantaged group` — the categories STD-TPL-001 v0.6 §6.1 supports rendering. Any other value is incompatible in this release. |
...
- Strategic Objective and its ancestor path, and plan horizon with its Multi-year justification where applicable — internal policy context only, never rendered in the Invitation, the issued Tender or any other supplier-visible output; and
...
| Strategic Objective / plan horizon | Internal policy context from the inherited Plan Item; visible to the Procurement Officer and Head of Procurement Function only; never rendered in the Invitation, the issued Tender or any supplier-visible output. |
...
**What this does not solve.** `actual_bid_opening_date` and the five milestones after it remain entirely unwritable in this release. Bid opening is an Evaluation & Award event, and no such module exists yet. This section closes exactly one of the seven gaps PLN-CHG-001 named, not all of them, and the trigger for the one it does close — the downstream publication-consumption process — is itself undesigned. That is not a correction owed by an existing document; it depends on a future Evaluation & Award module that does not yet exist, noted but not solved here. See §22 for the one required correction this document can actually name against an existing document — a separate, unrelated gap in PLN-CHG-001.
...
**This route is well-defined at the Tender-to-Requisition hop.** It is not well-defined one hop further back. If the actual defect originates in the Plan Item, the Strategic Objective or the Budget Line — not in the Requisition itself — REQ-CHG-001 §7.4A already carries the correction as far as Planning: it closes the Requisition Version as `Upstream correction required` and creates one `PlanItemCorrectionRequest`, naming the Plan Item, the reason and the requesting Requisition. What does not yet exist is Planning's side of that route — an inbound handler and Procurement Planner task to receive it. This document does not fix that; it names it, in §22, as the thing to fix in PLN-CHG-001.
...
This section's own §13.2 fixture carried exactly this defect in its "Package" row until this correction — six unrelated facts joined onto one line with `·` under one label. The general rule against that is now centralised in KT-STD-001 §2.2, per that standard's own controlling decision that a rule repeated verbatim across change units is deleted from them once centralised — it is inherited here, not restated.

### 13.2 Shared Ministry of Health fixture

**Renamed from Kenya Bureau of Standards, per SEED-001 v1.2.** One site is one Procuring Entity; a fixture keyed to a second, non-existent entity cannot be run in the same live system as every other approved KenTender document.
...
**Package.** Broken out, not summarised into one line — an earlier version of this fixture crammed the whole of this into a single "Package" row ("2 inherited items... · 1 grouped schedule line · ... · 0 related services · 0 acceptance requirements beyond delivery"), the same fact-cramming pattern found and corrected in TPUB-CHG-001's design section:

| Field | Value |
|---|---|
| Inherited items | 2 — 100 HRMD-lineage, 150 DHI-lineage |
| Grouped schedule lines | 1 — both items share one technical requirement, per §13.4 |
| Related services | None |
| Acceptance requirements | None beyond delivery |
...
Two inherited equipment items — `SRC-MOH-033-001` (100 Each, HRMD lineage) and `SRC-MOH-033-002` (150 Each, DHI lineage), sharing one technical requirement — combine onto one grouped schedule line, per STD-TPL-001 §8.4(2): rows sharing a specification render as one Section V line, not two. Both source `requisition_item_id`s remain retrievable as lineage on that line, per TPR-AC-007 and TPR-AC-045.

| Item | Quantity | Unit | Delivery location | Latest delivery | Warranty | Inherited from |
|---|---:|---|---|---|---:|---|
| Business laptops | 250 (100 + 150) | Each | Ministry of Health Headquarters, Afya House, Nairobi | 30 Sep 2027 | 36 months | `SRC-MOH-033-001`, `SRC-MOH-033-002` |
...
The fixture uses the exact stable row IDs and content from REQ-CHG-001 v1.7's Ministry of Health fixture (§16).
...
1. **Ready to prepare** — one row for `REQ-MOH-2027-033-001`, Open Tender, 2 items, required by 30 Sep 2027, action **Prepare Tender**.
...
| 2. Goods and requirements | Complete · 2 inherited items, 1 grouped schedule line · technical rows per §13.4 |
| 3. Price schedule | Complete · Generated from the 1 grouped schedule line (2 inherited items) |
...
Task 3 shows one generated supplier price row for the one grouped schedule line, combining both inherited items (100 + 150 = 250). Quantity and units are inherited. Unit price and tax show **Completed by Tenderer**; line total and Tender total show **Calculated from Tenderer response**. No fixture or authorised value appears in these cells.
...
| Inherited requirements | Complete · 2 items, 1 grouped schedule line |
...
- 2 equipment items — `SRC-MOH-033-001` (100 Each, HRMD lineage) and `SRC-MOH-033-002` (150 Each, DHI lineage) — combining onto one grouped Schedule of Requirements line of 250 Each, per STD-TPL-001 §8.4(2);
...
| TPR-AC-044 | Strategic Objective and plan horizon are visible read-only in Task 1 and the approval task, and are absent from every rendered Invitation and issued Tender, tested against both outputs directly, not inferred from the input side. |
| TPR-AC-045 | Two or more inherited equipment items sharing one technical requirement combine onto one grouped Schedule of Requirements line; every contributing `requisition_item_id` remains retrievable as lineage on that line, tested against the actual rendered output, not assumed from the inherited count. |
...
4. render tests for structured Section V, reservation-category rendering, supplier-response schema, Invitation/issued-Tender separation, grouped same-specification schedule lines with lineage intact, and confirming Strategic Objective and plan horizon are absent from both rendered outputs;
...
| TPR-SMOKE-20 | Verify Strategic Objective and plan horizon appear in Task 1 and the approval task; verify both are absent from the rendered Invitation and issued Tender. |
| TPR-SMOKE-21 | Load the two-item Ministry of Health fixture; verify Task 2 and the rendered Invitation/issued Tender show one 250-unit line; verify both `SRC-MOH-033-001` and `SRC-MOH-033-002` remain retrievable as lineage on that line. |
...
The result is **GO**, **SIMPLIFY FURTHER** or **REJECT**. A **GO** walkthrough authorises implementation, provided the template 1.1 release evidence — already delivered by STD-TPL-001 v0.6 — also exists before code binds it.
...
- register Procurement Officer and Head of Procurement Function in the AUTH-ADR-001 v1.7 registry with `scope_type = Site-wide`, and register Tender DocTypes in `kentender_scope_map` through both permission hooks;
...
- No production Tender is created until the installed version 1.1 bundle, the AUTH-ADR-001 v1.7 registration, and contract tests all pass.

## 22. Traceability and precedence

This document conforms to:

1. **KT-STD-001 v1.4** for document structure, design closed-input rules, shared fixtures, page-state rules and universal prohibitions;
...
5. **STD-ST-001 v0.5** for productised code-owned Tender templates and no STD Configuration module;
6. **STD-TPL-001 v0.6**, for the reviewed PPRA Goods source, locked text, forms, structured Section V and the completed version 1.1 release evidence;
7. **PLN-CHG-001 v1.15**, as the consumer of the milestone-actual write-back this document supplies in §9.5, and as the document still missing the inbound handler named below; and
8. **SEED-001 v1.2**, for the exact harmonized identifiers this document's fixture uses, including the two-reservation correction in its own §4.3.

Where earlier Tender Preparation or template material treats an attachment as the primary technical requirement, or PE/FY scope and User Permission as the authorization model, this structured boundary and role-bound authorization control. The official PPRA source controls locked standard wording. REQ-CHG-001 v1.7 — approved, and already realigned to AUTH-ADR-001 v1.7, its own compatibility test, the Strategic Objective and plan-horizon fields, and the §7.4A upstream-correction mechanism — controls authorised business requirements. This document controls the Tender Preparation journey and downstream mappings.

**Required correction in another document.** REQ-CHG-001 v1.7 already carries every correction an earlier draft of this paragraph asked for — the PE/FY-scope and User-Permission removal, the Ministry of Health fixture rename, and a fully designed upstream-correction mechanism at §7.4A. What REQ-CHG-001 cannot finish alone is receipt: §7.4A's `PlanItemCorrectionRequest` has an outbound half only. **PLN-CHG-001 needs an inbound handler for `PlanItemCorrectionRequest`, exposed as a Procurement Planner task**, per REQ-CHG-001 §7.4A and §9.1. Until it exists, both REQ-CHG-001's and this document's upstream-correction routes end at a request Planning has no designed way to receive. This is the next document in sequence.

Separately, and not a correction owed by any existing document: the publication-consumption process this document's §9.5 and §10.2 depend on (`AcknowledgeTenderPublicationConsumed`) has no owning module. That is a future Evaluation & Award build, not a defect in a current one.

## 23. Approval effect and next action

TPR-CHG-001 v0.7 is approved. It supersedes v0.6 and all earlier versions in full and is the single Tender Preparation requirements document for the first IT-equipment slice. v0.6 was itself approved on 7 September 2026; this domain correction superseded that approval.

Approval authorises everything v0.6 introduced — the sync to REQ-CHG-001 v1.7's actual `AuthorisedRequisitionHandoff v1.3`, Strategic Objective and plan horizon as internal-only inherited context per TPR-AC-044, and the §22 corrections — plus, new in v0.7: the corrected two-item, one-grouped-line fixture, with both source `requisition_item_id`s carried as lineage per TPR-AC-045.

Approval does not permit an incomplete template or improvised handoff. The next action is the required correction to PLN-CHG-001 named in §22 — the inbound `PlanItemCorrectionRequest` handler — after which both REQ-CHG-001's and this document's upstream-correction routes are complete end to end.

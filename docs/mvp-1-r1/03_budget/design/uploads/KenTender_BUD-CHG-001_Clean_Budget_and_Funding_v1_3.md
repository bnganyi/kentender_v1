...
| Supersedes | v1.2 (approved 28 August 2026) and all earlier versions, in full |
| Module | Budget & Funding |
| Change type | Realigns authorization, scope, fiscal year, DocType naming, fixtures and artboards to the one-site-one-PE model. Domain arithmetic, funding lineage and the Planning boundary are carried forward unchanged. |
...
| Finance Confirmation Officer | Site-wide | Open an assigned Procurement Planning Finance task and confirm or return it. Grants no Budget authoring, approval or activation authority. |
...
Procurement Planning solely owns the Finance task and its assignment, the sufficient, insufficient, return and planner-waiting UI states, the immutable Finance decision, and the Plan Item's reservation references.

Budget & Funding solely owns live line eligibility and positions, the locked availability check, creation of all reservations as one atomic operation, later revalidation, release and conversion, and the funding ledger.

The existing Planning surfaces `PLN-UI-07`, `PLN-UI-07A-1`, `PLN-UI-07A-2` and `PLN-UI-07B` remain valid and are reused. This change unit creates no second Finance drawer, form, decision page or waiting page.

**Open Budget & Funding** from a Planning Finance surface opens BUD-UI-05 for the exact line selected in the Finance allocation. Navigation creates no decision, reservation or mutation. For a combined Plan Item, each allocation opens its own line detail.
...
2. Budget resolves the caller's Active Finance Confirmation Officer assignment and validates every line, amount, owner scope, funding source and current position.
3. `check_funding` returns one short-lived token and a position for every allocation; it writes nothing.
4. Confirm Funding calls `reserve_funding` with the token, expected task and version locks, and one idempotency key.
...
| BUD-AC-011 | Need acceptance, DPP submission and Plan Item draft save create no reservation. |
...
- Do not create a reservation at Need acceptance, DPP submission or Plan Item draft save.
...
| PLN-CHG-001 / PLN-FR-001 | Finance contracts drop the Procuring Entity argument; `resolve_budget_context` takes a Fiscal Year only. Budget Line identifiers are unchanged. |

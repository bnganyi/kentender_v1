...
| Date | 28 August 2026 |
| Status | Approved |
| Approval | Product owner · 28 August 2026 |
...
This document is the single implementation authority for the Budget & Funding cleanup. It replaces the earlier Budget MVP requirements, BUD-CHG-001 v1.0 and BUD-CHG-001 v1.1 wherever they conflict with this document.
...
| Budget Reviewer and Budget Activation Authority split | Remove. Retain only **Budget Officer** and **Budget Approver**. Approval includes the recommend decision and activation as one atomic action, mirroring the two-role model shipped for Strategy Alignment (`KenTender_STR-CHG-001_Clean_Strategy_Alignment_v1.5.md` §6–7). The `In Review` and `Awaiting Activation` statuses are removed; a submitted version moves directly from `Submitted for approval` to `Active` or back to `Draft`. |
...
- a new Frappe shell, header, breadcrumb, global selector or navigation system;
- a separate Budget Reviewer or Budget Activation Authority workflow role, or a distinct recommend-then-activate two-step decision; or
...
The external approval reference, approval date and approval document pass this gate because the Budget Approver uses them to verify that the line set faithfully represents an externally approved procurement budget. They are not generic source-reference fields.
...
| `status` | Controls editability, decision actions and eligibility: `Draft`, `Submitted for approval`, `Active`, `Superseded` or `Closed`. |
...
Submitted, approved/activated, superseded and closed actors and timestamps are audit events, not editable fields. A Return reason is stored on the decision event, not as a generic version field.
...
| BUD-BR-005 | Only Draft and Returned-through-decision versions are editable; Submitted for approval, Active, Superseded and Closed versions are read-only. |
...
| BUD-BR-021 | Approval rechecks approval evidence, line total, floors, transfer balance, scope and concurrency under one transaction lock before activating. |
| BUD-BR-022 | Approving a successor atomically makes it Active and the previous Active Version Superseded while preserving all line, reservation and commitment identities. |
...
Version 1 is the initial baseline. Version 2 and later are successor revisions. There is no separate Budget Revision workflow, and there is no separate recommend-then-activate two-step decision — Approve both decides and activates in one atomic transaction.

### 6.2 Governance rules

- A Budget Officer may create and edit only Draft versions in an assigned PE/FY scope.
- The submitting Budget Officer cannot approve the same version, even if that user also holds Budget Approver.
- Return requires 10–500 characters and preserves the submitted snapshot and decision event.
- Approval means “available for KenTender procurement control”; it does not approve the public budget.
- At most one open Draft or Submitted-for-approval successor may exist for a Budget.
...
| Budget Approver | Inspect a submitted version; return it with a reason or approve and activate it within assigned scope. |
...
Only two Budget Version workflow roles exist: Budget Officer and Budget Approver. Assignments are scoped by PE, FY, optional organisation unit, capability and effective dates. The same server-side predicate applies to counts, rows, direct URLs, exports, services and task queues. A selected PE/FY changes context only; it grants no authority.
...
| `return_budget_version` | Require a correction reason and return a Submitted-for-approval version to Draft. |
| `approve_budget_version` | Revalidate authority, evidence, line total, floors, transfer balance, scope and concurrency; atomically activate the submitted version and supersede the previous Active version where applicable. |
...
- **Budget & Funding**; and
- **Approval tasks**, visible only to a scoped Budget Approver.
...
| BUD-UI-04 Approval task | `/app/budget/review/{budget_version_id}` | Budget Approver inspection and return-or-approve decision. |
...
The approval task uses **Overview**, **Budget Lines**, **Changes** and **History**. The proven Vue-in-Frappe page pattern and approved KenTender design system shall be reused. This document does not authorise a second dashboard, application shell or Frappe header.
...
| Approved and activated by | MOH Budget Approver |
...
**Implementation note (no `.dc.html` artboard exists for this screen):** build this screen by reusing, verbatim, the BUD-DES-04 header/tabs component and the Budget Lines table component as it renders in the BUD-DES-13 family's Budget Lines duplicate — do not design new table chrome.
...
|---|---|---|
| 3 Oct 2026, 11:15 EAT | Version 1 approved and activated | MOH Budget Approver |
...
**Implementation note (no `.dc.html` artboard exists for this screen):** build this screen by reusing, verbatim, the BUD-DES-04 header/tabs component and the version-history table component as it renders in BUD-DES-07's sibling Funding Activity event table — same row/column chrome, different event set as shown above.

### 11.9 BUD-DES-08 — Approval task · Overview

**Fixture context — outside the artboard:** MOH Budget Approver · `moh.budget.approver@example.test` · Ministry of Health · FY 2027/28 · 16 Mar 2027, 10:15 EAT · Frappe header breadcrumb: **Home > Budget & Funding > Approval tasks > MOH-BUD-2027-001-V2**
...
### 11.10 BUD-DES-09 — Approval task · Budget Lines
...
### 11.11 BUD-DES-10 — Approval task · Changes
...
### 11.12 BUD-DES-11 — Approval task · History
...
### 11.13 BUD-DES-12 — Retired

BUD-DES-12 (the separate "Activation task" duplicates of BUD-DES-08–11) modelled the now-removed `Awaiting Activation` status as a second task screen for the Budget Activation Authority. That status and role no longer exist: BUD-DES-08–11 already cover the single `Submitted for approval` decision point, and **Approve** on that same screen both decides and activates. Do not build a second task screen.

### 11.14 BUD-DES-13 — Initial baseline review · four tab variants

Create four initial-baseline Approver artboards by duplicating BUD-DES-08, BUD-DES-09, BUD-DES-10 and BUD-DES-11 respectively.

**Fixture context for all four artboards — outside the artboard:** MOH Budget Approver · `moh.budget.approver@example.test` · Ministry of Health · FY 2027/28 · 2 Oct 2026, 10:00 EAT · Frappe header breadcrumb: **Home > Budget & Funding > Approval tasks > MOH-BUD-2027-001-V1**
...
### 11.14A BUD-DES-13A — Retired

BUD-DES-13A (the "Initial baseline activation" duplicates) modelled the same now-removed `Awaiting Activation` stage for Version 1. BUD-DES-13 already covers the single `Submitted for approval` decision point for the initial baseline; Approve both decides and activates. Do not build a second task screen.
...
- Direct task routes require the exact current Budget Approver capability. A neutral viewer is denied rather than shown disabled workflow controls.
...
- The decision footer remains available on every tab. Every command carries Budget Version ID, expected status and expected record version.
- A Budget Approver may Return or Approve only while status is Submitted for approval.
- Return opens a dialog containing only **Return reason**, **Cancel** and **Return**. The reason is validated server-side.
- Approve reruns evidence, totals, line identity, floor, transfer, scope and concurrency checks under transaction locks and activates the version in the same transaction — there is no separate later activation step.
- Successful successor approval changes the new version to Active and the prior Active Version to Superseded atomically.
...
- The Budget Officer who submitted a version cannot approve it, even if that user also holds Budget Approver — enforced from the version's own submission audit event, not a separate capability-pair table.
...
- submit, return, approve/activate, supersede and close;
...
|---|---|---|
| `moh.budget.officer@example.test` | MOH Budget Officer | Budget Officer and separately assigned MOH Finance Confirmation capability for FY 2027/28. No approval. |
| `moh.budget.approver@example.test` | MOH Budget Approver | Budget Approver for PE-MOH and FY 2027/28 — return, approval and self-approval-segregation tests. No authoring. |
...
| `cgk.budget.approver@example.test` | Kisumu Budget Approver | Budget Approver for PE-CGK only. |
...
| Approved and activated | MOH Budget Approver | 3 Oct 2026, 11:15 EAT |
...
The revision shown in BUD-DES-08 through BUD-DES-15 is an isolated test fixture, not part of the default Active baseline. It represents a version that has been submitted but not yet decided, used to populate the pending Approval task screens.
...
The DHI floor is KES 80,000,000 from the default reservation. The proposed KES 90,000,000 amount therefore remains valid with KES 10,000,000 headroom. A separate isolated test copy of this fixture is decided twice in isolation — once returned to Draft, once approved and activated — to exercise both Budget Approver outcomes; the version is never left mid-transition between test runs.
...
| BUD-AC-004 | A System Administrator without a Budget assignment cannot create, submit, return, approve, close or confirm Finance. |
...
| BUD-AC-007 | Submission and approval fail when approval data is incomplete or the line total differs from authorised total. |
| BUD-AC-008 | Budget Approver return and approval use one scoped capability distinct from Budget Officer; the submitting Officer cannot approve their own version, even if they also hold Budget Approver. |
...
| BUD-AC-023 | Approving a valid successor atomically supersedes the prior Active Version and preserves Budget Line, reservation and commitment identities. |
...
| BUD-AC-033 | Budget Approver inspects Overview, Budget Lines, Changes and History for the exact submitted Version; no tab substitutes Active data. |
...
| BUD-AC-037 | No executable Budget metadata, permission, route, service, seed or active test refers to Budget Reviewer, Budget Activation Authority as a workflow role, or the removed `In Review`/`Awaiting Activation` statuses. |

### 16.1 Minimum automated coverage

| Rule group | Required automated coverage |
|---|---|
| Scope and permissions | BUD-BR-001–003; BUD-AC-003–004, 026–027, 037 |
...
- Replace the Budget Reviewer and Budget Activation Authority workflow roles with one **Budget Approver** role. Retain **Budget Officer**. Remove the `In Review` and `Awaiting Activation` statuses; a submitted version resolves directly to Draft (Return) or Active (Approve).
- Remove the separate `review_budget_version`/`activate_budget_version` two-step command pair; replace with `return_budget_version` and `approve_budget_version`, where `approve_budget_version` performs the full recheck-and-activate transaction in one call.
- Existing Budget Activation Authority holders may be granted Budget Approver during the controlled migration. Existing Budget Reviewer holders are not promoted automatically. Remove old roles after all permissions, fixtures and tests use the two-role model.
...
- Keep Claude Design exports under `docs/` as visual evidence. Port their semantic composition into repository-owned Vue components. Where an artboard is marked retired by this document (BUD-DES-12, BUD-DES-13A), do not port it.
...
- Budget Approver: open exact task → inspect all four tabs → return in one test run, approve in another;
...
- static scan showing no removed concept, expenditure artifact, legacy Demands/Home import, or leftover Budget Reviewer/Budget Activation Authority/`Awaiting Activation` reference in executable Budget code;
...
- No separate Budget Reviewer recommendation step reintroduced alongside Budget Approver activation, and no revival of the `In Review`/`Awaiting Activation` statuses under another label.

## 19. Traceability and precedence

This document reconciles and supersedes conflicting Budget requirements contained in:

- `KenTender_BUD-CHG-001_Clean_Budget_and_Funding_v1.0.md`;
- `KenTender_BUD-CHG-001_Clean_Budget_and_Funding_v1.1.md`; and
- `01_Budget_and_Funding_MVP1_Requirements.md`.

It replaces v1.1 in full: the Budget Version governance role and lifecycle model (sections 1.1, 4.2, 6, 7, 9.2, 11 artboards BUD-DES-08–13A, 12.5, 15.2–15.3, 15.6, 16 and 17.2) is the substantive change, mirroring the two-role Author/Approver collapse shipped for Strategy Alignment in `KenTender_STR-CHG-001_Clean_Strategy_Alignment_v1.5.md`. All other v1.1 content — the domain model outside `BudgetVersion.status`, the calculations and invariants, the Finance confirmation boundary, the error contract, and the seed arithmetic — is carried forward unchanged.
...
Budget stores no Strategy field. The selected Strategic Objective and its snapshot remain governed by `KenTender_STR-CHG-001_Clean_Strategy_Alignment_v1.5.md` and Procurement Planning.

Where an earlier Budget item is not retained in this v1.2 document, it is outside the approved Budget & Funding scope.

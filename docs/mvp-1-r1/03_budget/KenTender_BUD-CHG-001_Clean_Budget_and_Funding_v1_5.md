...
| Version | 1.5 |
| Change type | **Amendment to the approved v1.4.** Adopts KT-STD-001 v1.2 §3A: the authorisation verdict resolves before any content renders, a page-load denial is an inline state and never a modal, navigation is gated rather than hidden, and the Forbidden panel names the responsibilities that open the surface. No domain decision changes. |

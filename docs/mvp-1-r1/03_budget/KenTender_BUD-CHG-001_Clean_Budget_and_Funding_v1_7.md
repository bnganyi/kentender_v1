...
| Version | 1.7 |
| Change type | Corrects the exact gap found in production: Administrator, holding AUTH-ADR-001 §8's technical-read right, had no way to reach a Draft or Submitted budget version except a hand-typed URL, and the workspace's own read-only route (§12.3) explicitly scoped itself to Active, Superseded and Closed only. Adds BUD-DES-01A, a fourth server-decided workspace outcome documented in §12.1, and documents that BUD-UI-02 — already correctly authorising a technical reader in production, per direct testing — is the route it opens, read-only. Clarifies that the workspace's Forbidden state is a business-role check that never catches a technical reader, closing the same gap at the module's own entry point. Adds BUD-AC-042. This corrects documentation to match a right AUTH-ADR-001 v1.7 §8 already grants; no new right is created, no existing business rule, role or command changes. Earlier: Retires §15.4's stale per-Plan-Item reservation seed, which contradicted this document's own corrected funding model, and replaces it with the correct Requisition-stage reservation, per SEED-001 v1.0. Earlier still: v1.5 adopted KT-STD-001 v1.2 §3A page-state rules with no domain change; v1.4 moved funding reservation from Procurement Planning to Procurement Requisition and added the plan affordability contract. |
| Date | 10 September 2026 |
| Status | **Approved** |
| Approved on | 11 September 2026 |
| Supersedes | v1.6 and all earlier versions, in full — v1.6 was never itself approved, per its own header |
...
| Finance Confirmation Officer confirming per Plan Item | **Confirms once per Plan Version** against the affordability statement, per PLN-CHG-001 v1.15 §4.11. The role, its scope and its registry entry are unchanged. |
...
| `financial_year_id` referencing a KenTender `FinancialYear` | Replace with `fiscal_year`, referencing the ERPNext `Fiscal Year` governed by CFG-CHG-002 v0.9 §4.2. |
...
| Business authority and scope resolution | AUTH-ADR-001 v1.7 | Declare required business roles; implement no permission mechanism. |
...
**Correction.** Until this version, no artboard showed a technical reader anything for a Draft or Submitted version — the workspace and BUD-UI-03 both scoped to Active, Superseded and Closed only, and AUTH-ADR-001 §8's technical-read right had no artboard exercising it. This is that artboard.

**Fixture context — outside the artboard:** Administrator · System Manager · 10 Dec 2026, 15:05 EAT · Frappe header breadcrumb: **Home > Budget & Funding**

Same page content header, filter row and Current budget card as BUD-DES-01 — an Active version, if one exists for the year, still renders identically; this variant adds to it, never replaces it.

**Additional card**, shown only when a Draft or Submitted-for-approval version exists for the selected year:

- Heading: **Budget Version 2 — Submitted for approval** (fixture: a successor Draft submitted by Josphat Mwangi, awaiting a Budget Approver)
...
| Submitted | 12 Dec 2026, 09:00 EAT |
...
Duplicate BUD-DES-06, but for `MOH-BL-HWD-2027`, not `MOH-BL-DHI-2027` — this is the one Budget Line SEED-001's harmonized Requisition actually reserves against, per §15.4A.

**Fixture context — outside the artboard:** Naomi Chebet · `naomi.chebet@moh.example.test` · Auditor · 10 Dec 2026, 15:05 EAT · opened from the Active Budget Lines table · Frappe header breadcrumb: **Home > Budget & Funding > MOH-BL-HWD-2027**

Page content header eyebrow: **MOH-BL-HWD-2027**. Title: **Digital health workforce development**. Line identity card owner scope: **HR Management and Development**. Keep every other page content header, context strip and Line identity card element unchanged.

**Funding position row — four equal cards**

| Card label | Value |
|---|---:|
| Approved | KES 60,000,000 |
| Reserved | KES 50,000,000 |
| Committed | KES 0 |
| Available | KES 10,000,000 |

**Active reservations table**

| Reservation | Plan Item | Original amount | Remaining amount | Status | Action |
|---|---|---:|---:|---|---|
| RSV-MOH-2027-033-001 | PPI-MOH-2027-033 · Clinical training and deployment laptops for digital health rollout | KES 50,000,000 | KES 50,000,000 | Active | View Plan Item |
...
|---|---|---|---|---:|---|
| 4 Dec 2026, 10:00 EAT | Reservation confirmed | MOH-BL-HWD-2027 | PPI-MOH-2027-033 · RSV-MOH-2027-033-001 | KES 50,000,000 | Josphat Mwangi |
...
Naomi Chebet (Auditor) is added by STR-CHG-001 v1.7 §14.1 and reused here. Samuel Otieno, already in the register with an expired assignment, is the Forbidden fixture actor.
...
### 15.4 Integrated Planning position

**Corrected.** This section previously described a per-Plan-Item Finance task and a reservation created during Planning — the model this document's own §7–8 and BR-009 removed. Reservation begins only at Procurement Requisition; Planning's contribution is the one plan-level affordability statement, already seeded in PLN-CHG-001 §14.6.

Default position with no Requisition yet raised against either line:

| Budget Line | Approved | Reserved | Committed | Available |
|---|---:|---:|---:|---:|
| MOH-BL-DHI-2027 | KES 100,000,000 | KES 0 | KES 0 | KES 100,000,000 |
| MOH-BL-HWD-2027 | KES 60,000,000 | KES 0 | KES 0 | KES 60,000,000 |
| Total | KES 160,000,000 | KES 0 | KES 0 | KES 160,000,000 |

PLN-CHG-001 §14.5 carries two live Plan Items against these lines — `PPI-MOH-2027-021` (KES 80,000,000, `MOH-BL-DHI-2027`) and `PPI-MOH-2027-033` (KES 50,000,000, `MOH-BL-HWD-2027`) — both planned, neither reserved, both within their line's approved amount. `PPI-MOH-2027-033` is the item SEED-001 carries through Requisition into a live reservation; §15.4A gives the exact reservation this produces once REQ-CHG-001's harmonized Requisition is raised against it.

### 15.4A Reservation produced by the harmonized Requisition

This is the one reservation created anywhere in this seed set, and it is created at Requisition, not at Planning. SEED-001 is authoritative for the exact Requisition and Tender identifiers; this table gives only the Budget-side result.

| Record | Exact value |
|---|---|
| Reservation | `RSV-MOH-2027-033-001` |
| Plan Item | `PPI-MOH-2027-033` |
| Budget Line | `MOH-BL-HWD-2027` |
| Amount | KES 50,000,000 |
| Confirmed by | The Requisition authorisation event, per REQ-CHG-001, not a Budget or Planning actor |
| Status | Active |

Resulting position after this one reservation:

| Budget Line | Approved | Reserved | Committed | Available |
|---|---:|---:|---:|---:|
| MOH-BL-DHI-2027 | KES 100,000,000 | KES 0 | KES 0 | KES 100,000,000 |
| MOH-BL-HWD-2027 | KES 60,000,000 | KES 50,000,000 | KES 0 | KES 10,000,000 |
| Total | KES 160,000,000 | KES 50,000,000 | KES 0 | KES 110,000,000 |
...
| BUD-AC-039 | Every page resolves its authorisation verdict before rendering; a denied actor sees the inline Forbidden panel with no header, filter, content or empty state painted, and no permission modal appears on page load. |
| BUD-AC-040 | The Forbidden panel names the responsibilities that open the surface and directs the user to a KenTender administrator; it names no line manager or supervisor. |
| BUD-AC-041 | Selecting this module without access pushes its own route, highlights it in navigation, and lands on its Forbidden state; the module is never hidden and route and view never diverge. |
| BUD-AC-042 | Administrator and System Manager can reach and read a Draft or Submitted-for-approval version's full content and lines, read-only, via BUD-DES-01A and BUD-UI-02, without any Budget business assignment; the same request returns Not Found for a user holding no assignment and no technical-read grant. |
...
2. **AUTH-ADR-001 v1.7** for business authority, the role registry, responsibility assignment and the registered permission hooks.
3. **CFG-CHG-002 v0.9** for the site Procuring Entity, the ERPNext Fiscal Year surface, Organisation Unit records and the funding-source catalogue.
...
Budget stores no Strategy field. The selected Strategic Objective and its snapshot remain governed by STR-CHG-001 v1.7 and Procurement Planning.
...
| PLN-CHG-001 v1.15 | Aligned. One plan-level funding confirmation, no reservation at planning, `check_plan_affordability` consumed at plan submission, and no Procuring Entity argument on any contract. Procurement Budget Line identifiers are unchanged. |
...
BUD-CHG-001 v1.7 is approved. It supersedes v1.6 and all earlier versions in full and is the only Budget & Funding document to consult. v1.6 was never itself approved, per its own header — this was that same document's first approval, not a re-approval.

This approval authorises BUD-DES-01A, the corrected §12.1 and §12.2 rules, and BUD-AC-042 — closing the technical-read gap found in production — in addition to everything v1.6 already stated: the §15.4 correction, retirement of the stale per-Plan-Item reservation seed and its replacement with the correct Requisition-stage reservation, per SEED-001 v1.0; the move of funding reservation from Procurement Planning to Procurement Requisition; the `check_plan_affordability` contract and its blocking and advisory verdicts; and plan-level rather than per-item funding confirmation. It further authorises: the DocType rename to the `Procurement Budget` family; removal of `procuring_entity_id`; adoption of the ERPNext `Fiscal Year`; registration of Budget Officer, Budget Approver and Finance Confirmation Officer as site-wide business roles resolved through the AUTH-ADR-001 v1.7 permission hooks; removal of the Budget Viewer role, every capability string and every Fiscal Year permission grant; full separation from ERPNext accounting with no Cost Center reference; retention of `owner_org_unit_id` as a record-eligibility rule; removal of the Kisumu isolation baseline, its actors and every cross-PE isolation test; adoption of the KT-STD-001 shared fixture register; the corrected artboards in §11; and the acceptance contract in §16.

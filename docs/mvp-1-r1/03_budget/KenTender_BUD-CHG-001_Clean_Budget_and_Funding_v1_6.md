...
| Version | 1.6 |
| Change type | Retires §15.4's stale per-Plan-Item reservation seed, which contradicted this document's own corrected funding model, and replaces it with the correct Requisition-stage reservation, per SEED-001 v1.0. Earlier versions: v1.5 adopted KT-STD-001 v1.2 §3A page-state rules with no domain change; v1.4 moved funding reservation from Procurement Planning to Procurement Requisition and added the plan affordability contract. |
...
BUD-CHG-001 v1.6 supersedes v1.5 and all earlier versions in full and, on approval, becomes the only Budget & Funding document to consult. It further authorises the §15.4 correction: retirement of the stale per-Plan-Item reservation seed and its replacement with the correct Requisition-stage reservation, per SEED-001 v1.0.

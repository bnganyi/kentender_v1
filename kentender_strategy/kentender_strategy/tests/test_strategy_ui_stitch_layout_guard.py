...
class TestStrategyUiStitchLayoutGuard(FrappeTestCase):
	def test_tokens_and_shell_exist(self):
		...
		self.assertIn("periodCellHtml(p.effective_period_label", live)
		self.assertIn("kt-str-period-wrap", live)
		...
		# Plan column gets room; period stacks start/end.
		...
		self.assertIn("table-layout: fixed", portfolio_css)
		self.assertIn("kt-str-period-wrap", portfolio_css)
		self.assertIn("kt-str-plans-col-type", portfolio_css)
		self.assertIn("kt-str-period-start", portfolio_css)
		self.assertIn("white-space: nowrap", portfolio_css)
		self.assertNotIn("max-width: 6.75rem", portfolio_css)
		pf_fx = _read(FIXTURES / "portfolio.js")
		self.assertIn('title="Effective period"', pf_fx)
		self.assertIn(">Period</th>", pf_fx)
		...
		# Create must not reintroduce heavy primary field focus (module lock wins).
		self.assertNotIn("0 0 0 2px color-mix(in srgb, #001f48", create_css)
		self.assertNotIn(
			'[data-testid="kt-str-create-plan"] input:focus',
			create_css,
		)
		self.assertIn("use module lock in strategy_alignment_responsive.css", create_css)
		...
		# Shared table footer — Strategy shim + canonical core artifact.
		footer_fx = _read(FIXTURES / "table_footer.js")
		self.assertIn("kentender_core.ui_fixtures.tablePaginationFooterHtml", footer_fx)
		self.assertIn('testid: "kt-str-table-footer"', footer_fx)
		core_footer = (
			Path(frappe.get_app_path("kentender_core")) / "public" / "js" / "kt_stitch_table_footer.js"
		)
		self.assertTrue(core_footer.is_file(), core_footer)
		...
		self.assertIn('value="100"', core_fx)
		...
		self.assertIn("data-kt-str-footer-pager", footer_fx)
		util_css = _read(UTILITIES)
		...
		# Last-loaded focus lock for all Strategy data-entry fields.
		self.assertIn("Focus chrome lock", responsive)
		self.assertIn("body.kt-str-surface.kt-cl-shell .kt-str-root :is(input, select, textarea", responsive)
		self.assertIn("var(--kt-str-focus-border", responsive)
		self.assertIn("box-shadow: 0 0 0 1px var(--kt-str-focus-ring", responsive)
		shell = _read(SHELL)
		...
		self.assertIn("rememberPlanChrome", shell)
		self.assertIn("hydratePlanChrome", shell)
		self.assertIn("harvestPlanChrome", shell)
		self.assertIn("data-kt-str-chrome-hydrated", shell)
		self.assertIn("rememberPlanChrome", live_js)

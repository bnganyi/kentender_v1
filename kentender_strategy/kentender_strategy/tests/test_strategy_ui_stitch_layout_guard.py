...
class TestStrategyUiStitchLayoutGuard(FrappeTestCase):
	def test_tokens_and_shell_exist(self):
		...
		# Shared table footer (Showing X of Y + Rows per page + numbered pager).
		...
		self.assertIn("kt-str-footer-page-size-glyph", footer_fx)
		self.assertIn("expand_more", footer_fx)
		...
		self.assertIn("Rows per page", footer_fx)
		self.assertNotIn("Page Size", footer_fx)
		self.assertIn("Showing 0 of 0", footer_fx)
		self.assertNotIn("Showing 0 of 0 records", footer_fx)
		util_css = _read(UTILITIES)

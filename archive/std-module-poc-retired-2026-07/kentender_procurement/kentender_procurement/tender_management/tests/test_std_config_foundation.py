...
class TestStdConfigFoundation(IntegrationTestCase):
	...
	def test_boot_exposes_flag(self) -> None:
		...
		proc = boot["kentender_procurement"]
		self.assertIn("std_config_ui_v2_enabled", proc)
		self.assertIn("can_use_std_advanced_catalogue", proc)
		self.assertIn("can_view_technical_json", proc)
		self.assertIn("can_edit_technical_json", proc)

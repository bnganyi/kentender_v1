...
def _advanced_detail(
	...
	section_projection = project_advanced_section_content(package)
	base_sections = [
	...
	for section in base_sections:
		...
		projected = section_projection.get(key) or {}
		if projected.get("summary"):
			payload["summary"] = projected["summary"]
		if projected.get("rows"):
			payload["rows"] = projected["rows"]
		sections.append(payload)
	json_text, truncated = raw_package_json_text(package)
	return {
		"intro_text": intro_text,
		"sections": sections,
		"raw_package": {
			"collapsed_by_default": not bool(json_text),
			"technical_label": "Technical (Read-Only)",
			"read_only": True,
			"visible_for_advanced_users": has_advanced_permission,
			"json_text": json_text if has_advanced_permission else "",
			"truncated": truncated,
		},
		"editing": {
			"enabled": False,
		...
		"source_mappings": project_source_mappings(package, fallback_rows=mapping_rows),
	}
...
def get_std_library_template_detail(version_code: str) -> dict:
	...
	doc = row[0]
	package = parse_package_json(str(doc.get("package_json") or ""))
	doc_ns = frappe._dict(doc)
	display_title = resolve_template_title(doc_ns, package)
	display_methods = resolve_procurement_methods(doc_ns, package)
	display_family = resolve_template_family(doc_ns, package)
	...
	methods = display_methods or _supported_methods(str(doc.get("procurement_method_profile") or ""))
	...
	advanced_detail = _advanced_detail(lifecycle_status, user_roles, package)
	audit_detail = _audit_detail(code, user_roles)
	return {
		"ok": True,
		"detail": {
			"title": display_title or str(doc.get("template_title") or code),
			...
			"summary": {
				"identity": {
					...
					"authority": str(doc.get("source_authority") or ""),
					"template_family": display_family,
				},
				"source_evidence": {

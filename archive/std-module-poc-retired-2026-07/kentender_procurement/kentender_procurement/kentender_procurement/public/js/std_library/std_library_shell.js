...
<div class="std-library-detail-tab-content" role="tabpanel" id="std-library-detail-tabpanel" aria-labelledby="${activeTestId}">${tabContentHtml}</div>`;
		...
		detailInner.querySelectorAll("[data-testid='std-advanced-open-technical-json']").forEach((btn) => {
			btn.addEventListener("click", function () {
				const code = btn.getAttribute("data-kt-std-advanced-configure") || item?.version_code || "";
				if (code) {
					frappe.set_route("std-configurator", code, "technical-json");
				}
			});
		});
	}

...
<div class="kt-std-cfg-root" data-testid="kt-std-cfg-root">
	...
	<div class="kt-std-cfg-canvas">
		...
		<div class="kt-std-cfg-page-head" data-testid="kt-std-cfg-doc-header">
			...
			<p class="kt-std-cfg-subtitle">${__(
				"Configure the structural definitions and rules for the building works standard document. This version will serve as the master template for all entity-level procurement.",
			)}</p>
		</div>
		<div class="kt-std-cfg-tabbar-wrap">
	...
	function _setActiveTab(slug, options) {
		const normalized = _normalizeTabSlug(slug);
		const nextTab = _tabBySlug(normalized, _context);
		const next = nextTab ? normalized : "overview";
		_activeTab = next;
		if (_root) {
	...
	shell.mount = function mount(wrapper, templateCode, tabSlug) {
		...
		_activeTab = _normalizeTabSlug(tabSlug || "overview");

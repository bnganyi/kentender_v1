...
| ID | Screen | Architecture | Design source | Deployed asset | Layout guard | Playwright | Status | Evidence |
|---|---|---|---|---|---|---|---|---|
| ITW-01 | Tender Configuration Dashboard | **native** | `IT-STD-Wizard-v2/screen-01/code.html` | `it_wizard/*` + `it_wizard_dashboard_page.js` + `kt_it_wizard.css` | `test_it_wizard_dashboard_desk_wiring` + `test_kt_fonts_selfhosted` + `test_it_wizard_native_architecture` | `dashboard-desk-wiring.spec.ts` + `dashboard-visual.spec.ts` | Done | Native componentized 2026-07-17; shared shell library; ITW-BE-DASH-001..004; PW-ITW-DASH-01..08 |
| ITW-02 | Tender STD Configuration Overview | **native** | `02 std-config-overview/code.html` | `it_wizard/screens/configuration_home.js` + `it_wizard_overview_page.js` + `kt_it_wizard.css` | `test_it_wizard_overview_desk_wiring` + `test_it_wizard_native_architecture` | `overview-desk-wiring.spec.ts` | Done | Native re-migration 2026-07-17 — iframe removed; shared shell library; PW-ITW-OVERVIEW 6/6 plain DOM; design HTML reference only |
| ITW-03 | Tender Profile | iframe-transitional | `03 tender-profile/code.html` | `it_wizard_tender_profile.html` | `test_it_wizard_ui_tender_profile_layout_guard` | `tender-profile-desk-wiring.spec.ts` | Done (transitional) | ITW-LG-03 (3/3); pending native migration |
| ITW-04 | Tender Data Sheet (TDS) | iframe-transitional | `04 tds/code.html` | `it_wizard_tds.html` | `test_it_wizard_ui_tds_layout_guard` | `tds-desk-wiring.spec.ts` | Done (transitional) | ITW-LG-04 (3/3); pending native migration |
| ITW-05 | IT Requirements | native | `IT-STD-Wizard-v2/screen-03/code.html` | `it_wizard_it_requirements.html` (reference) | `test_it_wizard_ui_it_requirements_layout_guard` | `it-requirements-desk-wiring.spec.ts` | Done (native) | `it-wizard-screen-03-gate`; v2 flat table + drawer A–E; 2026-07-17 |
| ITW-06 | Implementation Schedule | iframe-transitional | `06 implementation-schedule/code.html` | `it_wizard_implementation_schedule.html` | `test_it_wizard_ui_implementation_schedule_layout_guard` | `implementation-schedule-desk-wiring.spec.ts` | Done (transitional) | ITW-LG-06 (3/3); pending native migration |
| ITW-07 | System Inventory | iframe-transitional | `07 system-inventory/code.html` | `it_wizard_system_inventory.html` | `test_it_wizard_ui_system_inventory_layout_guard` | `system-inventory-desk-wiring.spec.ts` | Done (transitional) | ITW-LG-07 (3/3); pending native migration |
| ITW-08 | Price Schedule | iframe-transitional | `08 price-schedule/code.html` | `it_wizard_price_schedule.html` | `test_it_wizard_ui_price_schedule_layout_guard` | `static-screens.spec.ts` (price) | Done (transitional) | ITW-LG-08 (3/3); pending native migration |
| ITW-09 | Evaluation Setup | iframe-transitional | `09 evaluation-setup/code.html` | `it_wizard_evaluation_setup.html` | `test_it_wizard_ui_evaluation_setup_layout_guard` | `static-screens.spec.ts` (evaluation) | Done (transitional) | ITW-LG-09 (3/3); pending native migration |
| ITW-10 | Forms & Evidence | iframe-transitional | `10 forms-and-evidence/code.html` | `it_wizard_forms_and_evidence.html` | `test_it_wizard_ui_forms_and_evidence_layout_guard` | `static-screens.spec.ts` (forms) | Done (transitional) | ITW-LG-10 (3/3); pending native migration |
| ITW-11 | SCC / Contract Carry-Forward | iframe-transitional | `11 scc/code.html` | `it_wizard_scc.html` | `test_it_wizard_ui_scc_layout_guard` | `static-screens.spec.ts` (scc) | Done (transitional) | ITW-LG-11 (3/3); pending native migration |
| ITW-12 | Validation Report | iframe-transitional | `12 validation-report/code.html` | `it_wizard_validation_report.html` | `test_it_wizard_ui_validation_report_layout_guard` | `static-screens.spec.ts` (validation) | Done (transitional) | ITW-LG-12 (3/3); pending native migration |
| ITW-13 | Review & Approval | iframe-transitional | `13 review-and-approval/code.html` | `it_wizard_review_and_approval.html` | `test_it_wizard_ui_review_and_approval_layout_guard` | `static-screens.spec.ts` (review) | Done (transitional) | ITW-LG-13 (3/3); pending native migration |
| ITW-14 | Final Tender Preview | iframe-transitional | `14 render-preview/code.html` | `it_wizard_render_preview.html` | `test_it_wizard_ui_render_preview_layout_guard` | `static-screens.spec.ts` (preview) | Done (transitional) | ITW-LG-14 (3/3); pending native migration |
| ITW-15 | Publication Readiness | iframe-transitional | `15 publication-readiness/code.html` | `it_wizard_publication_readiness.html` | `test_it_wizard_ui_publication_readiness_layout_guard` | `static-screens.spec.ts` (publication) | Done (transitional) | ITW-LG-15 (3/3); pending native migration |
| ITW-00 | Preview index | design-reference | — | `index.html` | — | `static-screens.spec.ts` (index) | Done | PW-ITW-00; not a production Desk screen |
...
- DocTypes, services, whitelisted APIs, Procurement sidebar link (Tender Management cluster)
- **Native re-implementation (2026-07-17):** replaced the iframe hydrator + static HTML with a self-contained native Desk page (`it_wizard_dashboard_page.js` / `.css`, `frappe.call` only, zero external requests) following the DIA create-demand pattern. Byte-for-byte HTML layout guard removed in favour of the structural guard `test_it_wizard_dashboard_desk_wiring`.
- **Create-modal logout fix (2026-07-17):** `wizard_create_options_service.list_create_options_for_it_wizard` no longer calls `frappe.set_user()` inside the web request. `frappe.set_user()` rewrites `frappe.session.sid` to the username, corrupting the response `sid` cookie and logging the user out on the next request (create → Guest → re-login). Permission is now gated via `frappe.has_permission(..., user=act)`. Regression: `test_create_options_context_does_not_corrupt_session_sid`.
- **Visual-fidelity re-port + self-hosted font foundation (2026-07-17):** Screen 01 re-ported 1:1 to `IT-STD-Wizard-v2/screen-01/code.html` — fixed top app bar + footer toolbar, full-screen takeover (Desk navbar + left sidebar hidden; brand tile routes back to Procurement), KPI cards with progress bars + qualitative deltas, always-visible filter chips, numbered pagination + rows-per-page selector, Material Symbols icons (replacing inline SVG), and brand typography (Manrope/Inter/JetBrains Mono).
  - **Self-hosted fonts (app-wide):** `public/fonts/**` (Manrope/Inter/JetBrains Mono latin+latin-ext woff2 + Material Symbols Outlined variable) + shared `public/css/kt_fonts.css` (`@font-face`, relative `/assets` URLs, no CDN), registered first in `hooks.py` `app_include_css`. Licenses: OFL/Apache-2.0 (`public/fonts/LICENSES.md`).
  - **Iframe screens 02–15 swapped off the Google Fonts CDN** to the self-hosted `kt_fonts.css` (14 `it_tender_wizard_impl/*.html`). Tailwind CDN left in place (separate follow-up). Verbatim layout guards updated to normalize the font-`<link>` swap (`normalize_font_links`).
  - Evidence: `test_kt_fonts_selfhosted` (6/6); `test_it_wizard_dashboard_desk_wiring` design-chrome markers; PW-ITW-DASH-06/07/08 (chrome takeover, self-hosted brand fonts + Material Symbols loaded, numbered pagination); `dashboard-visual.spec.ts` snapshot; all 14 `it-wizard-static-gate` verbatim guards green.
- Gate: `make it-wizard-screen-01-gate SITE=kentender.midas.com` + `make kt-fonts-gate SITE=kentender.midas.com` (full ITW-01..06: `make it-wizard-wiring-regression-gate`)

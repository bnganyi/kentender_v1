...
from kentender_procurement.it_tender_wizard.services.wizard_create_options_service import (
	list_create_options_for_it_wizard,
	merge_create_payload_from_package,
	resolve_create_option,
)
...
def list_eligible_tender_shells() -> list[dict[str, Any]]:
	"""Deprecated shim — map create options to legacy shell rows."""
	assert_permission(PERM_VIEW)
	return [
		{
			"id": row["procurement_package_id"],
			"code": row["planning_package_ref"],
			"name": row.get("planning_package_name") or row["procurement_package_label"],
			"planning_package_ref": row["planning_package_ref"],
			"procuring_entity_id": row.get("procuring_entity_id") or "",
			"procuring_entity_name": row.get("procuring_entity_name") or "",
			"procurement_method_code": row.get("procurement_method_code") or "",
			"procurement_method_label": row.get("procurement_method_label") or "",
		}
		for row in list_create_options_for_it_wizard(frappe.session.user)
	]


def _resolve_preselect_procurement_package_id(
	*,
	procurement_package_id: str | None,
	tender_id: str | None,
) -> str:
	preselect = (procurement_package_id or "").strip()
	if preselect:
		return preselect
	legacy = (tender_id or "").strip()
	if not legacy:
		return ""
	if resolve_create_option(legacy):
		return legacy
	alt = frappe.db.get_value("Procurement Package", {"package_code": legacy}, "name")
	return (alt or "").strip()


def get_create_configuration_context(
	*,
	procurement_package_id: str | None = None,
	tender_id: str | None = None,
	std_version_id: str | None = None,
	plan_item_id: str | None = None,
) -> dict[str, Any]:
	"""Payload for Screen 01 create modal: approved procurement package options."""
	assert_permission(PERM_VIEW)
	create_options = list_create_options_for_it_wizard(frappe.session.user)
	preselect = _resolve_preselect_procurement_package_id(
		procurement_package_id=procurement_package_id,
		tender_id=tender_id,
	)
	return {
		"create_options": create_options,
		"preselect_procurement_package_id": preselect,
		"plan_item_id": (plan_item_id or "").strip(),
		"std_version_id": (std_version_id or "").strip(),
	}


def create_configuration(payload: dict[str, Any]) -> dict[str, Any]:
	assert_permission(PERM_CREATE)
	if (payload.get("procurement_package_id") or "").strip():
		payload = merge_create_payload_from_package(
			str(payload.get("procurement_package_id")),
			payload,
		)
	...
	if (
		...
		or payload.get("procurement_plan_item_id")
		or payload.get("procurement_package_id")
	):
...
def get_configuration_summary(configuration_id: str) -> dict[str, Any]:
	...
	return build_configuration_home(configuration_id)

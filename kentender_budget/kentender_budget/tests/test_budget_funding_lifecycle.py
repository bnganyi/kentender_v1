...
def _purge_test_reservations_for_budget(budget_code: str = "MOH-BUD-0001") -> None:
...
class TestBudgetFundingLifecycle(FrappeTestCase):
	...
	def test_moh_0001_includes_reservation_commitment_expenditure_sorted(self):
		dto = list_funding_lifecycle("MOH-BUD-0001")
		...
		self.assertIn("COM-MOH-0001", by_code)
		self.assertIn("EXP-MOH-0001", by_code)
		self.assertEqual(by_code["RSV-MOH-0001"]["source_doctype"], "Funding Reservation")
		self.assertEqual(by_code["COM-MOH-0001"]["source_doctype"], "Procurement Commitment")
		self.assertEqual(by_code["EXP-MOH-0001"]["source_doctype"], "Expenditure Snapshot")
	...
	def test_reserve_dedup_activity_stable_audit_has_evidence(self):
		...
		key = "TEST:LIFECYCLE-DEDUP:MOH-BL-0002:15000000.00"
		...
		life = list_funding_lifecycle("MOH-BUD-0001")
		domain_rsv = [
			e
		...
		activity = list_funding_activity("MOH-BUD-0001")
		...
		audit = get_budget_audit("MOH-BUD-0001", event_type=EVENT_RESERVED)
	...
	def test_pe_moe_officer_denied_moh_budget(self):
		frappe.set_user("budget.officer@moe.test")
		try:
			with self.assertRaises(frappe.PermissionError):
				list_funding_lifecycle("MOH-BUD-0001")
		finally:
			frappe.set_user("Administrator")
	...
	def test_reserve_updates_activity_via_shared_path(self):
		line = frappe.db.get_value("Budget Line", {"generated_reference": "MOH-BL-0002"}, "name")
		before = list_funding_activity("MOH-BUD-0001")
		before_codes = {r["code"] for r in before["rows"]}
		key = "TEST:LIFECYCLE-ACT:MOH-BL-0002:12000000.00"
		result = reserve_funding(
			budget_line=line,
		...
		after = list_funding_activity("MOH-BUD-0001")

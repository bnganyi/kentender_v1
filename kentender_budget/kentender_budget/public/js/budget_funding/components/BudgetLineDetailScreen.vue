		...
		<div class="kt-shell" style="max-width: 1000px">
			...
			<template v-else-if="line">
				<div class="kt-card kt-blueprint" style="padding: 0">
					...
					<div style="display: flex; gap: 32px; padding: 22px 28px; border-bottom: 1px solid var(--kt-color-divider); flex-wrap: wrap">
					...
					<div style="padding: 22px 28px; border-bottom: 1px solid var(--kt-color-divider)">
						<div class="kt-kpi-row" style="margin-bottom: 10px" data-testid="bud-line-position-cards">
							<div class="kt-kpi-card">
								...
								<div class="kt-kpi-value">{{ formatKes(line.positions.approved, currency) }}</div><div class="kt-kpi-sub">{{ __("Registered allocation") }}</div>
							</div>
							<div class="kt-kpi-card">
								...
								<div class="kt-kpi-value" :class="{ 'is-zero': !line.positions.reserved }">{{ formatKes(line.positions.reserved, currency) }}</div><div class="kt-kpi-sub">{{ __("Reserved for requisitions") }}</div>
							</div>
							<div class="kt-kpi-card">
								...
								<div class="kt-kpi-value" :class="{ 'is-zero': !line.positions.committed }">{{ formatKes(line.positions.committed, currency) }}</div><div class="kt-kpi-sub">{{ __("Committed to contracts") }}</div>
							</div>
							<div class="kt-kpi-card" :class="availableClass">
								...
								<div class="kt-kpi-value">{{ formatKes(line.positions.available, currency) }}</div><div class="kt-kpi-sub">{{ __("Available to reserve") }}</div>
							</div>
						</div>
					...
					<div style="padding: 22px 28px; border-bottom: 1px solid var(--kt-color-divider)">
						<h3 class="kt-card-title" style="margin: 0 0 14px; display: flex; align-items: center; gap: 6px">
							<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12.586 2.586A2 2 0 0 0 11.172 2H4a2 2 0 0 0-2 2v7.172a2 2 0 0 0 .586 1.414l8.704 8.704a2.426 2.426 0 0 0 3.42 0l6.58-6.58a2.426 2.426 0 0 0 0-3.42Z" /><circle cx="7.5" cy="7.5" r=".5" fill="currentColor" /></svg>
							{{ __("Line identity") }}
						</h3>
				...
				<p class="kt-muted" style="font-size: 12px; margin-top: 12px">{{ __("There is no release, convert or adjust action here; those belong to the owning Requisition and Contract processes.") }}</p>
			</template>
		</div>
	</div>
</template>

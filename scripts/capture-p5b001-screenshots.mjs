/**
 * One-off MCP screenshot capture for P5B-001 tracker evidence.
 * Run: node scripts/capture-p5b001-screenshots.mjs
 */
...
async function capture(page, route, filename) {
	...
	await page.waitForSelector('[data-testid="pp2-page-header"]', { timeout: 60_000 });
...
await login(page);
await capture(page, '/desk/procurement-planning', 'p5b001_planning_home_page_header_mcp.png');
await capture(page, '/desk/procurement-planning/packages', 'p5b001_packages_page_header_mcp.png');
await browser.close();
console.log('done');

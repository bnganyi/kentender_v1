...
- **Loading state: the skeleton is only for a screen that has nothing to show yet.** A screen already loaded in this session (Back, Cancel, a save that routes, the page coming back into view) renders its last payload at once and refreshes in place, exposing `data-refreshing="true"` on the shell while it does (CSS draws the 2px bar). Keep the last payload per screen identity in `kentender_core.desk_page.createScreenCache()` (see `ProcurementPlanning.vue`/`DepartmentalNeeds.vue`), or wrap sibling screens in `<KeepAlive>` and revalidate in `onActivated` (see `Budget.vue`). A `v-if="loading"` that replaces content the user was just looking at is a defect, not a loading state. Every loader carries a sequence token so a slower, older response can never overwrite a newer one.

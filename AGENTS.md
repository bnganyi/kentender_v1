...
- **Page controller: one call, never hand-rolled.** The whole `public/js/<slug>_page.js` is:
  ```js
  kentender_core.desk_page.register("<slug>", {
  	title: __("..."),
  	bundles: ["kt_industry_page_rail.bundle.js", "<slug>.bundle.js"],
  	mount: (el) => frappe.kt_mount_<slug>(el),
  	sidebarWorkspaceKey: "procurement",
  	// group + pages: only when several Frappe Pages share one bundle (Planning's four)
  });
  ```
  `kentender_core.desk_page` (`kt_desk_page.js`, always loaded) owns `on_page_load/show/hide`: it mounts the app **once and keeps it alive across navigations** (Frappe keeps every page container in the DOM and only show()/hide()s it), moves one live app between the pages of a `group`, applies the Industry chrome idempotently, and tells the app when it is hidden/resumed. Do not write `on_page_hide → app.unmount()`, do not `empty()` the mount point, do not `frappe.require` in a page controller yourself — every one of those was a hand-copied variant that re-introduced the blank-then-skeleton flash on navigation (root-caused 2026-09-05, eleven controllers).
...
- Read the route through the app's `useRouteState(pageSlug)` composable, which is a thin adapter over `kentender_core.desk_page.useRoute(...)` (each app keeps the adapter only because bundles do not share a Vue runtime — §6.6). It returns `{ route, go, epoch, isShown }`: one `frappe.router` listener per app, paused while the page is hidden or the component is `KeepAlive`-deactivated, re-synced on resume, and `epoch` ticks when the page resumes on the same route — `watch(epoch, () => load({ quiet: true }))` is how a screen revalidates what it already shows. Never subscribe to `frappe.router.on("change")` directly in a screen. (**`frappe.router.off()` does not unbind anything** — Frappe's `EventEmitterMixin.off()` rebuilds a fresh wrapper closure and unbinds *that*, a confirmed framework bug — which is one more reason the listener lives in core, bound once per long-lived app.)
- **Loading state: the skeleton is only for a screen that has nothing to show yet.** A screen already loaded in this session (Back, Cancel, a save that routes, the page coming back into view) renders its last payload at once and refreshes in place, exposing `data-refreshing="true"` on the shell while it does (CSS draws the 2px bar). Keep the last payload per screen identity in `kentender_core.desk_page.createScreenCache()` (see `ProcurementPlanning.vue`/`DepartmentalNeeds.vue`), or wrap sibling screens in `<KeepAlive>` and revalidate in `onActivated` (see `Budget.vue`). A `v-if="loading"` that replaces content the user was just looking at is a defect, not a loading state. Every loader carries a sequence token so a slower, older response can never overwrite a newer one — use `kentender_core.desk_page.createSequenceGuard()` (`next()`/`isCurrent(token)`) instead of hand-rolling a `let request = 0` counter per screen.
- **A command's post-mutation reload must be awaited *inside* the same async function whose pending flag guards the buttons — never issued after that guarded call has already returned.** Every command handler needs the same pending-guard + try/catch/finally shape: use `kentender_core.desk_page.createCommandRunner(vue, { onStart, onError, mintKey })` for it rather than hand-copying a local `run()` (`onStart(label)`/`onError(err, label)` keep the screen's own error-surface convention — inline banner vs. dialog field; `onError` receiving `null` from a caught error is intentional, matching every existing local `run()`'s "swallow and surface" contract; `mintKey(label)` keeps the screen's own idempotency-key convention, all optional). Whatever runner is used, the reload that refreshes the optimistic-lock stamp (`record_version`/`expected_version`/`modified`) the *next* command will send must be the last thing awaited inside the function passed to `run()` — not a separate `await load(...)` after `await run(...)` returns. Root-caused 2026-09-11: every hand-copied `run()` correctly held `pending` across the mutating call, but callers then fired the post-save reload after `run()` already returned and cleared `pending`, so a second command could fire with a stale stamp before that reload landed. Confirmed live in Departmental Needs (Save draft → immediate Submit refused as a stale write) and Budget (an edit made in that window silently reverted by the stale reload); latent in Procurement Planning, Procurement Requisitions, Tender Preparation, and System Setup's `ProcuringEntityTab.vue` before the 2026-09-11 fix pass (`docs/mvp-1-r1/00_common/`).
- **Controlled inputs are bound to the caller's own selection, never to the server echo.** Vue re-patches `value` on every render, so `<select :value="response.financial_year">` snaps back to the previous year while the new one is still loading and then jumps forward — the "dropdown changes by itself" defect. Bind to the ref the change handler sets synchronously and let the response merely confirm it.
- **An editor never re-hydrates its form from an in-place refresh that carries nothing new.** Guard the `watch(() => props.record, hydrate)` on record identity + `record_version` (or a content signature), or a background revalidation discards what the user has typed.
...
- An Industry (Vue-in-Desk) page never touches `kentender_core.cl_shell` itself: `kentender_core.desk_page.register()` applies the native chrome (keeps Frappe's workspace sidebar, hides the navbar/page-head, empties `#kt-cl-chrome-host`) and `kt_cl_shell_router.js` defers to `kentender_core.desk_page.ownsRoute()` for every registered slug, so it never calls `leaveNative()` under such a page. Before that deferral existed the router stripped the `kt-cl-shell` body classes after **every** navigation (Frappe fires the router `change` event after `on_page_show`), reflowing the whole content column twice per click. The navbar is restored automatically when the route leaves to a non-registered page.
...
- Do **not** add an Industry page to `kentender_core.cl_surface_registry.js` — that table renders the Civic Ledger (Tailwind) toolbar, a different design system. It remains the registry for the legacy Civic Ledger surfaces only.
...
- **`kt_industry_tokens.css` (`.kt-industry` scope, owned by `kentender_core`) is the one canonical design system for the entire application — a hard rule, not a suggestion.** No app may define its own competing token file (its own `--*-color-accent`, its own `.kt-btn`/`.kt-input`/`.kt-card`/`.kt-status` component rules, its own button-height/radius/spacing scale). Every Vue-in-Desk page wraps its root in `class="kt-industry"` and consumes `kt_industry_tokens.css` directly, loaded via that app's own `hooks.py` `app_include_css` pointed at `kentender_core`'s published asset URL (`/assets/kentender_core/css/kt_industry_tokens.css`, cache-busted via a local `_asset_version()` helper resolving `kentender_core`'s file path — see `kentender_strategy/hooks.py` for the pattern). Frappe serves every installed app's static assets from one shared `/assets/<app_name>/` namespace, so this cross-app load is the same mechanism as any of an app's own `app_include_css` entries, just pointed at a different app's published path — there is no platform reason to fork a second copy. This applies to vanilla-JS Desk pages too, not just Vue: `.kt-industry`'s classes are plain CSS (see `kentender_core/public/js/authorization_admin_pages.js` for a non-Vue consumer).
  ...
  - Civic Ledger (`kt_cl_*`, Tailwind-based content system) and Stitch Desk (`kt_stitch_desk_chrome.css`, its own Tailwind vocabulary, enforced internally-consistent by `ui-stitch-desk-chrome-gate`) are **legacy debt, not sanctioned alternatives to Industry.** They exist today only because the modules using them have not yet been rebuilt — both predate this rule and are non-Vue (vanilla-JS/Tailwind fixture) systems, so neither appears on the Industry gate's allowlist. `ui-stitch-desk-chrome-gate` keeps Stitch Desk internally consistent purely as a stopgap against further drift while it awaits replacement, not as endorsement. **Never build new screens against either system, and never extend either system to cover new surfaces — even inside a module that currently uses one of them.** Any new screen, or any existing screen undergoing more than a one-line patch, is built or migrated onto `.kt-industry`. Legacy modules get migrated module-by-module, each rebuilt fully onto Industry the way Strategy was, not patched in place. The Civic Ledger sidebar shell (`kt_cl_shell.js`'s `enterNative()`, §6.4/6.5 above) is orthogonal to this rule and is never touched by an Industry migration — it is shared native chrome, not a content design system.
  - **Using the correct token file is necessary but not sufficient — the `.dc.html` artboard's markup is the literal build source for any screen it covers, not background reference.** Every screen with a `docs/**/design/*.dc.html` file has an exact, complete HTML/CSS design: the wrapper structure, card/frame classes (`.kt-blueprint`/`.kt-corner`), section-title elements (`.kt-card-title`), and layout (e.g. a fixed footer bar) are all literally present in that file's markup. Build the component by porting that structure class-for-class and element-for-element — not by re-deriving a layout from the prose spec section plus general knowledge of which `kt-industry` classes exist. A component can use 100%-correct tokens and classes, satisfy `ui-industry-design-gate`, and still look like an approximation, because gate compliance only checks *which* design system is in use, never whether the artboard's actual composition was reproduced. Confirmed failure case: `StdCfgPackage.vue` and `StdCfgDocuments.vue` (Phase 11, commit `20f0177`) both wrap real content in bare `.kt-card`/`.kt-table` with no `.kt-blueprint`/`.kt-corner` frame and no `.kt-card-title` section labels, even though both classes exist in `kt_industry_tokens.css` and are used correctly in the *loading skeleton* of the same files — because the skeleton was written by glancing at the pattern, and the real content was written from the spec's content checklist instead of the artboard's markup. When a screen's rendered output is checked against its design, diff it against the `.dc.html` file directly, not from memory of "does it use Industry."
  - **The artboard rule above is machine-enforced, and every UI phase must ship its enforcement — prose compliance has empirically failed twice** (STD Phase 11 after the rule was given verbally; the 2026-09-02 System setup build after it was written here). The pattern is `make ui-system-setup-fidelity-gate` / `tests/ui/smoke/design-fidelity/` with `tests/ui/helpers/designFidelity.ts`: a Playwright spec renders the screen's own `.dc.html` in the same browser (with the dc-runtime `support.js` blocked, so the literal markup is the oracle) and derives every expectation from that render — the artboard's ordered structural landmarks (card titles, dialog titles, labels, legends, table headers, buttons) must appear in order in the live page, and geometry measured off the artboard (column ratios, row heights, dialog widths, grid label columns, truncation state of named fixture text) must match within tolerance. Fixture *data* (ids, codes, dates, names) is exempt; structure and geometry are not. **Definition of done for any screen with an artboard: component tests + browser journey + its design-fidelity spec green.** When building or rebuilding a module's screens, add them to a `design-fidelity` spec as each screen lands (per-route vertical slices), not at the end.
  - **A plan that reuses an existing Vue component for a screen with a new or changed artboard must declare that reuse as an explicit exception to the artboard rule and get the owner's sign-off in the plan itself.** Both recorded violations of the artboard rule entered through "reuse the previous build's components" decisions that silently carried the old composition into screens whose design had changed. Reused components count as new code for the fidelity gate: they pass the *new* artboard's spec or they are ports, not reuse.
  - Cascade trap behind several confirmed fidelity defects (620px dialogs, lost card padding, 2026-09-03): `kt_industry_tokens.css` can be loaded again *after* a page's own stylesheet (multiple apps include it), so a page rule at the same specificity as a tokens base rule (`.kt-industry .kt-foo` vs `.kt-industry .kt-card`) silently loses. When overriding a property that a tokens base class also sets, chain the base class into the selector (`.kt-industry .kt-card.kt-foo`, `.kt-industry .kt-dialog.kt-foo`) so specificity, not load order, decides.
...
### 6.10 No default Frappe popups for user-correctable errors (hard rule)

**User-correctable validation must never surface as Frappe's generic "Message" dialog** (`frappe.msgprint` chrome, or the modal Frappe's own `frappe.call()` machinery pops automatically for a `MandatoryError`/`frappe.throw()`-style rejection). A field the user can fix on the current screen gets an inline error next to that field — never a second, duplicate popup on top of it.

- **API/service contract**: a write endpoint that can fail on user input returns a structured failure — `{"ok": false, "errors": {"field_name": "Human message"}}` — instead of raising `frappe.throw()`/letting a DocType's `reqd:1` raise `MandatoryError`. Enforce "is this field mandatory *right now*" in the service layer (e.g. only at submit/approval, not at draft save) rather than as an unconditional DocType-level `reqd:1`, whenever the field is legitimately optional at an earlier stage — see `Procurement Budget Version.approval_document`, enforced only at submit-for-review by `budget_readiness_contracts.py`, not at draft insert.
- **Client contract**: every shared `frappeCall(method, args)` data-adapter (one physical copy per app per §6.6 — `kt_admin_shared`, `budget_shared`, `nds_shared`, `pln_shared`, `strategy_shared`, `reference_data`, and any new one) must call `frappe.call({ ..., silent: true })`. This is not cosmetic: per Frappe's own `frappe/public/js/frappe/request.js`, `frappe.request.cleanup()` calls `frappe.msgprint(messages)` for *any* response carrying `_server_messages` — including a plain `frappe.throw()` or core `MandatoryError` — independently of whatever `error`/`catch` handling the caller does, unless `opts.silent` is set (`if (messages && !opts.silent) { ... frappe.msgprint(messages); }`). Without `silent: true`, an unhandled server exception renders twice: once as the default dialog, once as the screen's own inline error. `silent: true` does **not** suppress the distinct dialogs Frappe raises for permission (403) or server-error (500/502/504) status codes — those are separate code paths in the same file and stay allowed (see below).
  - **When adding a new `frappeCall.js` copy, copy this flag along with it — it has silently gone missing in at least one app copy before** (`kentender_budget/public/js/budget_shared/data/frappeCall.js` was missing it while four sibling copies already had it, producing exactly the "Value missing for …" Message-dialog bug this rule now documents). Diff a new copy against an existing one (e.g. `kentender_core/public/js/kt_admin_shared/data/frappeCall.js`) rather than retyping it from memory.
- **Allowed**: `frappe.show_alert()` for success feedback, permission refusals, or a non-field system failure; an intentional titled dialog for a genuine hard stop that isn't about a specific form field.
- **Evidence before calling a form screen done**: submit it with a required field missing and confirm — in a real browser, not just an API-level test — that the failure renders inline next to that field and that no dialog titled "Message" (or containing "Value missing for …") appears.
- This mirrors `.cursor/rules/kentender-form-errors.mdc` and the `kt_form_errors.js`/`KtErrorBanner.vue` helpers already in `kentender_core`; this section is the authoritative statement of the rule for this file's own precedence order (§1).

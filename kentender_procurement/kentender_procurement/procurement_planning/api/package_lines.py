...
"""P4-007 — Package line traceability read API."""
...
def _planning_read_gate() -> tuple[str | None, dict[str, Any] | None]:
	return pp_api_gates.planning_api_read_gate(
		...
		message=_("You do not have access to the Procurement Planning package lines."),
		fail=_fail,
	)


@frappe.whitelist()
def get_pp_package_line_traceability(package_code: str | None = None) -> dict[str, Any]:
	"""Whitelisted PP2 package lines — demand item → package line → budget mapping."""
	...
	return get_package_line_traceability(code, frappe.session.user)

...
"""P2-013 — planning correction and supersession service."""
...
from kentender_procurement.procurement_planning.services.planning_correction_service import (
	create_planning_correction_or_supersession,
)
...
def _correction_payload(**overrides) -> dict:
	base = {
		"correction_type": CORRECTION_TYPE_POST_RELEASE,
		"reason": "Scope change requires governed correction",
		"affected_fields_json": {"procurement_method": "Restricted Tender"},
	}
	base.update(overrides)
	return base


class TestPP2PlanningCorrectionP2013(IntegrationTestCase):
	def setUp(self):
		super().setUp()
	...
	def _seed_budget_line(self) -> tuple[str | None, str | None, str | None, str | None]:
		...
		dept = ensure_department(f"Dept Corr {frappe.generate_hash(length=4)}", ent)
		...
		plan = frappe.get_doc(
			{
				...
				"plan_name": f"Corr plan {frappe.generate_hash(length=4)}",
				"plan_code": f"PP-CORR-{frappe.generate_hash()[:6]}",
				"fiscal_year": 2029,
	...
	def _mk_demand(self, bl_name: str, entity: str, dept: str) -> frappe.model.document.Document:
		doc = frappe.get_doc(
			{
				"doctype": "Demand",
				"title": f"Corr demand {frappe.generate_hash(length=4)}",
				"demand_id": f"DEM-CORR-{frappe.generate_hash()[:8]}",
	...
	def _mk_journey(self, demand_id: str) -> str:
		jc = f"JRN-CORR-{frappe.generate_hash()[:8]}"
		now = now_datetime()
		frappe.db.sql(
			"""
			...
			""",
			(jc, now, now, jc, f"Correction test journey {jc}", demand_id),
		)
	...
	def _create_draft_package(self) -> tuple[str, str, str, str, str]:
		...
		item_code = f"DEMITEM-CORR-{frappe.generate_hash()[:8]}"
		frappe.db.commit()
	...
	def _released_package(self) -> tuple[str, str]:
	...
	def _replacement_draft_package(self) -> str:
		package_code, _, _, _, _ = self._create_draft_package()
		return package_code
	...
	def _assert_blocker(self, exc: BaseException, code: str) -> None:
		blob = self._err_text(exc).lower()
		self.assertIn(code.lower(), blob, msg=f"expected {code} in {self._err_text(exc)!r}")

	def test_post_release_correction_unlocks_editing(self) -> None:
		if self._skip:
		...
		if not frappe.db.exists("DocType", "Planning Correction Supersession Record"):
			self.skipTest("Planning Correction Supersession Record not migrated")
		package_code, _journey = self._released_package()
		...
		with self.assertRaises(frappe.ValidationError) as locked:
			pkg.save(ignore_permissions=True)
		self.assertIn(POST_RELEASE_LOCK_MESSAGE, str(locked.exception))

		out = create_planning_correction_or_supersession(
			package_code, _correction_payload(), "Administrator"
		)
		self.assertTrue(out.get("ok"))
		correction_code = out.get("correction_code")
		self.assertTrue(correction_code)
		self._track("Planning Correction Supersession Record", correction_code)

		pkg.reload()
		self.assertEqual(pkg.status, PKG_RETURNED)
		self.assertFalse(bool(pkg.locked_after_release))
		pkg.method_override_flag = 1
		pkg.method_override_reason = "Post-release correction requires method change"
		pkg.procurement_method = "Restricted Tender"
		pkg.save(ignore_permissions=True)
		pkg.reload()
		self.assertEqual(pkg.procurement_method, "Restricted Tender")

	def test_supersession_marks_package_superseded(self) -> None:
		if self._skip:
			self.skipTest("PP or Demand not installed")
		if not self._require_template():
			self.skipTest("No active Procurement Template with profiles available")
		if not frappe.db.exists("DocType", "Planning Correction Supersession Record"):
			self.skipTest("Planning Correction Supersession Record not migrated")
		replacement_code = self._replacement_draft_package()
		package_code, _journey = self._released_package()
		out = create_planning_correction_or_supersession(
			package_code,
			_correction_payload(
				correction_type=CORRECTION_TYPE_SUPERSESSION,
				replacement_package_code=replacement_code,
				reason="Replace released baseline with revised package",
			),
			"Administrator",
		)
		self.assertTrue(out.get("ok"))
		...
		pkg = frappe.get_doc("Procurement Package", package_code)
		self.assertEqual(pkg.status, PKG_SUPERSEDED)
		corr = frappe.get_doc("Planning Correction Supersession Record", correction_code)
		self.assertEqual(corr.supersedes_package_code, package_code)
		self.assertEqual(corr.superseded_by_package_code, replacement_code)
	...
	def test_blocked_without_reason(self) -> None:
		...
		if not self._require_template():
			self.skipTest("No active Procurement Template with profiles available")
		package_code, _ = self._released_package()
		with self.assertRaises(frappe.ValidationError) as ctx:
			create_planning_correction_or_supersession(
				package_code,
				_correction_payload(reason=""),
				"Administrator",
			)
		self._assert_blocker(ctx.exception, PackagePlanningCorrection.REASON_REQUIRED)

	def test_blocked_wrong_state(self) -> None:
		if self._skip:
			self.skipTest("PP or Demand not installed")
		if not self._require_template():
			self.skipTest("No active Procurement Template with profiles available")
		...
		self._assert_blocker(ctx.exception, PackagePlanningCorrection.INVALID_STATE)

	def test_blocked_supersession_without_replacement(self) -> None:
		if self._skip:
			self.skipTest("PP or Demand not installed")
		if not self._require_template():
			self.skipTest("No active Procurement Template with profiles available")
		...
		with self.assertRaises(frappe.ValidationError) as ctx:
			create_planning_correction_or_supersession(
				...
				_correction_payload(
					correction_type=CORRECTION_TYPE_SUPERSESSION,
					replacement_package_code="",
				),
				"Administrator",
			)
		self._assert_blocker(ctx.exception, PackagePlanningCorrection.REPLACEMENT_REQUIRED)

	def test_direct_edit_still_blocked_before_correction(self) -> None:
		if self._skip:
			self.skipTest("PP or Demand not installed")
		if not self._require_template():
			self.skipTest("No active Procurement Template with profiles available")
		...
		with self.assertRaises(frappe.ValidationError) as ctx:
			pkg.save(ignore_permissions=True)
		blob = self._err_text(ctx.exception)
		self.assertTrue(
			PackagePostReleaseLock.LOCKED_AFTER_RELEASE in blob
			or POST_RELEASE_LOCK_MESSAGE in str(ctx.exception)
		)

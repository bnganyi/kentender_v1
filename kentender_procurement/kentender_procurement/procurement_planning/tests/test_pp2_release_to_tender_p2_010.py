...
"""P2-010 — release_package_to_tender_management service."""
...
class TestPP2ReleaseToTenderP2010(IntegrationTestCase):
	def setUp(self):
		super().setUp()
	...
	def _seed_budget_line(self) -> tuple[str | None, str | None, str | None, str | None]:
		...
		dept = ensure_department(f"Dept Rel {frappe.generate_hash(length=4)}", ent)
		...
		plan = frappe.get_doc(
			{
				...
				"plan_name": f"Rel plan {frappe.generate_hash(length=4)}",
				"plan_code": f"PP-REL-{frappe.generate_hash()[:6]}",
				"fiscal_year": 2029,
	...
	def _mk_demand(self, bl_name: str, entity: str, dept: str) -> frappe.model.document.Document:
		doc = frappe.get_doc(
			{
				"doctype": "Demand",
				"title": f"Rel demand {frappe.generate_hash(length=4)}",
				"demand_id": f"DEM-REL-{frappe.generate_hash()[:8]}",
	...
	def _mk_journey(self, demand_id: str) -> str:
		jc = f"JRN-REL-{frappe.generate_hash()[:8]}"
		now = now_datetime()
		frappe.db.sql(
			"""
			...
			""",
			(jc, now, now, jc, f"Release test journey {jc}", demand_id),
		)
	...
	def _create_draft_package(self) -> tuple[str, str, str, str, str]:
		...
		item_code = f"DEMITEM-REL-{frappe.generate_hash()[:8]}"
		frappe.db.commit()
	...
	def test_release_success(self) -> None:
		...
		with self._release_patches(has_tender=True):
			out = release_package_to_tender_management(package_code, "Administrator")
		...
		self._track("Procurement Handoff Card", expected_release)
		...
		self.assertEqual(self._audit_count("Package Released to Tender Management", expected_release), 1)
		self.assertEqual(self._audit_count("Package Locked After Release", package_code), 1)

		release_audit = frappe.get_all(
			"Planning Audit Event",
			filters={
				"object_code": expected_release,
				"event_type": "Package Released to Tender Management",
			},
			fields=["evidence_ref"],
			limit=1,
		)
		self.assertEqual((release_audit[0] if release_audit else {}).get("evidence_ref"), expected_release)
	...
	def test_blocked_invalid_state(self) -> None:
		...
		if not self._require_template():
			self.skipTest("No active Procurement Template with profiles available")
		package_code, _, _ = self._approved_with_passing_readiness()
		frappe.db.commit()
		...
		self.assertTrue(
			PackageReleaseToTender.INVALID_STATE in err_text
			or "ready for release" in err_text.lower()
		)
		self.assertEqual(
	...
	def test_blocked_plan_inactive(self) -> None:
		if self._skip:
			self.skipTest("PP or Demand not installed")
		if not self._require_template():
			self.skipTest("No active Procurement Template with profiles available")
		package_code, journey_code, demand_code, bl_code, plan_name = self._create_draft_package()
		record_package_method_decision(package_code, _works_payload(), "Administrator")
		self._track("Package Method Decision", f"METHDEC-{package_code}")
		self._approve_package(package_code)
		self._seed_upstream_handoffs(journey_code, demand_code, bl_code)
		...
		run_out = run_package_readiness_checks(package_code, "Administrator")
		if run_out.get("readiness_code"):
			self._track("Package Readiness Result", run_out.get("readiness_code"))
		mark_package_ready_for_release(package_code, "Administrator")
		...
		err_text = str(ctx.exception)
		self.assertTrue(
			PackageReleaseToTender.PLAN_NOT_ACTIVE in err_text
			or "active" in err_text.lower()
		)
	...
	def test_blocked_readiness_stale(self) -> None:
		...
		if not self._require_template():
			self.skipTest("No active Procurement Template with profiles available")
		package_code, _journey_code = self._ready_for_release_package()
		frappe.db.commit()
		...
		err_text = str(ctx.exception)
		self.assertTrue(
			PackageReleaseToTender.READINESS_STALE in err_text
	...
	def test_blocked_handoff_incomplete(self) -> None:
		if self._skip:
			self.skipTest("PP or Demand not installed")
		if not self._require_template():
			self.skipTest("No active Procurement Template with profiles available")
		...
		with self._release_patches(has_tender=False):
			with self.assertRaises(frappe.ValidationError) as ctx:
				release_package_to_tender_management(package_code, "Administrator")
		err_text = str(ctx.exception)
		self.assertTrue(
			PackageReleaseToTender.HANDOFF_INCOMPLETE in err_text
			or "handoff" in err_text.lower()
		)
		self.assertEqual(
			frappe.db.get_value("Procurement Package", package_code, "status"),
			PKG_READY_FOR_RELEASE,
		)
		self.assertFalse(frappe.db.exists("Procurement Handoff Card", expected_release))
	...
	def test_idempotent_recall(self) -> None:
		...
		with self._release_patches(has_tender=True):
			first = release_package_to_tender_management(package_code, "Administrator")
			self._track("Procurement Handoff Card", expected_release)

			second = release_package_to_tender_management(package_code, "Administrator")

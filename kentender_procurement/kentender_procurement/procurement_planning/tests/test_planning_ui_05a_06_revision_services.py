...
class TestPlanningUI05A06RevisionServices(IntegrationTestCase):
	...
	def test_editor_projects_exact_admitted_contract(self):
		...
		self.assertIn("Open tender", result["method_options"])
		self.assertIn(result["method_catalogue_source"], {"catalogue", "doctype_options", "fallback"})
		self.assertTrue(result["method_recommendation_reason_code"].startswith("PROCUREMENT_METHOD_"))
	...
	def test_editor_rejects_unknown_and_unconfigured_method(self):
		...
		method = update_plan_item(plan_item=item, user=planner, fields={"procurement_method": "Uncatalogued method test sentinel"}, expected_version_token=token, idempotency_key="UI06-ALT")

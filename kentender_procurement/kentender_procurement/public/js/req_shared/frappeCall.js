// Shared frappe.call wrapper for the Procurement Requisitions data adapter.
// Verbatim copy of kentender_procurement's own pln_shared/frappeCall.js
// (AGENTS.md §6.6 — a page-level component object can't cross a bundle
// boundary, but a pure helper with no Vue/component identity carries no such
// restriction; each app/module keeps its own copy rather than importing
...
export async function frappeCall(method, args) {
	try {
		...
		// error summary (AGENTS.md §6.10): the same refusal rendered twice.
		// The message itself still reaches the caller through extractErrorMessage below.

...
(function () {
	...
	function open(options) {
		...
		function renderRows() {
			...
			var html = rows.map(function (row) { var checked = selected.has(row.demand); return '<tr class="hover:bg-surface-container-low transition-colors relative' + (checked ? ' kt-pln-selected-row' : '') + '" data-demand="' + esc(row.demand) + '" data-kt-pln-elig-row><td class="px-4 py-4 whitespace-nowrap" data-kt-col="select"><input class="h-4 w-4 text-primary focus:ring-primary border-border-subtle rounded cursor-pointer" aria-label="Select ' + esc(row.title) + '" type="checkbox" data-kt-demand-select data-kt-pln-elig-check ' + (checked ? "checked" : "") + '></td><td class="px-4 py-4" data-kt-col="primary"><div class="font-body-sm text-body-sm text-on-surface font-medium mb-1" data-kt-pln-elig-title>' + esc(row.title) + '</div><div class="font-data-md text-xs text-on-surface-variant">' + esc(row.demand_code) + '</div></td><td class="px-4 py-4" data-kt-col="text" data-kt-pln-elig-ou-cell><div class="font-body-sm text-body-sm text-on-surface">' + esc(row.organisation_unit_label) + '</div></td><td class="px-4 py-4 text-right font-data-md" data-kt-col="count">' + row.need_item_count + '</td><td class="px-4 py-4 whitespace-nowrap text-right font-data-md" data-kt-col="money" data-kt-pln-elig-row-amount>' + esc(row.available_to_plan_display) + '</td><td class="px-4 py-4 whitespace-nowrap font-body-sm" data-kt-col="date">' + esc(dateLabel(row.required_by)) + '</td><td class="px-4 py-4 font-body-sm" data-kt-col="text" data-kt-pln-elig-funding-cell>' + esc(row.proposed_budget_line_display || "—") + '</td><td class="px-4 py-4 whitespace-nowrap" data-kt-col="status"><span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-label-caps text-[10px] font-bold bg-status-available/10 text-status-available border border-status-available/20">Planning Ready</span></td></tr>'; }).join("");
			if (!html) html = '<tr class="kt-pln-demand-empty"><td colspan="8"><span class="material-symbols-outlined" aria-hidden="true">inventory_2</span><strong>No Approved Demands are available to add</strong><span>All eligible Demands are already represented in this Draft Plan, or none match the current context.</span></td></tr>';
			$body.html(html); renderSummary();
		}

...
"""TPR-CHG-001 v0.6 §11.2 command envelope.

Every mutating command runs inside this envelope: a required idempotency key
(replayed verbatim from the Tender Preparation Command Journal on retry,
rejected on reuse with a different payload), a required expected record
version checked under a row lock, and a monotonic bump on success. Copied
from Procurement Requisitions' proven `envelope.py` (itself Planning's) —
same mechanics, this module's own journal doctype and error contract; see
AGENTS.md §4.2, "reuse existing services".
"""
...
def replay_or_none(idempotency_key: str, payload: dict[str, Any]) -> dict[str, Any] | None:
	...
	if not key:
		fail("TPR_STALE_VERSION", "An idempotency key is required.")
...
def record_command(
	*,
	...
	frappe.get_doc(
		{
			"doctype": "Tender Preparation Command Journal",
...
def assert_task_token(task_doc, presented_token: str) -> None:
	if cstr(presented_token) == "" or cstr(task_doc.task_token) != cstr(presented_token):
		fail("TPR_STALE_VERSION", "This task has already changed. Reload to see the current decision.")


@contextmanager
def atomic(_label: str = ""):
	"""§10.5 — a named savepoint around the Tender-owned writes in
	`ApproveTenderForPublication` (decision, approved Version, package,
	handoff and files commit together or not at all) and the other
	multi-record lifecycle commands. A failure *inside* this block rolls back
	to the savepoint without discarding the caller's own outer transaction —
	the same reasoning Requisitions' envelope gives for why direct Python
	callers (seeds, tests) cannot rely on `frappe.handler`'s
	catch-and-rollback wrapper."""
	savepoint = f"tpr_{uuid4().hex[:12]}"

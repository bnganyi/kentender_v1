...
"""Render Preview contracts for ITW-14."""
...
RENDER_PREVIEW_STEP_CODE = STEP_CODE
...
def _default_checklist(*, complete: bool) -> list[dict[str, Any]]:
	rows = [
		{"item_code": "PREV-SECTIONS", "title": "All sections present in preview", "confirmed": complete},
		{"item_code": "PREV-LOCKED", "title": "Locked clauses render read-only", "confirmed": complete},
		{"item_code": "PREV-PARAMS", "title": "Configured parameters substituted", "confirmed": complete},
	]
	return rows


def _ensure_render_preview_doc(instance_name: str, *, seed_complete: bool | None = None):
	name = _dedupe_docs("Tender STD Render Preview", instance_name)
	if name:
		return frappe.get_doc("Tender STD Render Preview", name)
	checklist = _default_checklist(complete=bool(seed_complete))
	doc = frappe.get_doc(
		{
			"doctype": "Tender STD Render Preview",
			"tender_std_instance": instance_name,
			"preview_code": STEP_CODE,
			"preview_title": "Render Preview",
			"description": "Confirm the generated tender preview before publication.",
			"review_status": "APPROVED" if seed_complete else "DRAFT",
			"lock_status": "UNLOCKED",
			"confirmation_checklist_json": _dump_checklist(checklist),
			"package_summary": "IT tender configuration package preview.",
			"status": "CONFIRMED" if seed_complete else "DRAFT",
		}
	)
	try:
...
def _build_payload(configuration_id: str, doc, overview: dict[str, Any]) -> dict[str, Any]:
	...
	payload.update(
		{
			"preview_code": str(doc.preview_code or "").strip(),
			"preview_title": str(doc.preview_title or "").strip(),
...
def get_render_preview(configuration_id: str) -> dict[str, Any]:
	assert_permission(PERM_VIEW)
	instance = _get_instance(configuration_id)
	doc = _ensure_render_preview_doc(instance.name)
	return _build_payload(configuration_id, doc, build_configuration_overview(configuration_id))


def save_render_preview(configuration_id: str, payload: dict[str, Any]) -> dict[str, Any]:
	...
	if str(doc.lock_status or "").strip() != "UNLOCKED":
		frappe.throw(
			f"Render preview is locked with status {doc.lock_status}.",
			title="ITW_PREVIEW_LOCKED",
	...
	if payload.get("preview_title") is not None:
		doc.preview_title = str(payload.get("preview_title") or "").strip()
	...
	checklist = _parse_checklist(doc.confirmation_checklist_json)
	complete = bool(checklist) and all(item.get("confirmed") for item in checklist)
	if complete and doc.status == "DRAFT":

...
class TestBe09StdProdEngine(UnitTestCase):
	def test_engine_exposes_read_client_and_defaults(self) -> None:
		...
		self.assertIn("MODULE_ROW_MAP_FOCUS", source)
		self.assertIn("map_focus", source)
		self.assertIn("data-std-prod-map-focus", source)
		self.assertIn("data-std-prod-table-footer", source)
		self.assertIn("ensure_breadcrumb_nav", source)
		self.assertIn("data-std-prod-page-size", source)
		self.assertIn("__stdProdFormRowTemplate", source)
		self.assertIn("__stdProdRequirementRowTemplate", source)
		self.assertIn("Clause Inventory", source)

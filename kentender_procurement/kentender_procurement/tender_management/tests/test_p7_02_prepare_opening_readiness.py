...
"""P7-02 — doc 9 §12.2 ``prepare_opening_readiness`` / ``prepareOpeningReadiness``.

Run::

	bench --site kentender.midas.com run-tests --app kentender_procurement \\
		--module kentender_procurement.tender_management.tests.test_p7_02_prepare_opening_readiness
"""
...
class TestP702PrepareOpeningReadiness(_P401Tm2Cleanup, P6PublishedTm2Fixture):
	p6_supplier_fixture_prefix = "P702"
	...
	def test_p7_02_prepare_happy_path(self) -> None:
		tcode, tm2, _sup, si, _cl = self._closed_with_one_sealed_bid()
		out = prepare_opening_readiness("Administrator", tcode, context=self._prepare_ctx())
		self.assertTrue(out.get("ok"), out)
		self.assertEqual(out.get("tender_status"), "Opening Ready")
		self.assertEqual(out.get("opening_readiness_code"), f"ORR-{tcode}")
		self.assertEqual(frappe.db.get_value("TM2 Tender", tm2, "status"), "Opening Ready")
		orr_name = str(out.get("tm2_opening_readiness_record") or "")
		self.assertTrue(orr_name)
		orr = frappe.get_doc("TM2 Opening Readiness Record", orr_name)
		self.assertEqual(orr.readiness_status, "Ready")
		self.assertEqual(orr.dom_output_code, out.get("dom_output_code"))
		self.assertEqual(orr.tender_std_instance_code, si)
		self.assertEqual(orr.valid_submission_count, 1)
		refs_raw = orr.sealed_submission_refs
		refs = json.loads(refs_raw) if isinstance(refs_raw, str) else refs_raw
		self.assertIsInstance(refs, dict)
		self.assertIsInstance(refs.get("refs"), list)
		self.assertEqual(len(refs.get("refs")), 1)
		ev = frappe.get_all(
			"TM2 Tender Audit Event",
			filters={"tm2_tender": tm2, "event_type": "Opening Readiness Created"},
			pluck="name",
		)
		self.assertEqual(len(ev), 1)

	def test_p7_02_prepare_not_closed_denied(self) -> None:
	...
	def test_p7_02_prepare_closed_no_valid_submissions_denied(self) -> None:
		tcode, tm2, _sup, _si = self._published_si_supplier()
		self._past_deadline(tm2)
		out_c = close_tender("Administrator", tcode, context=self._close_ctx())
		self.assertTrue(out_c.get("ok"), out_c)
		self.assertEqual(out_c.get("tender_status"), "Closed - No Valid Submissions")
		out = prepare_opening_readiness("Administrator", tcode, context=self._prepare_ctx())
		self.assertFalse(out.get("ok"))
		self.assertEqual(out.get("denial_code"), DenialCode.AUTH_STATE_DENIED.value)

	def test_p7_02_prepare_second_call_duplicate_denied(self) -> None:
		tcode, _tm2, _sup, _si, _cl = self._closed_with_one_sealed_bid()
		ctx = self._prepare_ctx()
		out1 = prepare_opening_readiness("Administrator", tcode, context=ctx)
		self.assertTrue(out1.get("ok"), out1)
		out2 = prepareOpeningReadiness("Administrator", tcode, context=ctx)
		self.assertFalse(out2.get("ok"))
		self.assertEqual(out2.get("denial_code"), DenialCode.AUTH_CONTEXT_DENIED.value)

	def test_p7_02_prepare_role_denied_without_permission(self) -> None:
		tcode, _tm2, _sup, _si, _cl = self._closed_with_one_sealed_bid()
		out = prepare_opening_readiness("Administrator", tcode, context={"granted_permissions": []})
		self.assertFalse(out.get("ok"))
		self.assertEqual(out.get("denial_code"), DenialCode.AUTH_ROLE_DENIED.value)

	def test_p7_02_prepare_binding_snapshot_mismatch_denied(self) -> None:
		tcode, tm2, _sup, _si, _cl = self._closed_with_one_sealed_bid()
	...
	def test_p7_02_prepare_dom_not_consumable_denied(self) -> None:
		tcode, tm2, _sup, si, _cl = self._closed_with_one_sealed_bid()
		dom_name = frappe.db.get_value("Tender STD Instance", si, "current_dom_output_code")
		self.assertTrue(dom_name)
		if frappe.db.exists("Tender STD Generated Output", dom_name):
			frappe.db.set_value(
			...
			try:
				out = prepare_opening_readiness("Administrator", tcode, context=self._prepare_ctx())
				self.assertFalse(out.get("ok"))
				self.assertEqual(out.get("denial_code"), DenialCode.AUTH_DOM_MISSING_OR_STALE.value)
			finally:
				frappe.db.set_value(
					"Tender STD Generated Output",
					dom_name,
	...
	def test_p7_02_prepare_unsealed_bid_denied(self) -> None:
		tcode, tm2, _sup, _si, _cl = self._closed_with_one_sealed_bid()
	...
	def test_p7_02_prepare_publication_snapshot_adapter_denied(self) -> None:
		tcode, _tm2, _sup, _si, _cl = self._closed_with_one_sealed_bid()
		bad = {
			"ok": False,
		...
		with patch(
			"kentender_procurement.tender_management.services.prepare_opening_readiness.get_tender_std_output_refs",
			return_value=bad,
		):
			out = prepare_opening_readiness("Administrator", tcode, context=self._prepare_ctx())
		self.assertFalse(out.get("ok"))

...
_TECHNICAL_JSON_ROLES: frozenset[str] = frozenset(
	{
		"Administrator",
		"System Manager",
...
_CONFIGURATOR_WRITE_ROLES: frozenset[str] = frozenset(
	{
		"Administrator",
		"System Manager",
		"STD Template Administrator",
...
def _assert_technical_json_access() -> None:
	...
	if frappe.session.user == "Administrator":
		return
	if _has_any_role(_TECHNICAL_JSON_ROLES):
		return
	frappe.throw(_("Not permitted"), frappe.PermissionError)
		...
		tabs.append(
			{
				...
				"read_only": slug in {"preview", "approval", "technical-json"},
			}
		)
	return tabs
...
def get_configurator_context(template_code: str) -> dict[str, Any]:
	...
	return {
		...
		"title": doc.template_title or doc.template_name or doc.template_code,
		"lifecycle_status": doc.lifecycle_status,
		"editable": editable,
		"can_view_technical_json": _has_any_role(_TECHNICAL_JSON_ROLES),
...
def get_section(template_code: str, section: str) -> dict[str, Any]:
	...
	package = _parse_package_json(doc)
	std_config = _ensure_std_config(package)
	data = std_config.get(key)
	if data is None:
		data = _section_default(key)

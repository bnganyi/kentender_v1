...
"""Qualification overview — /tenders/<publication_ref>/sections/qualification_and_capability."""
...
def get_context(context):
	...
	if not publication_ref:
		raise frappe.DoesNotExistError(_("Qualification and Capability not found."))

	path = portal_qualification_url(publication_ref)
	if frappe.session.user == "Guest":
		frappe.local.flags.redirect_location = "/login?redirect-to=" + quote(path, safe="")
		raise frappe.Redirect

	try:
		sec = get_qualification_and_capability(publication_ref)
	...
	context.title = sec.get("section_title") or _("Qualification and Capability")
	return context

...
"""Preliminary Requirements — Website at /tenders/<publication_ref>/sections/preliminary_requirements_and_evidence."""
...
def get_context(context):
	...
	if not publication_ref:
		raise frappe.DoesNotExistError(_("Preliminary Requirements not found."))

	path = portal_preliminary_url(publication_ref)
	if frappe.session.user == "Guest":
		frappe.local.flags.redirect_location = "/login?redirect-to=" + quote(path, safe="")
		raise frappe.Redirect

	try:
		sec = get_preliminary_requirements(publication_ref)
	...
	context.title = sec.get("section_title") or _("Preliminary Requirements and Evidence")
	return context

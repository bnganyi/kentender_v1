...
"""X100 Evidence Register — Website at /tenders/<publication_ref>/evidence."""
...
from kentender_procurement.tender_configurations.services.bid_issues import portal_issues_url
...
def get_context(context):
	...
	evidence_path = portal_evidence_url(publication_ref)
	if frappe.session.user == "Guest":
		frappe.local.flags.redirect_location = "/login?redirect-to=" + quote(evidence_path, safe="")
		raise frappe.Redirect

	try:
		register = get_evidence_register(publication_ref)
	except frappe.PermissionError:
		raise
	except Exception:
		raise frappe.DoesNotExistError(_("Published tender not found."))

	# Sidebar shell fields shared with A2/A3.
	shell = {
		...
		"evidence_url": register.get("evidence_url") or evidence_path,
		"issues_url": portal_issues_url(publication_ref),
	}
	...
	context.title = _("Evidence Register")
	return context

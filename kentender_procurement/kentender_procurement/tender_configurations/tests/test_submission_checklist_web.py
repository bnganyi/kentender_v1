...
"""A2 Website Submission Checklist — /tenders/<publication_ref>/workspace."""
...
class TestSubmissionChecklistWeb(IntegrationTestCase):
	def setUp(self):
		clear_website_cache()
		...
		frappe.db.set_value(
			...
			self.cfg_id,
			{
				...
				"bidder_submission_schema": json.dumps(
					{"version": 1, "sections": [{"key": "eligibility", "title": "Eligibility", "required": True}]}
				),
				"short_scope_summary": "A2W checklist web scope.",
			},
		)
	...
	def _publish(self):
		...
		save_publication_setup(
			pub_id,
			{
				...
				"tender_notice": "A2W checklist notice.",

...
"""TPR-CHG-001 v0.8 §3 "Installed template release" / plan D3 — the read-only template registry row (relocated from the retired Tender Preparation module).
...
release (operational users cannot edit or choose another release).
"""
...
class SupportedTenderTemplate(Document):
	def validate(self) -> None:
		if not self.flags.get("kt_template_install"):
			frappe.throw("The Supported Tender Template registry is written only by the template installer (TPR-CHG-001 v0.8 §3).")

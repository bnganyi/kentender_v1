/**
 * P5C-014 — Include-in-Plan success state journey.
 */
...
test.describe('P5C-014 Include success journey', () => {
	...
	test('shows include success summary with Create Package next action', async ({ page }) => {
		...
		await expect(success).toContainText('Demand added to the procurement plan.');
		...
		await expect(success).not.toContainText('PLANINCL-MOH-2026-001');
		await page.screenshot({ path: 'p5c014_include_plan_success.png', fullPage: true });
	});

	test('Create Package routes to packages surface and Back keeps approved-demands context', async ({
		page,
	}) => {
		await submitInclude(page);
		...
		await expandRightPanel(page);
		await submitInclude(page);
		await page.getByTestId('pp2-back-to-approved-demands').click();
		await expect(page.getByTestId('pp2-approved-demands-page')).toBeVisible({ timeout: 30000 });
		await expect(page.getByTestId('pp2-include-plan-success')).toHaveCount(0);
	});
});

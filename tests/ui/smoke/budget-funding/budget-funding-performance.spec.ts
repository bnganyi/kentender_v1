...
test.describe("Budget Funding Performance (BUD-UI-02)", () => {
	...
	test("live root shows KPIs, pack codes, full KES, tables, Export, disclaimer", async ({
		...
		await expect(root).toContainText("MOH-TGT-AVAIL-2028");
	...
	test("target filter narrows coverage; exception action without Message dialog", async ({
		...
		await expect(targetFilter.locator('option[value="MOH-TGT-AVAIL-2028"]')).toHaveCount(1, {
			timeout: 15_000,
		});
		await targetFilter.selectOption("MOH-TGT-AVAIL-2028");
		...
		await expect(
			root.locator(
				'[data-testid="kt-bud-performance-coverage-row"][data-target-code="MOH-TGT-AVAIL-2028"]',
			),

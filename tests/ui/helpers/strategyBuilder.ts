...
export async function submitNewNodeDialog(
	...
	if (isTargetDialog) {
		...
		const period = d.locator(
			'[data-fieldname="target_period_value"] input, .form-group[data-fieldname="target_period_value"] input',
		);
		if ((await period.count()) > 0) {
			await period.fill(opts.targetYear ?? String(new Date().getFullYear()));
		}
		...
		if ((await period.count()) > 0) {
			await period.fill(opts.targetYear);
		}
	}
